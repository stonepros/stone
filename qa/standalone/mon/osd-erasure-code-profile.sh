...
function run() {
    ...
    export STONE_MON="127.0.0.1:7220" # git grep '\<7220\>' : there must be only one
    export STONE_ARGS
...
function TEST_set() {
    ...
    #
    stone osd erasure-code-profile set $profile 2>&1 || return 1
    stone osd erasure-code-profile get $profile | \
    ...
    #
    ! stone osd erasure-code-profile set $profile > $dir/out 2>&1 || return 1
    grep 'will not override' $dir/out || return 1
    stone osd erasure-code-profile set $profile key=other --force || return 1
    stone osd erasure-code-profile get $profile | \
        grep key=other || return 1

    stone osd erasure-code-profile rm $profile # cleanup
}
...
function TEST_ls() {
    ...
    local profile=myprofile
    ! stone osd erasure-code-profile ls | grep $profile || return 1
    stone osd erasure-code-profile set $profile 2>&1 || return 1
    stone osd erasure-code-profile ls | grep $profile || return 1
    stone --format xml osd erasure-code-profile ls | \
...
function TEST_rm() {
    ...
    stone osd erasure-code-profile set $profile 2>&1 || return 1
    stone osd erasure-code-profile ls | grep $profile || return 1
    stone osd erasure-code-profile rm $profile || return 1
    ! stone osd erasure-code-profile ls | grep $profile || return 1
    stone osd erasure-code-profile rm WRONG 2>&1 | \
        grep "WRONG does not exist" || return 1

    stone osd erasure-code-profile set $profile || return 1
    create_pool poolname 12 12 erasure $profile || return 1
    ! stone osd erasure-code-profile rm $profile > $dir/out 2>&1 || return 1
    grep "poolname.*using.*$profile" $dir/out || return 1
    stone osd pool delete poolname poolname --yes-i-really-really-mean-it || return 1
    stone osd erasure-code-profile rm $profile || return 1
...
function TEST_get() {
    ...
    stone osd erasure-code-profile get $default_profile | \
        grep plugin=jerasure || return 1
    stone --format xml osd erasure-code-profile get $default_profile | \
        grep '<plugin>jerasure</plugin>' || return 1
    ! stone osd erasure-code-profile get WRONG > $dir/out 2>&1 || return 1
...
function TEST_set_idempotent() {
    ...
    # stone osd erasure-code-profile set: verify that it is idempotent,
    # as if it was using the same code path.
    #
    stone osd erasure-code-profile set default k=2 m=2 2>&1 || return 1
    local profile
    #
    ...
    ! stone osd erasure-code-profile ls | grep $profile || return 1
    stone osd erasure-code-profile set $profile k=2 crush-failure-domain=osd 2>&1 || return 1
    stone osd erasure-code-profile ls | grep $profile || return 1
    stone osd erasure-code-profile set $profile k=2 crush-failure-domain=osd 2>&1 || return 1
    stone osd erasure-code-profile rm $profile # cleanup
    ...
    ! stone osd erasure-code-profile ls | grep $profile || return 1
    stone osd erasure-code-profile set $profile plugin=lrc k=4 m=2 l=3 crush-failure-domain=osd 2>&1 || return 1
    stone osd erasure-code-profile ls | grep $profile || return 1
    stone osd erasure-code-profile set $profile plugin=lrc k=4 m=2 l=3 crush-failure-domain=osd 2>&1 || return 1
    stone osd erasure-code-profile rm $profile # cleanup
}
...
function TEST_format_json() {
    ...
    stone --format json osd erasure-code-profile get default | \
...
function TEST_invalid_crush_failure_domain() {
    ...
    ! stone osd erasure-code-profile set $profile k=4 m=2 crush-failure-domain=$crush_failure_domain 2>&1 || return 1
}
