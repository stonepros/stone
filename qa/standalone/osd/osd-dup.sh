...
function run() {
    ...
    STONE_ARGS+="--filestore_wbthrottle_xfs_ios_hard_limit=900 "
    STONE_ARGS+="--filestore_wbthrottle_btrfs_ios_hard_limit=900 "
...
function TEST_filestore_to_bluestore() {
    ...
    echo "osd fsid $ofsid"
    O=$STONE_ARGS
    STONE_ARGS+="--log-file $dir/cot.log --log-max-recent 0 "
    stone-objectstore-tool --type bluestore --data-path $dir/0 --fsid $ofsid \
			  --op mkfs --no-mon-config || return 1
    stone-objectstore-tool --data-path $dir/0.old --target-data-path $dir/0 \
			  --op dup || return 1
    STONE_ARGS=$O

    activate_osd $dir 0 || return 1

    while ! stone osd stat | grep '3 up' ; do sleep 1 ; done
    stone osd metadata 0 | grep bluestore || return 1
