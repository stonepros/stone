...
function do_recovery_out1() {
    ...
    stone tell osd.$(get_primary $poolname obj1) debug kick_recovery_wq 0
    sleep 2
...
function TEST_recovery_sizeup() {
    ...
    stone osd set norecover
    stone osd out osd.$primary osd.$otherosd
    stone osd pool set test size 4
    stone osd unset norecover
    # Get new primary
...
function TEST_recovery_sizedown() {
    ...
    stone osd pool set test size 2
    stone osd unset norecover
    stone tell osd.$(get_primary $poolname obj1) debug kick_recovery_wq 0
    sleep 2
...
function TEST_recovery_undersized() {
    ...
    for i in $(seq 1 60)
    do
      if stone pg dump pgs | grep ^$PG | grep -qv recovering
      then
          break
      fi
      if [ $i = "60" ];
...
function TEST_recovery_multi() {
    ...
    stone osd pool set $poolname min_size 1
