...
# The main point of these tests beyond stone_test_libstonesqlite is to:
#
# - Ensure you can load the Stone VFS via the dynamic load extension mechanism
#   in SQLite.
...
function sqlite {
  ...
  sqlite3 -cmd '.output /dev/null' -cmd '.load libstonesqlite.so' -cmd 'pragma journal_mode = PERSIST' -cmd ".open file:///$pool:$ns/baz.db?vfs=stone" -cmd '.output stdout' <<<"$a" &
  if [ "$background" != b ]; then
    wait
  fi
}
...
jobs -l
kill -STOP -- $(jobs -p)
# stonesqlite_lock_renewal_timeout is 30s
sleep 45
date
kill -CONT -- $(jobs -p)
sleep 10
