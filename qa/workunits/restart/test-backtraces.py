...
def mkdir(stone, d):
    ...
    stone.mkdir(d, 0o755)
    return stone.stat(d)['st_ino']

def create(stone, f):
    print("creating {f}".format(f=f), file=sys.stderr)
    fd = stone.open(f, os.O_CREAT | os.O_RDWR, 0o644)
    stone.close(fd)
    return stone.stat(f)['st_ino']

def set_mds_config_param(stone, param):
    with open('/dev/null', 'rb') as devnull:
        confarg = ''
        if conf != '':
            confarg = '-c {c}'.format(c=conf)
        r = subprocess.call("stone {ca} mds tell a injectargs '{p}'".format(ca=confarg, p=param), shell=True, stdout=devnull)
        if r != 0:
            raise Exception
...
def flush(stone, testnum):
    print('flushing {t}'.format(t=testnum), file=sys.stderr)
    set_mds_config_param(stone, '--mds_log_max_segments 1')
    ...
    stone.shutdown()
    print('flush reinitializing stone', file=sys.stderr)
    stone = stonefs.LibStoneFS(conffile=conf)
    print('flush doing mount', file=sys.stderr)
    stone.mount()
    return stone

def kill_mds(stone, location, killnum):
    print('killing mds: {l}.{k}'.format(l=location, k=killnum), file=sys.stderr)
    set_mds_config_param(stone, '--mds_kill_{l}_at {k}'.format(l=location, k=killnum))

def wait_for_mds(stone):
    # wait for restart
    while True:
        confarg = ''
        if conf != '':
            confarg = '-c {c}'.format(c=conf)
        r = subprocess.check_output("stone {ca} mds stat".format(ca=confarg), shell=True).decode()
...
rooti = stone.stat('/')['st_ino']

test = -1
if len(sys.argv) > 1:
    test = int(sys.argv[1])

conf = '/etc/stonepros/stone.conf'
if len(sys.argv) > 2:
    conf = sys.argv[2]
...
if test < 0 or test == i:
  ...
  ino, expected_bt = make_abc(stone, rooti, i)
  stone = flush(stone, i)
