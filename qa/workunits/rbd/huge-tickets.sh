...
# This is a test for http://tracker.stone.com/issues/8979 and the fallout
...
# them to libstone (through 'rbd map' on a scratch image).  Bad kernels
...
function generate_keyring() {
    ...
    stone-authtool -C -n client.$user --cap mon 'allow *' --gen-key /tmp/keyring-$user
...
for user in {foo,bar,baz}; do
    stone auth import -i /tmp/keyring-$user
