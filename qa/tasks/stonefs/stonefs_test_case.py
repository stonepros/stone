...
def needs_trimming(f):
    """
    Mark fn as requiring a client capable of trimming its cache (i.e. for stone-fuse
...
class MountDetails():
    ...
    def __init__(self, mntobj):
        ...
        self.stonefs_name = mntobj.stonefs_name
        self.stonefs_mntpt = mntobj.stonefs_mntpt
    ...
    def restore(self, mntobj):
        ...
        mntobj.stonefs_name = self.stonefs_name
        mntobj.stonefs_mntpt = self.stonefs_mntpt
        mntobj.hostfs_mntpt = self.hostfs_mntpt


class StoneFSTestCase(StoneTestCase):
    """
    Test case for Stone FS, requires caller to populate Filesystem and Mounts,
    ...
    def setUp(self):
        super(StoneFSTestCase, self).setUp()
        ...
        try:
            ...
            # Fallback for older Stone cluster
    ...
    def tearDown(self):
        ...
        # To prevent failover messages during Unwind of stone task
        self.mds_cluster.delete_all_filesystems()
        ...
        for subsys, key in self.configs_set:
            self.mds_cluster.clear_stone_conf(subsys, key)

        return super(StoneFSTestCase, self).tearDown()

    def set_conf(self, subsys, key, value):
        self.configs_set.add((subsys, key))
        self.mds_cluster.set_stone_conf(subsys, key, value)

    def auth_list(self):
        """
        Convenience wrapper on "stone auth ls"
        """
    ...
    def delete_mds_coredump(self, daemon_id):
        ...
        if core_dir:  # Non-default core_pattern with a directory in it
            ...
            cores = [l.partition(":")[0]
                     ...
                     if re.match(r'.*stone-mds.* -i +{0}'.format(daemon_id), l)]
