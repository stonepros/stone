"""
Before running this testsuite, add path to stonefs-shell module to $PATH and
export $PATH.
"""
...
class TestStoneFSShell(StoneFSTestCase):
    CLIENTS_REQUIRED = 1

    def setUp(self):
        super(TestStoneFSShell, self).setUp()

        conf_contents = "[stonefs-shell]\ncolors = False\ndebug = True\n"
    ...
    def run_stonefs_shell_cmd(self, cmd, mount_x=None, shell_conf_path=None,
        ...
        args = ["stonefs-shell", "-c", shell_conf_path]
        if opts:
            args += opts
        args.extend(("--", cmd))
    ...
    def negtest_stonefs_shell_cmd(self, **kwargs):
        """
        This method verifies that stonefs shell command fails with expected
        return value and/or error message.

        kwargs is expected to hold the arguments same as
        run_stonefs_shell_cmd() with the following exceptions -
        ...
        try:
            proc = self.run_stonefs_shell_cmd(args=['some', 'cmd'],
                                             check_status=False,
                                             stdout=stdout)
        ...
        proc = self.run_stonefs_shell_cmd(**kwargs)
        if retval:
    ...
    def get_stonefs_shell_cmd_output(self, cmd, mount_x=None,
                                    shell_conf_path=None, opts=None,
                                    stdout=None, stdin=None,check_status=True):
        return ensure_str(self.run_stonefs_shell_cmd(
    ...
    def get_stonefs_shell_cmd_error(self, cmd, mount_x=None,
                                   shell_conf_path=None, opts=None,
    ...
    def run_stonefs_shell_script(self, script, mount_x=None,
        ...
        scriptpath = tempfile_mkstemp(prefix='test-stonefs', text=True)[1]
        with open(scriptpath, 'w') as scriptfile:
            scriptfile.write(script)
        # copy script to the machine running stonefs-shell.
        mount_x.client_remote.put_file(scriptpath, scriptpath)
        mount_x.run_shell_payload(f"chmod 755 {scriptpath}")

        args = ["stonefs-shell", '-b', scriptpath]
        if shell_conf_path:
    ...
    def get_stonefs_shell_script_output(self, script, mount_x=None,
                                       shell_conf_path=None, opts=None,
                                       stdout=None, stdin=None,
                                       check_status=True):
        return ensure_str(self.run_stonefs_shell_script(
            script=script, mount_x=mount_x, shell_conf_path=shell_conf_path,
            opts=opts, stdout=stdout, stdin=stdin,
            check_status=check_status).stdout.getvalue().strip())


class TestMkdir(TestStoneFSShell):
    def test_mkdir(self):
        """
        Test that mkdir creates directory
        """
        o = self.get_stonefs_shell_cmd_output("mkdir d1")
        log.info("stonefs-shell output:\n{}".format(o))
    ...
    def test_mkdir_with_07000_octal_mode(self):
        ...
        """
        self.negtest_stonefs_shell_cmd(cmd="mkdir -m 07000 d2")
        try:
    ...
    def test_mkdir_with_negative_octal_mode(self):
        ...
        """
        self.negtest_stonefs_shell_cmd(cmd="mkdir -m -0755 d3")
        try:
    ...
    def test_mkdir_with_non_octal_mode(self):
        ...
        """
        o = self.get_stonefs_shell_cmd_output("mkdir -m u=rwx d4")
        log.info("stonefs-shell output:\n{}".format(o))
    ...
    def test_mkdir_with_bad_non_octal_mode(self):
        ...
        """
        self.negtest_stonefs_shell_cmd(cmd="mkdir -m ugx=0755 d5")
        try:
    ...
    def test_mkdir_path_without_path_option(self):
        ...
        """
        self.negtest_stonefs_shell_cmd(cmd="mkdir d5/d6/d7")
        try:
    ...
    def test_mkdir_path_with_path_option(self):
        ...
        """
        o = self.get_stonefs_shell_cmd_output("mkdir -p d5/d6/d7")
        log.info("stonefs-shell output:\n{}".format(o))
...
class TestRmdir(TestStoneFSShell):
    dir_name = "test_dir"
    ...
    def test_rmdir(self):
        ...
        """
        self.run_stonefs_shell_cmd("mkdir " + self.dir_name)
        self.run_stonefs_shell_cmd("rmdir "+ self.dir_name)
    ...
    def test_rmdir_non_existing_dir(self):
        ...
        """
        self.negtest_stonefs_shell_cmd(cmd="rmdir test_dir")
        self.dir_does_not_exists()
    ...
    def test_rmdir_dir_with_file(self):
        ...
        """
        self.run_stonefs_shell_cmd("mkdir " + self.dir_name)
        self.run_stonefs_shell_cmd("put - test_dir/dumpfile", stdin="Valid File")
        self.run_stonefs_shell_cmd("rmdir" + self.dir_name)
        self.mount_a.stat(self.dir_name)
    ...
    def test_rmdir_existing_file(self):
        ...
        """
        self.run_stonefs_shell_cmd("put - dumpfile", stdin="Valid File")
        self.negtest_stonefs_shell_cmd(cmd="rmdir dumpfile")
    ...
    def test_rmdir_p(self):
        ...
        """
        self.run_stonefs_shell_cmd("mkdir -p test_dir/t1/t2/t3")
        self.run_stonefs_shell_cmd("rmdir -p "+ self.dir_name)
        self.dir_does_not_exists()
    ...
    def test_rmdir_p_valid_path(self):
        ...
        """
        self.run_stonefs_shell_cmd("mkdir -p test_dir/t1/t2/t3")
        self.run_stonefs_shell_cmd("rmdir -p test_dir/t1/t2/t3")
        self.dir_does_not_exists()
    ...
    def test_rmdir_p_non_existing_dir(self):
        ...
        """
        self.negtest_stonefs_shell_cmd(cmd="rmdir -p test_dir")
        self.dir_does_not_exists()
...
class TestGetAndPut(TestStoneFSShell):
    def test_without_target_dir(self):
        ...
        self.run_stonefs_shell_cmd('put ' + tempdir)
        for file_ in files:
        ...
        self.run_stonefs_shell_cmd('get ' + tempdirname)
        pwd = self.get_stonefs_shell_cmd_output('!pwd')
        for file_ in files:
            if file_ == tempdirname:
    ...
    def test_get_with_target_name(self):
        ...
        o = self.get_stonefs_shell_cmd_output("put - dump4", stdin=s)
        log.info("stonefs-shell output:\n{}".format(o))
        ...
        o = self.get_stonefs_shell_cmd_output("get dump4 .")
        log.info("stonefs-shell output:\n{}".format(o))

        o = self.get_stonefs_shell_cmd_output("!cat dump4")
    ...
    def test_get_without_target_name(self):
        ...
        """
        s = 'D' * 1024
        o = self.get_stonefs_shell_cmd_output("put - dump5", stdin=s)
        log.info("stonefs-shell output:\n{}".format(o))
        ...
        o = self.get_stonefs_shell_cmd_output("get dump5")
        o = self.get_stonefs_shell_cmd_output("!stat dump5 || echo $?")
        log.info("stonefs-shell output:\n{}".format(o))
        l = o.split('\n')
        try:
            ret = int(l[1])
    ...
    def test_get_to_console(self):
        ...
        s = 'E' * 1024
        s_hash = crypt.crypt(s, '.A')
        o = self.get_stonefs_shell_cmd_output("put - dump6", stdin=s)
        log.info("stonefs-shell output:\n{}".format(o))
        ...
        o = self.get_stonefs_shell_cmd_output("get dump6 -")
        o_hash = crypt.crypt(o, '.A')
        log.info("stonefs-shell output:\n{}".format(o))
...
class TestSnapshots(TestStoneFSShell):
    def test_snap(self):
        """
        ...
        self.get_stonefs_shell_cmd_output("mkdir data_dir")
        s = 'A' * 10240
        o = self.get_stonefs_shell_cmd_output("put - data_dir/data_a", stdin=s)
        s = 'B' * 10240
        o = self.get_stonefs_shell_cmd_output("put - data_dir/data_b", stdin=s)
        s = 'C' * 10240
        o = self.get_stonefs_shell_cmd_output("put - data_dir/data_c", stdin=s)
        s = 'D' * 10240
        o = self.get_stonefs_shell_cmd_output("put - data_dir/data_d", stdin=s)
        s = 'E' * 10240
        o = self.get_stonefs_shell_cmd_output("put - data_dir/data_e", stdin=s)

        o = self.get_stonefs_shell_cmd_output("ls -l /data_dir")
        log.info("stonefs-shell output:\n{}".format(o))

        # create the snapshot - must pass
        o = self.get_stonefs_shell_cmd_output("snap create snap1 /data_dir")
        log.info("stonefs-shell output:\n{}".format(o))
        self.assertEqual("", o)
        ...
        self.negtest_stonefs_shell_cmd(cmd="snap create snap1 /data_dir",
        ...
        o = self.get_stonefs_shell_cmd_output("snap delete snap1 /data_dir")
        log.info("stonefs-shell output:\n{}".format(o))
        self.assertEqual("", o)
        try:
            o = self.mount_a.stat(sdn)
        ...
        self.negtest_stonefs_shell_cmd(cmd="snap delete snap1 /data_dir",
...
class TestCD(TestStoneFSShell):
    ...
    def test_cd_with_no_args(self):
        ...
        script = 'cd {}\ncd\ncwd\n'.format(path)
        output = self.get_stonefs_shell_script_output(script)
...
class TestDU(TestStoneFSShell):
    ...
    def test_du_works_for_regfiles(self):
        ...
        du_output = self.get_stonefs_shell_cmd_output('du ' + regfilename)
    ...
    def test_du_works_for_non_empty_dirs(self):
        ...
        # XXX: we stat `regfile_abspath` here because stone du reports a non-empty
        # directory's size as sum of sizes of all files under it.
        size = humansize(self.mount_a.stat(regfile_abspath)['st_size'])
        expected_output = r'{}{}{}'.format(size, " +", dirname)

        sleep(10)
        du_output = self.get_stonefs_shell_cmd_output('du ' + dirname)
        self.assertRegex(du_output, expected_output)
    ...
    def test_du_works_for_hardlinks(self):
        ...
        du_output = self.get_stonefs_shell_cmd_output('du ' + hlinkname)
        self.assertRegex(du_output, expected_output)
    ...
    def test_du_works_for_softlinks_to_files(self):
        ...
        du_output = self.get_stonefs_shell_cmd_output('du ' + slinkname)
        self.assertRegex(du_output, expected_output)
    ...
    def _setup_files(self, return_path_to_files=False, path_prefix='./'):
        ...
        # TODO: is there a way to trigger/force update stone.dir.rbytes?
        # wait so that attr stone.dir.rbytes gets a chance to be updated.
        sleep(20)
    ...
    def test_du_works_recursively_with_no_path_in_args(self):
        ...
        du_output = self.get_stonefs_shell_cmd_output('du -r')
    ...
    def test_du_with_path_in_args(self):
        ...
        for p in path_to_files:
            args.append(p)
        du_output = self.get_stonefs_shell_cmd_output(args)
    ...
    def test_du_with_no_args(self):
        ...
        du_output = self.get_stonefs_shell_cmd_output('du')

        for expected_output in expected_patterns_in_output:
            # Since CWD is StoneFS root and being non-recursive expect only
            # CWD in DU report.
            if expected_output.find('/') == len(expected_output) - 1:
                self.assertRegex(du_output, expected_output)


class TestDF(TestStoneFSShell):
    def validate_df(self, filename):
        df_output = self.get_stonefs_shell_cmd_output('df '+filename)
        log.info("stonefs-shell df output:\n{}".format(df_output))

        shell_df = df_output.splitlines()[1].split()

        block_size = int(self.mount_a.df()["total"]) // 1024
        log.info("stonefs df block size output:{}\n".format(block_size))

        st_size = int(self.mount_a.stat(filename)["st_size"])
        log.info("stonefs stat used output:{}".format(st_size))
        log.info("stonefs available:{}\n".format(block_size - st_size))
    ...
    def test_df_with_no_args(self):
        expected_output = ''
        df_output = self.get_stonefs_shell_cmd_output('df')
    ...
    def test_df_for_valid_directory(self):
        ...
        log.info("stonefs-shell mount output:\n{}".format(mount_output))
    ...
    def test_df_for_invalid_directory(self):
        ...
        self.negtest_stonefs_shell_cmd(cmd='df ' + dir_abspath,
                                      errmsg='error in stat')

    def test_df_for_valid_file(self):
        s = 'df test' * 14145016
        o = self.get_stonefs_shell_cmd_output("put - dumpfile", stdin=s)
        log.info("stonefs-shell output:\n{}".format(o))
        self.validate_df("dumpfile")


class TestQuota(TestStoneFSShell):
    dir_name = 'testdir'

    def create_dir(self):
        mount_output = self.get_stonefs_shell_cmd_output('mkdir ' + self.dir_name)
        log.info("stonefs-shell mount output:\n{}".format(mount_output))

    def set_and_get_quota_vals(self, input_val, check_status=True):
        self.run_stonefs_shell_cmd(['quota', 'set', '--max_bytes',
                                   input_val[0], '--max_files', input_val[1],
                                   self.dir_name], check_status=check_status)

        quota_output = self.get_stonefs_shell_cmd_output(['quota', 'get', self.dir_name],
                                                        check_status=check_status)
...
class TestXattr(TestStoneFSShell):
    dir_name = 'testdir'
    ...
    def set_get_list_xattr_vals(self, input_val, negtest=False):
        setxattr_output = self.get_stonefs_shell_cmd_output(
            ['setxattr', self.dir_name, input_val[0], input_val[1]])
        log.info("stonefs-shell setxattr output:\n{}".format(setxattr_output))

        getxattr_output = self.get_stonefs_shell_cmd_output(
            ['getxattr', self.dir_name, input_val[0]])
        log.info("stonefs-shell getxattr output:\n{}".format(getxattr_output))

        listxattr_output = self.get_stonefs_shell_cmd_output(
            ['listxattr', self.dir_name])
        log.info("stonefs-shell listxattr output:\n{}".format(listxattr_output))
    ...
    def test_non_existing_dir(self):
        ...
        self.negtest_stonefs_shell_cmd(cmd=['setxattr', self.dir_name, input_val[0],
                                       input_val[1]])
        self.negtest_stonefs_shell_cmd(cmd=['getxattr', self.dir_name, input_val[0]])
        self.negtest_stonefs_shell_cmd(cmd=['listxattr', self.dir_name])

class TestLS(TestStoneFSShell):
    dir_name = ('test_dir')
    hidden_dir_name = ('.test_hidden_dir')

    def test_ls(self):
        """ Test that ls prints files in CWD. """
        self.run_stonefs_shell_cmd(f'mkdir {self.dir_name}')

        ls_output = self.get_stonefs_shell_cmd_output("ls")
    ...
    def test_ls_a(self):
        ...
        self.run_stonefs_shell_cmd(f'mkdir {self.hidden_dir_name}')

        ls_a_output = self.get_stonefs_shell_cmd_output(['ls', '-a'])
    ...
    def test_ls_a_prints_non_hidden_dir(self):
        ...
        self.run_stonefs_shell_cmd(f'mkdir {self.hidden_dir_name} {self.dir_name}')
    ...
    def test_ls_H_prints_human_readable_file_size(self):
        ...
        ls_H_output = self.get_stonefs_shell_cmd_output(['ls', '-lH'])
    ...
    def test_ls_s_sort_by_size(self):
        ...
        self.run_stonefs_shell_cmd(f"write {test_file1}", stdin=file1_content)
        self.run_stonefs_shell_cmd(f"write {test_file2}", stdin=file2_content)

        ls_s_output = self.get_stonefs_shell_cmd_output(['ls', '-lS'])
...
class TestMisc(TestStoneFSShell):
    def test_issue_stonefs_shell_cmd_at_invocation(self):
        """
        Test that `stonefs-shell -c conf cmd` works.
        """
        # choosing a long name since short ones have a higher probability
        # of getting matched by coincidence.
        dirname = 'somedirectory'
        self.run_stonefs_shell_cmd(['mkdir', dirname])

        output = self.mount_a.client_remote.sh(['stonefs-shell', 'ls']).\
            strip()
    ...
    def test_help(self):
        ...
        """
        o = self.get_stonefs_shell_cmd_output("help all")
        log.info("output:\n{}".format(o))

class TestShellOpts(TestStoneFSShell):
    """
    ...
    def setUp(self):
        ...
        # now: '?'
        # editor: '?'
        self.editor_val = self.get_stonefs_shell_cmd_output(
    ...
    def write_tempconf(self, confcontents):
        self.tempconfpath = self.mount_a.client_remote.mktemp(
            suffix='stonefs-shell.conf')
        self.mount_a.client_remote.write_file(self.tempconfpath,
                         confcontents)

    def test_reading_conf(self):
        self.write_tempconf("[stonefs-shell]\neditor =  ???")

        # output of following command -
        # StoneFS:~/>>> set editor
        # editor: 'vim'
        final_editor_val = self.get_stonefs_shell_cmd_output(
