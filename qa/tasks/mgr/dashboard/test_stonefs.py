...
class StonefsTest(DashboardTestCase):
    STONEFS = True

    AUTH_ROLES = ['stonefs-manager']
    ...
    def mk_dirs(self, path, expectedStatus=200):
        self._post("/api/stonefs/{}/tree".format(self.get_fs_id()),
                   params={'path': path})
        self.assertStatus(expectedStatus)

    def rm_dir(self, path, expectedStatus=200):
        self._delete("/api/stonefs/{}/tree".format(self.get_fs_id()),
                     params={'path': path})
        self.assertStatus(expectedStatus)

    def get_root_directory(self, expectedStatus=200):
        data = self._get("/api/stonefs/{}/get_root_directory".format(self.get_fs_id()))
        self.assertStatus(expectedStatus)
    ...
    def _ls_dir(self, path, expectedLength, depth, baseApiPath):
        ...
        if depth is not None:
            params['depth'] = depth
        data = self._get("/{}/stonefs/{}/ls_dir".format(baseApiPath, self.get_fs_id()),
                         params=params)
    ...
    def set_quotas(self, max_bytes=None, max_files=None):
        ...
        self._put("/api/stonefs/{}/quota".format(self.get_fs_id()), data=quotas,
    ...
    def test_access_permissions(self):
        ...
        self._get("/api/stonefs/{}/clients".format(fs_id))
        self.assertStatus(403)
        self._get("/api/stonefs/{}".format(fs_id))
        self.assertStatus(403)
        self._get("/api/stonefs/{}/mds_counters".format(fs_id))
        self.assertStatus(403)
        self._get("/ui-api/stonefs/{}/tabs".format(fs_id))
        self.assertStatus(403)

    def test_stonefs_clients(self):
        fs_id = self.get_fs_id()
        data = self._get("/api/stonefs/{}/clients".format(fs_id))
        self.assertStatus(200)

        self.assertIn('status', data)
        self.assertIn('data', data)

    def test_stonefs_evict_client_does_not_exist(self):
        fs_id = self.get_fs_id()
        self._delete("/api/stonefs/{}/client/1234".format(fs_id))
        self.assertStatus(404)

    def test_stonefs_evict_invalid_client_id(self):
        fs_id = self.get_fs_id()
        self._delete("/api/stonefs/{}/client/xyz".format(fs_id))
        self.assertStatus(400)
        self.assertJsonBody({
            "component": 'stonefs',
            "code": "invalid_stonefs_client_id",
            "detail": "Invalid stonefs client ID xyz"
        })

    def test_stonefs_get(self):
        fs_id = self.get_fs_id()
        data = self._get("/api/stonefs/{}/".format(fs_id))
        self.assertStatus(200)

        self.assertToHave(data, 'stonefs')
        self.assertToHave(data, 'standbys')
        self.assertToHave(data, 'versions')

    def test_stonefs_mds_counters(self):
        fs_id = self.get_fs_id()
        data = self._get("/api/stonefs/{}/mds_counters".format(fs_id))
        self.assertStatus(200)

        self.assertIsInstance(data, dict)
        self.assertIsNotNone(data)

    def test_stonefs_mds_counters_wrong(self):
        self._get("/api/stonefs/baadbaad/mds_counters")
        self.assertStatus(400)
        self.assertJsonBody({
            "component": 'stonefs',
            "code": "invalid_stonefs_id",
            "detail": "Invalid stonefs ID baadbaad"
        })

    def test_stonefs_list(self):
        data = self._get("/api/stonefs/")
        self.assertStatus(200)

        self.assertIsInstance(data, list)
        stonefs = data[0]
        self.assertToHave(stonefs, 'id')
        self.assertToHave(stonefs, 'mdsmap')

    def test_stonefs_get_quotas(self):
        fs_id = self.get_fs_id()
        data = self._get("/api/stonefs/{}/quota?path=/".format(fs_id))
        self.assertStatus(200)
    ...
    def test_stonefs_tabs(self):
        fs_id = self.get_fs_id()
        data = self._get("/ui-api/stonefs/{}/tabs".format(fs_id))
        self.assertStatus(200)
        self.assertIsInstance(data, dict)
    ...
    def test_snapshots(self):
        ...
        self._post("/api/stonefs/{}/snapshot".format(fs_id),
        ...
        self._delete("/api/stonefs/{}/snapshot".format(fs_id),
                     params={'path': '/movies/dune', 'name': 'test'})
        self.assertStatus(200)
        ...
        # Cleanup. Note, the StoneFS Python extension (and therefor the Dashboard
