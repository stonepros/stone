...
#include "stone_objectstore_tool.h"
...
CompatSet get_test_compat_set() {
  ...
  stone_osd_feature_incompat.insert(STONE_OSD_FEATURE_INCOMPAT_SHARDS);
#endif
...
struct lookup_slow_ghobject : public action_on_object_t {
  ...
  void call(ObjectStore *store, coll_t coll, ghobject_t &ghobj, object_info_t &oi) override {
    ...
    stone::signedspan first_seek_time = start1 - start1;
    stone::signedspan last_seek_time = first_seek_time;
    stone::signedspan total_time = first_seek_time;
    {
      auto ch = store->open_collection(coll);
  ...
  void dump(Formatter *f, bool human_readable) const {
    ...
    for (auto i = _objects.begin();
	 ...
	 ++i) {
      ...
      coll_t coll;
      ghobject_t ghobj;
      stone::signedspan first_seek_time;
      stone::signedspan last_seek_time;
      stone::signedspan total_time;
      string tail_key;
	    ...
	    PGLog::IndexedLog &log, pg_missing_t &missing)
{
  try {
    ...
    ostringstream oss;
    stone_assert(struct_ver > 0);
    PGLog::read_log_and_missing(
      fs, ch,
      pgid.make_pgmeta_oid(),
      info, log, missing,
      oss,
      g_stone_context->_conf->osd_ignore_stale_divergent_priors);
...
int finish_remove_pgs(ObjectStore *store)
{
  ...
  for (vector<coll_t>::iterator it = ls.begin();
    ...
    if (it->is_temp(&pgid) ||
      ...
      OSD::recursive_remove_collection(g_stone_context, store, pgid, *it);
      continue;
    }
	     ...
	     pg_missing_t &missing)
{
  ...
  if (!divergent.empty()) {
    stone_assert(missing.get_items().empty());
		   ...
		   PastIntervals &past_intervals)
{
  ...
  uint64_t max_entries = g_stone_context->_conf->osd_max_pg_log_entries;
  ...
  stone_assert(info.last_update.version > max_entries);
  version_t trim_to = info.last_update.version - max_entries;
  size_t trim_at_once = g_stone_context->_conf->osd_pg_log_trim_max;
  eversion_t new_tail;
  bool done = false;
    ...
    PastIntervals &past_intervals)
{
  ...
  if (pgid.is_no_shard()) {
    pgb.superblock.compat_features.incompat.remove(STONE_OSD_FEATURE_INCOMPAT_SHARDS);
  }
			       ...
			       bool force, std::string pgidstr)
{
  ...
  SnapMapper mapper(g_stone_context, &driver, 0, 0, 0, pgid.shard);
  ...
  if (!dry_run) {
    ...
    pg_log_t::filter_log(pgid, ms.osdmap, g_stone_context->_conf->osd_hit_set_namespace,
    ...
    filter_divergent_priors(pgid, ms.osdmap, g_stone_context->_conf->osd_hit_set_namespace,
	...
	if (obj.nspace == g_stone_context->_conf->osd_hit_set_namespace)
	  return false;
	stone_assert(!obj.is_temp());
		     ...
		     ghobject_t &ghobj, bool all, bool force, enum rmtype type)
{
  ...
  OSDriver driver(
    ...
    coll_t(),
    OSD::make_snapmapper_oid());
  SnapMapper mapper(g_stone_context, &driver, 0, 0, 0, pg.shard);
  struct stat st;
...
int dup(string srcpath, ObjectStore *src, string dstpath, ObjectStore *dst)
{
  ...
  {
    for (auto k : {"magic", "whoami", "stone_fsid", "fsid"}) {
      string val;
      src->read_meta(k, &val);
...
void usage(po::options_description &desc)
{
    ...
    cerr << std::endl;
    cerr << "stone-objectstore-tool ... <object> (get|set)-bytes [file]" << std::endl;
    cerr << "stone-objectstore-tool ... <object> set-(attr|omap) <key> [file]" << std::endl;
    cerr << "stone-objectstore-tool ... <object> (get|rm)-(attr|omap) <key>" << std::endl;
    cerr << "stone-objectstore-tool ... <object> get-omaphdr" << std::endl;
    cerr << "stone-objectstore-tool ... <object> set-omaphdr [file]" << std::endl;
    cerr << "stone-objectstore-tool ... <object> list-attrs" << std::endl;
    cerr << "stone-objectstore-tool ... <object> list-omap" << std::endl;
    cerr << "stone-objectstore-tool ... <object> remove|removeall" << std::endl;
    cerr << "stone-objectstore-tool ... <object> dump" << std::endl;
    cerr << "stone-objectstore-tool ... <object> set-size" << std::endl;
    cerr << "stone-objectstore-tool ... <object> clear-data-digest" << std::endl;
    cerr << "stone-objectstore-tool ... <object> remove-clone-metadata <cloneid>" << std::endl;
    cerr << std::endl;
...
int mydump_journal(Formatter *f, string journalpath, bool m_journal_dio)
{
  ...
  FileJournal *journal = new FileJournal(g_stone_context, uuid_d(), NULL, NULL,
...
int main(int argc, char **argv)
{
  ...
  if (fd >= 0) {
    ...
    tmp.push_back(dpath);
    tmp.insert(tmp.end(), stone_option_strings.begin(),
	       stone_option_strings.end());
    tmp.swap(stone_option_strings);
  }

  vector<const char *> stone_options;
  stone_options.reserve(stone_options.size() + stone_option_strings.size());
  for (vector<string>::iterator i = stone_option_strings.begin();
       i != stone_option_strings.end();
       ++i) {
    stone_options.push_back(i->c_str());
  }
  ...
  ObjectStore *fs = ObjectStore::create(g_stone_context, type, dpath, jpath, flags);
  if (fs == NULL) {
  ...
  if (op == "apply-layout-settings") {
    ...
    stone_assert(superblock != nullptr);
...
// The following code for export, info, log require omap or !skip-mount-omap
  if (it != ls.end()) {
    ...
    if (struct_ver < PG::get_compat_struct_v()) {
      cerr << "PG is too old to upgrade, use older Stone version" << std::endl;
      ret = -EFAULT;
      goto out;
    }
    if (debug)
	...
	fs->queue_transaction(ch, std::move(*t));
      ...
      stone_assert(!"Should have already checked for valid --op");
    }
  } else {
    cerr << "PG '" << pgid << "' not found" << std::endl;
    ret = -ENOENT;
