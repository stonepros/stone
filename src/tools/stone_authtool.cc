...
void usage()
{
  cout << "usage: stone-authtool keyringfile [OPTIONS]...\n"
...
int main(int argc, const char **argv)
{
  ...
  /* Handle options unique to stone-authtool
  ...
  for (i = args.begin(); i != args.end(); ) {
    ...
    if (stone_argparse_double_dash(args, i)) {
      break;
    } else if (stone_argparse_flag(args, i, "-g", "--gen-key", (char*)NULL)) {
      gen_key = true;
    } else if (stone_argparse_flag(args, i, "--gen-print-key", (char*)NULL)) {
      gen_print_key = true;
    } else if (stone_argparse_witharg(args, i, &val, "-a", "--add-key", (char*)NULL)) {
      if (val.empty()) {
        cerr << "Option --add-key requires an argument" << std::endl;
        exit(1);
      }
      add_key = val;
    } else if (stone_argparse_flag(args, i, "-l", "--list", (char*)NULL)) {
      list = true;
    } else if (stone_argparse_witharg(args, i, &val, "--caps", (char*)NULL)) {
      caps_fn = val;
    } else if (stone_argparse_witharg(args, i, &val, "--cap", (char*)NULL)) {
    ...
    } else if (stone_argparse_flag(args, i, "-p", "--print-key", (char*)NULL)) {
      print_key = true;
    } else if (stone_argparse_flag(args, i, "-C", "--create-keyring", (char*)NULL)) {
      create_keyring = true;
    } else if (stone_argparse_witharg(args, i, &val, "--import-keyring", (char*)NULL)) {
      import_keyring = val;
  ...
  if (gen_print_key) {
    CryptoKey key;
    key.create(g_stone_context, STONE_CRYPTO_AES);
    cout << key << std::endl;
    return 0;
  }
  ...
  if (!import_keyring.empty()) {
	...
	exit(1);
      ...
      //other.print(cout);
      keyring.import(g_stone_context, other);
      modified = true;
    } else {
