...
static void usage(std::ostream& out)
{
  ...
  out << R"(Stone configuration query tool
...
EXAMPLES
$ stone-conf --name mon.0 -c /etc/stone/stone.conf 'mon addr'
		  ...
		  const std::string &key, bool resolve_search)
{
  ...
  if (ret == -ENOENT)
    ...
    if (resolve_search) {
      string result;
      ret = stone_resolve_file_search(val, result);
      if (!ret)
	puts(result.c_str());
    }
    else {
...
static int dump_all(const string& format)
{
  if (format == "" || format == "plain") {
    ...
    if (f) {
      f->open_object_section("stone-conf");
...
static void maybe_override_pid(vector<const char*>& args)
{
  for (auto i = args.begin(); i != args.end(); ++i) {
    string val;
    if (stone_argparse_witharg(args, i, &val, "--pid", (char*)NULL)) {
...
int main(int argc, const char **argv)
{
  ...
  auto cct = [&args] {
    ...
    return global_init(&defaults, args, STONE_ENTITY_TYPE_CLIENT,
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ) {
    if (stone_argparse_double_dash(args, i)) {
      break;
    } else if (stone_argparse_witharg(args, i, &val, "-s", "--section", (char*)NULL)) {
      sections.push_back(val);
    } else if (stone_argparse_flag(args, i, "-r", "--resolve_search", (char*)NULL)) {
      resolve_search = true;
    } else if (stone_argparse_flag(args, i, "-h", "--help", (char*)NULL)) {
      action = "help";
    } else if (stone_argparse_witharg(args, i, &val, "--lookup", (char*)NULL)) {
      action = "lookup";
      lookup_key = val;
    } else if (stone_argparse_flag(args, i, "-L", "--list_all_sections", (char*)NULL)) {
      action = "list-sections";
      section_list_prefix = "";
    } else if (stone_argparse_witharg(args, i, &val, "-l", "--list_sections", (char*)NULL)) {
      action = "list-sections";
      section_list_prefix = val;
    } else if (stone_argparse_witharg(args, i, &val, "--filter_key", (char*)NULL)) {
      filter_key.push_back(val);
    } else if (stone_argparse_witharg(args, i, &val, "--filter_key_value", (char*)NULL)) {
    ...
    } else if (stone_argparse_flag(args, i, "-D", "--dump_all", (char*)NULL)) {
      action = "dumpall";
