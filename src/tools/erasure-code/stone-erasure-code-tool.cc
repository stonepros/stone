...
void usage(const std::string message, ostream &out) {
  ...
  out << "usage: stone-erasure-code-tool test-plugin-exists <plugin>" << std::endl;
  out << "       stone-erasure-code-tool validate-profile <profile> [<display-param> ...]" << std::endl;
  out << "       stone-erasure-code-tool calc-chunk-size <profile> <object_size>" << std::endl;
  out << "       stone-erasure-code-tool encode <profile> <stripe_unit> <want_to_encode> <fname>" << std::endl;
  out << "       stone-erasure-code-tool decode <profile> <stripe_unit> <want_to_decode> <fname>" << std::endl;
  out << "" << std::endl;
...
int do_validate_profile(const std::vector<const char*> &args) {
  ...
  for (auto &param : display_params) {
    ...
    if (param == "chunk_count") {
      ...
      stone_abort_msgf("unknown display_param: %s", param.c_str());
    }
  }
...
int do_encode(const std::vector<const char*> &args) {
  ...
  std::map<int, stone::bufferlist> encoded_data;
