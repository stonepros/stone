...
#undef dout_prefix
#define dout_prefix *_dout << "stonefs::mirror::FSMirror " << __func__
...
public:
  MirrorAdminSocketHook(StoneContext *cct, const Filesystem &filesystem, FSMirror *fs_mirror)
...
FSMirror::FSMirror(StoneContext *cct, const Filesystem &filesystem, uint64_t pool_id,
...
void FSMirror::cleanup() {
  dout(20) << dendl;
  stone_unmount(m_mount);
  stone_release(m_mount);
  m_ioctx.close();
  m_cluster.reset();
}
...
void FSMirror::reopen_logs() {
  ...
  if (m_cluster) {
    reinterpret_cast<StoneContext *>(m_cluster->cct())->reopen_logs();
  }
...
void FSMirror::init(Context *on_finish) {
  ...
  std::scoped_lock locker(m_lock);
  int r = connect(g_stone_context->_conf->name.to_str(),
                  g_stone_context->_conf->cluster, &m_cluster, "", "", m_args);
  if (r < 0) {
    m_init_failed = true;
...
void FSMirror::add_peer(const Peer &peer) {
  ...
  stone_assert(m_peer_replayers.size() == 1); // support only a single peer
}
