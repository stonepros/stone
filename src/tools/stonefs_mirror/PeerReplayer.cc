...
#define dout_subsys stone_subsys_stonefs_mirror
#undef dout_prefix
#define dout_prefix *_dout << "stonefs::mirror::PeerReplayer("   \
...
const std::string PEER_CONFIG_KEY_PREFIX = "stonefs/mirror/peer";

std::string snapshot_dir_path(StoneContext *cct, const std::string &path) {
...
std::string snapshot_path(StoneContext *cct, const std::string &path, const std::string &snap_name) {
...
int opendirat(MountRef mnt, int dirfd, const std::string &relpath, int flags,
  ...
  int r = stone_openat(mnt, dirfd, relpath.c_str(), flags, 0);
  if (r < 0) {
    return r;
  }

  int fd = r;
  r = stone_fdopendir(mnt, fd, dirp);
  stone_close(mnt, fd);
  return r;
}
...
public:
  PeerReplayerAdminSocketHook(StoneContext *cct, const Filesystem &filesystem,
...
PeerReplayer::PeerReplayer(StoneContext *cct, FSMirror *fs_mirror,
  ...
  : m_cct(cct),
    ...
    m_lock(stone::make_mutex("stonefs::mirror::PeerReplayer::" + stringify(peer.uuid))) {
...
int PeerReplayer::init() {
  ...
  if (!r) {
    ...
    try {
      ...
      stonex_key = root_obj.at("key").get_str();
  ...
  r = connect(remote_client, remote_cluster, &m_remote_cluster, mon_host, stonex_key);
  if (r < 0) {
  ...
  auto nr_replayers = g_stone_context->_conf.get_val<uint64_t>(
    "stonefs_mirror_max_concurrent_directory_syncs");
...
void PeerReplayer::shutdown() {
  ...
  m_replayers.clear();
  stone_unmount(m_remote_mount);
  stone_release(m_remote_mount);
...
boost::optional<std::string> PeerReplayer::pick_directory() {
  ...
  auto now = clock::now();
  auto retry_timo = g_stone_context->_conf.get_val<uint64_t>(
    "stonefs_mirror_retry_failed_directories_interval");
...
int PeerReplayer::register_directory(const std::string &dir_root,
  ...
  dout(20) << ": dir_root=" << dir_root << dendl;
  stone_assert(m_registered.find(dir_root) == m_registered.end());
...
void PeerReplayer::unregister_directory(const std::string &dir_root) {
  ...
  stone_assert(it != m_registered.end());
...
int PeerReplayer::try_lock_directory(const std::string &dir_root,
  ...
  int r = stone_open(m_remote_mount, dir_root.c_str(), O_RDONLY | O_DIRECTORY, 0);
  if (r < 0 && r != -ENOENT) {
  ...
  if (r == -ENOENT) {
    ...
    r = stone_mkdirs(m_remote_mount, dir_root.c_str(), 0755);
    if (r < 0) {
    ...
    r = stone_open(m_remote_mount, dir_root.c_str(), O_RDONLY | O_DIRECTORY, 0);
    if (r < 0) {
      derr << ": failed to open remote dir_root=" << dir_root << ": " << cpp_strerror(r)
           << dendl;
      return r;
  ...
  int fd = r;
  r = stone_flock(m_remote_mount, fd, LOCK_EX | LOCK_NB, (uint64_t)replayer->get_thread_id());
  if (r != 0) {
    if (r == -EWOULDBLOCK) {
      dout(5) << ": dir_root=" << dir_root << " is locked by stonefs-mirror, "
    ...
    if (stone_close(m_remote_mount, fd) < 0) {
...
void PeerReplayer::unlock_directory(const std::string &dir_root, const DirRegistry &registry) {
  ...
  int r = stone_flock(m_remote_mount, registry.fd, LOCK_UN,
  ...
  r = stone_close(m_remote_mount, registry.fd);
  if (r < 0) {
...
int PeerReplayer::build_snap_map(const std::string &dir_root,
  ...
  stone_dir_result *dirp = nullptr;
  int r = stone_opendir(mnt, snap_dir.c_str(), &dirp);
  if (r < 0) {
  ...
  auto entry = stone_readdir(mnt, dirp);
  while (entry != NULL) {
  ...
  for (auto &snap : snaps) {
    ...
    r = stone_get_snap_info(mnt, snap_path.c_str(), &info);
    if (r < 0) {
...
int PeerReplayer::propagate_snap_deletes(const std::string &dir_root,
  ...
  for (auto &snap : snaps) {
    dout(20) << ": deleting dir_root=" << dir_root << ", snapshot=" << snap
             << dendl;
    int r = stone_rmsnap(m_remote_mount, dir_root.c_str(), snap.c_str());
    if (r < 0) {
...
int PeerReplayer::propagate_snap_renames(
  ...
  for (auto &snapp : snaps) {
    ...
    dout(20) << ": renaming dir_root=" << dir_root << ", snapshot from="
             << from << ", to=" << to << dendl;
    int r = stone_rename(m_remote_mount, from.c_str(), to.c_str());
    if (r < 0) {
...
int PeerReplayer::remote_mkdir(const std::string &epath, const struct stone_statx &stx,
                               const FHandles &fh) {
  dout(10) << ": remote epath=" << epath << dendl;

  int r = stone_mkdirat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(), stx.stx_mode & ~S_IFDIR);
  if (r < 0 && r != -EEXIST) {
  ...
  r = stone_chownat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(), stx.stx_uid, stx.stx_gid,
                   AT_SYMLINK_NOFOLLOW);
  if (r < 0) {
  ...
  r = stone_chmodat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(), stx.stx_mode & ~S_IFMT,
                   AT_SYMLINK_NOFOLLOW);
  if (r < 0) {
  ...
  r = stone_utimensat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(), times, AT_SYMLINK_NOFOLLOW);
  if (r < 0) {
...
int PeerReplayer::copy_to_remote(const std::string &dir_root,  const std::string &epath,
                                 const struct stone_statx &stx, const FHandles &fh) {
  ...
  int r = stone_openat(m_local_mount, fh.c_fd, epath.c_str(), O_RDONLY | O_NOFOLLOW, 0);
  if (r < 0) {
  ...
  l_fd = r;
  r = stone_openat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(),
  ...
  while (true) {
    ...
    r = stone_preadv(m_local_mount, l_fd, iov, NR_IOVECS, -1);
    if (r < 0) {
    ...
    r = stone_pwritev(m_remote_mount, r_fd, iov, iovs, -1);
    if (r < 0) {
  ...
  if (r == 0) {
    r = stone_fsync(m_remote_mount, r_fd, 0);
    if (r < 0) {
...
close_remote_fd:
  if (stone_close(m_remote_mount, r_fd) < 0) {
...
close_local_fd:
  if (stone_close(m_local_mount, l_fd) < 0) {
...
int PeerReplayer::remote_file_op(const std::string &dir_root, const std::string &epath,
  ...
  if (need_data_sync) {
    if (S_ISREG(stx.stx_mode)) {
      ...
      r = stone_unlinkat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(), 0);
      if (r < 0 && r != -ENOENT) {
        derr << ": failed to remove remote symlink=" << epath << dendl;
        return r;
      }
      char *target = (char *)alloca(stx.stx_size+1);
      r = stone_readlinkat(m_local_mount, fh.c_fd, epath.c_str(), target, stx.stx_size);
      if (r < 0) {
      ...
      r = stone_symlinkat(m_remote_mount, target, fh.r_fd_dir_root, epath.c_str());
      if (r < 0 && r != EEXIST) {
...
int PeerReplayer::cleanup_remote_dir(const std::string &dir_root,
  ...
  struct stone_statx tstx;
  int r = stone_statxat(m_remote_mount, fh.r_fd_dir_root, epath.c_str(), &tstx,
                       STONE_STATX_MODE | STONE_STATX_UID | STONE_STATX_GID |
                       STONE_STATX_SIZE | STONE_STATX_ATIME | STONE_STATX_MTIME,
  ...
  while (!rm_stack.empty()) {
    ...
    if (entry.is_directory()) {
      ...
      struct dirent de;
      while (true) {
        r = stone_readdirplus_r(m_remote_mount, entry.dirp, &de, &stx,
                               STONE_STATX_MODE, AT_NO_ATTR_SYNC | AT_SYMLINK_NOFOLLOW, NULL);
        if (r < 0) {
      ...
      if (r == 0) {
        r = stone_unlinkat(m_remote_mount, fh.r_fd_dir_root, entry.epath.c_str(), AT_REMOVEDIR);
        if (r < 0) {
        ...
        if (stone_closedir(m_remote_mount, entry.dirp) < 0) {
      ...
      r = stone_unlinkat(m_remote_mount, fh.r_fd_dir_root, entry.epath.c_str(), 0);
      if (r < 0) {
        derr << ": failed to remove remote directory=" << entry.epath << ": "
             << cpp_strerror(r) << dendl;
        break;
...
int PeerReplayer::should_sync_entry(const std::string &epath, const struct stone_statx &cstx,
  ...
  *need_attr_sync = false;
  struct stone_statx pstx;
  int r = stone_statxat(fh.p_mnt, fh.p_fd, epath.c_str(), &pstx,
                       STONE_STATX_MODE | STONE_STATX_UID | STONE_STATX_GID |
                       STONE_STATX_SIZE | STONE_STATX_CTIME | STONE_STATX_MTIME,
                       AT_NO_ATTR_SYNC | AT_SYMLINK_NOFOLLOW);
...
int PeerReplayer::propagate_deleted_entries(const std::string &dir_root,
  ...
  while (true) {
    ...
    int len = stone_getdents(fh.p_mnt, dirp, (char *)dire, 512);
    if (len < 0) {
    ...
    for (int i = 0; i < nr; ++i) {
      ...
      r = stone_statxat(fh.p_mnt, fh.p_fd, dpath.c_str(), &pstx,
                       STONE_STATX_MODE, AT_NO_ATTR_SYNC | AT_SYMLINK_NOFOLLOW);
      if (r < 0) {
      ...
      struct stone_statx cstx;
      r = stone_statxat(m_local_mount, fh.c_fd, dpath.c_str(), &cstx,
                       STONE_STATX_MODE, AT_NO_ATTR_SYNC | AT_SYMLINK_NOFOLLOW);
      if (r < 0 && r != -ENOENT) {
      ...
      if (purge_remote) {
        ...
        if (S_ISDIR(pstx.stx_mode)) {
          ...
          r = stone_unlinkat(m_remote_mount, fh.r_fd_dir_root, dpath.c_str(), 0);
        }
...
int PeerReplayer::open_dir(MountRef mnt, const std::string &dir_path,
  ...
  int fd = stone_open(mnt, dir_path.c_str(), O_DIRECTORY | O_RDONLY, 0);
  if (fd < 0) {
  ...
  snap_info info;
  int r = stone_get_snap_info(mnt, dir_path.c_str(), &info);
  if (r < 0) {
...
int PeerReplayer::pre_sync_check_and_open_handles(
  ...
  if (fd < 0) {
    if (!prev || fd != -ENOENT) {
      stone_close(m_local_mount, fh->c_fd);
      return fd;
    }
...
int PeerReplayer::do_synchronize(const std::string &dir_root, const Snapshot &current,
  ...
  r = stone_fsetxattr(m_remote_mount, fh.r_fd_dir_root, "stone.mirror.dirty_snap_id",
                     snap_id_str.c_str(), snap_id_str.size(), 0);
  if (r < 0) {
    derr << ": error setting \"stone.mirror.dirty_snap_id\" on dir_root=" << dir_root
         << ": " << cpp_strerror(r) << dendl;
    return r;
  }

  struct stone_statx tstx;
  r = stone_fstatx(m_local_mount, fh.c_fd, &tstx,
                  STONE_STATX_MODE | STONE_STATX_UID | STONE_STATX_GID |
                  STONE_STATX_SIZE | STONE_STATX_ATIME | STONE_STATX_MTIME,
                  AT_NO_ATTR_SYNC | AT_SYMLINK_NOFOLLOW);
  if (r < 0) {
  ...
  stone_dir_result *tdirp;
  r = stone_fdopendir(m_local_mount, fh.c_fd, &tdirp);
  if (r < 0) {
  ...
  while (!sync_stack.empty()) {
    ...
    if (entry.is_directory()) {
      ...
      struct dirent de;
      while (true) {
        r = stone_readdirplus_r(m_local_mount, entry.dirp, &de, &stx,
                               STONE_STATX_MODE | STONE_STATX_UID | STONE_STATX_GID |
                               STONE_STATX_SIZE | STONE_STATX_ATIME | STONE_STATX_MTIME,
      ...
      if (r == 0) {
        ...
        if (stone_closedir(m_local_mount, entry.dirp) < 0) {
...
int PeerReplayer::synchronize(const std::string &dir_root, const Snapshot &current,
  ...
  int r = stone_getxattr(m_remote_mount, dir_root.c_str(), "stone.mirror.dirty_snap_id", nullptr, 0);
  if (r < 0 && r != -ENODATA) {
  ...
  if (r < 0) {
    dout(5) << ": missing \"stone.mirror.dirty_snap_id\" xattr on remote -- using"
            << " incremental sync with remote scan" << dendl;
    r = do_synchronize(dir_root, current, boost::none);
  } else {
    size_t xlen = r;
    char *val = (char *)alloca(xlen+1);
    r = stone_getxattr(m_remote_mount, dir_root.c_str(), "stone.mirror.dirty_snap_id", (void*)val, xlen);
    if (r < 0) {
  ...
  r = stone_mksnap(m_remote_mount, dir_root.c_str(), current.first.c_str(), 0755,
...
int PeerReplayer::do_sync_snaps(const std::string &dir_root) {
  ...
  auto snaps_per_cycle = g_stone_context->_conf.get_val<uint64_t>(
    "stonefs_mirror_max_snapshot_sync_per_cycle");
...
void PeerReplayer::run(SnapshotReplayerThread *replayer) {
  ...
  auto scan_interval = g_stone_context->_conf.get_val<uint64_t>(
    "stonefs_mirror_directory_scan_interval");
...
void PeerReplayer::reopen_logs() {
  ...
  if (m_remote_cluster) {
    reinterpret_cast<StoneContext *>(m_remote_cluster->cct())->reopen_logs();
  }
}
