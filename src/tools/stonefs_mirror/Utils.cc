...
#undef dout_prefix
#define dout_prefix *_dout << "stonefs::mirror::Utils " << __func__
...
int connect(std::string_view client_name, std::string_view cluster_name,
  ...
  if (r < 0 && r != -ENOENT) {
    derr << ": could not read stone conf: " << ": " << cpp_strerror(r) << dendl;
    return r;
  }
  ...
  if (!stonex_key.empty()) {
    r = cct->_conf.set_val("key", std::string(stonex_key));
    if (r < 0) {
...
int mount(RadosRef cluster, const Filesystem &filesystem, bool cross_check_fscid,
  ...
  stone_mount_info *cmi;
  int r = stone_create_with_context(&cmi, reinterpret_cast<StoneContext*>(cluster->cct()));
  if (r < 0) {
    derr << ": mount error: " << cpp_strerror(r) << dendl;
    return r;
  }

  r = stone_conf_set(cmi, "client_mount_uid", "0");
  if (r < 0) {
    derr << ": mount error: " << cpp_strerror(r) << dendl;
    return r;
  }

  r = stone_conf_set(cmi, "client_mount_gid", "0");
  if (r < 0) {
    derr << ": mount error: " << cpp_strerror(r) << dendl;
    return r;
  }
  ...
  auto mount_timeout = g_stone_context->_conf.get_val<std::chrono::seconds>
    ("stonefs_mirror_mount_timeout").count();
  r = stone_set_mount_timeout(cmi, mount_timeout);
  if (r < 0) {
    derr << ": mount error: " << cpp_strerror(r) << dendl;
    return r;
  }
  ...
  r = stone_select_filesystem(cmi, filesystem.fs_name.c_str());
  if (r < 0) {
    derr << ": mount error: " << cpp_strerror(r) << dendl;
    return r;
  }

  r = stone_mount(cmi, NULL);
  if (r < 0) {
    derr << ": mount error: " << cpp_strerror(r) << dendl;
    return r;
  }

  auto fs_id = stone_get_fs_cid(cmi);
