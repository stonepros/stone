...
#undef dout_prefix
#define dout_prefix *_dout << "stonefs::mirror::ClusterWatcher " << __func__

namespace stonefs {
namespace mirror {

ClusterWatcher::ClusterWatcher(StoneContext *cct, MonClient *monc, ServiceDaemon *service_daemon,
                               Listener &listener)
  : Dispatcher(cct),
    m_monc(monc),
...
bool ClusterWatcher::ms_can_fast_dispatch2(const cref_t<Message> &m) const {
  return m->get_type() == STONE_MSG_FS_MAP;
}
...
bool ClusterWatcher::ms_dispatch2(const ref_t<Message> &m) {
  if (m->get_type() == STONE_MSG_FS_MAP) {
