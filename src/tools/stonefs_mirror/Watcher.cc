...
#undef dout_prefix
#define dout_prefix *_dout << "stonefs::mirror::Watcher " << __func__

using stonefs::mirror::watcher::RewatchRequest;
...
Watcher::Watcher(librados::IoCtx &ioctx, std::string_view oid, ContextWQ *work_queue)
  : m_oid(oid),
    ...
    m_lock(stone::make_shared_mutex("stonefs::mirror::snap_watcher")),
    m_state(STATE_IDLE),
    m_watch_ctx(*this) {
}
...
void Watcher::handle_register_watch(int r, Context *on_finish) {
  ...
  {
    std::scoped_lock locker(m_lock);
    stone_assert(m_state == STATE_REGISTERING);
...
void Watcher::rewatch() {
  ...
  Context *unregister_watch_ctx = nullptr;
  {
    ...
    stone_assert(m_state == STATE_REWATCHING);
