...
#undef dout_prefix
#define dout_prefix *_dout << "stonefs::mirror::Mirror " << __func__
...
public:
  stone::mutex timer_lock = stone::make_mutex("stonefs::mirror::timer_lock");
...
public:
  ...
  explicit ThreadPoolSingleton(StoneContext *cct)
    ...
    work_queue = new ContextWQ("Mirror::work_queue", stone::make_timespan(60), this);
...
int Mirror::init_mon_client() {
  ...
  m_monc->set_want_keys(STONE_ENTITY_TYPE_MON);
...
void Mirror::handle_enable_mirroring(const Filesystem &filesystem,
  ...
  stone_assert(mirror_action.action_in_progress);
...
void Mirror::enable_mirroring(const Filesystem &filesystem, uint64_t local_pool_id,
  ...
  stone_assert(!mirror_action.fs_mirror);
...
void Mirror::update_fs_mirrors() {
  ...
  double blocklist_interval = g_stone_context->_conf.get_val<std::chrono::seconds>
    ("stonefs_mirror_restart_mirror_on_blocklist_interval").count();
  bool check_blocklist = blocklist_interval > 0 && ((now - m_last_blocklist_check) >= blocklist_interval);

  double failed_interval = g_stone_context->_conf.get_val<std::chrono::seconds>
    ("stonefs_mirror_restart_mirror_on_failure_interval").count();
...
void Mirror::schedule_mirror_update_task() {
  ...
  double after = g_stone_context->_conf.get_val<std::chrono::seconds>
    ("stonefs_mirror_action_update_interval").count();
