...
#undef dout_prefix
#define dout_prefix *_dout << "stonefs::mirror::ServiceDaemon: " << this << " " \
                           << __func__
...
struct AttributeDumpVisitor : public boost::static_visitor<void> {
  ...
  AttributeDumpVisitor(stone::Formatter *f, std::string_view name)
    : f(f), name(name) {
  }
...
ServiceDaemon::ServiceDaemon(StoneContext *cct, RadosRef rados)
  : m_cct(cct),
    m_rados(rados),
...
int ServiceDaemon::init() {
  ...
  if (id.find(STONEFS_MIRROR_AUTH_ID_PREFIX) == 0) {
    id = id.substr(STONEFS_MIRROR_AUTH_ID_PREFIX.size());
  }
  std::string instance_id = stringify(m_rados->get_instance_id());

  std::map<std::string, std::string> service_metadata = {{"id", id},
                                                         {"instance_id", instance_id}};
  int r = m_rados->service_daemon_register("stonefs-mirror", instance_id,
                                           service_metadata);
  if (r < 0) {
    return r;
  }
