...
/**
 * usage: stone-monstore-tool <store-path> <command> [options]
 *
 * commands:
 *
 *  store-copy < --out arg >
 ...
 * wanted syntax:
 *
 * stone-monstore-tool PATH CMD [options]
 *
 * stone-monstore-tool PATH store-copy <PATH2 | -o PATH2>
 * stone-monstore-tool PATH dump-keys
 * stone-monstore-tool PATH compact
 * stone-monstore-tool PATH get monmap [VER]
 * stone-monstore-tool PATH get osdmap [VER]
 * stone-monstore-tool PATH dump-paxos STARTVER ENDVER
 *
 *
 */
void usage(const char *n, po::options_description &d)
{
  ...
  std::cerr
    << "\nPlease Note:\n"
    << "* Stone-specific options should be in the format --option-name=VAL\n"
...
int update_osdmap(MonitorDBStore& store, version_t ver, bool copy,
  ...
  osdmap.encode(bl, STONE_FEATURES_ALL|STONE_FEATURE_RESERVED);
  ...
  if (copy) {
    ...
    if (inc.crush.length()) {
      inc.crush.clear();
      crush->encode(inc.crush, STONE_FEATURES_SUPPORTED_DEFAULT);
    }
  ...
  stone_assert(osdmap.have_crc());
  inc.full_crc = osdmap.get_crc();
  bl.clear();
  // be consistent with OSDMonitor::update_from_paxos()
  inc.encode(bl, STONE_FEATURES_ALL|STONE_FEATURE_RESERVED);
...
int rewrite_transaction(MonitorDBStore& store, int version,
  ...
  stone_assert(good_version < last_committed);
...
static int update_auth(MonitorDBStore& st, const string& keyring_path)
{
  ...
  KeyRing keyring;
  int r = keyring.load(g_stone_context, keyring_path);
  if (r < 0) {
  ...
  {
    KeyServer ks(g_stone_context, nullptr);
    KeyServerData::Incremental auth_inc;
		       ...
		       const vector<string>& mon_ids)
{
  ...
  monmap.print(cout);
  bufferlist bl;
  monmap.encode(bl, STONE_FEATURES_ALL);
  monmap.set_epoch(0);
  auto t = make_shared<MonitorDBStore::Transaction>();
...
static int update_monitor(MonitorDBStore& st)
{
  ...
  bufferlist bl;
  bl.append(STONE_MON_ONDISK_MAGIC "\n");
  auto t = make_shared<MonitorDBStore::Transaction>();
...
static int update_creating_pgs(MonitorDBStore& st)
{
  ...
  bufferlist newbl;
  encode(creating, newbl, STONE_FEATURES_ALL);
...
static int update_mgrmap(MonitorDBStore& st)
{
...

  {
    ...
    auto initial_modules =
      get_str_vec(g_stone_context->_conf.get_val<string>("mgr_initial_modules"));
    copy(begin(initial_modules),
	 end(initial_modules),
	 inserter(map.modules, end(map.modules)));
    bufferlist bl;
    map.encode(bl, STONE_FEATURES_ALL);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    // being collected (thus being part of stone_option_strings and eventually
    // passed on to global_init() below).
    // Instead we specify po::exclude_positional, which has the upside of
    // completely avoid this, but the downside of having to specify stone
  ...
  stone_options.reserve(stone_option_strings.size());
  ...
  auto cct = global_init(
    NULL, stone_options, STONE_ENTITY_TYPE_MON,
