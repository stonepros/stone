...
{
  out << "usage: stone-dencoder [commands ...]" << std::endl;
  out << "\n";
...
int main(int argc, const char **argv)
{
  ...
  Dencoder *den = NULL;
  uint64_t features = STONE_FEATURES_SUPPORTED_DEFAULT;
  bufferlist encbl;
  uint64_t skip = 0;
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ++i) {
    ...
    if (*i == string("help") || *i == string("-h") || *i == string("--help")) {
      ...
      cout << STONE_GIT_NICE_VER << std::endl;
	...
	exit(1);
      ...
      cout << STONE_FEATURES_SUPPORTED_DEFAULT << std::endl;
      exit(0);
	...
	exit(1);
      ...
      den->encode(encbl, features | STONE_FEATURE_RESERVED); // hack for OSDMap
