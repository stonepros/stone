...
int Dumper::init(mds_role_t role_, const std::string &type)
{
  ...
  if (type == "mdlog") {
    ...
    stone_abort(); // should not get here 
  }
  return 0;
}
...
int Dumper::dump(const char *dump_file)
{
  ...
  Journaler journaler("dumper", ino, fs->mds_map.get_metadata_pool(),
                      STONE_FS_ONDISK_MAGIC, objecter, 0, 0,
                      &finisher);
  ...
  if (fd >= 0) {
    ...
    snprintf(buf, HEADER_LEN, "Stone mds%d journal dump\n start offset %llu (0x%llx)\n\
    ...
    for (uint64_t pos = start; pos < start + len; pos += chunk_size) {
      ...
      C_SaferCond cond;
      lock.lock();
      filer.read(ino, &journaler.get_layout(), STONE_NOSNAP,
...
int Dumper::undump(const char *dump_file, bool force)
{
   ...
   * prezeroing behaviour */
  {
    ...
    filer.purge_range(ino, &h.layout, snapc, last_obj, purge_count,
		      stone::real_clock::now(), 0, &purge_cond);
    lock.unlock();
    purge_cond.wait();
  }
  ...
  if (!len) {
    ...
    lock.lock();
    filer.zero(ino, &h.layout, snapc, h.write_pos, len, stone::real_clock::now(), 0, &zero_cond);
    lock.unlock();
    zero_cond.wait();
  }
  ...
  while (left > 0) {
    ...
    filer.write(ino, &h.layout, snapc, pos, l, j,
		stone::real_clock::now(), 0, &write_cond);
    lock.unlock();
