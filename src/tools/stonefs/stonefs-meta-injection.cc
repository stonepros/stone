...
using std::string;
using namespace std;
static string version = "stonefs-meta-injection v1.1";
...
int main(int argc, const char **argv)
{
  ...
  general.add_options()
      ...
      ("rank,r", po::value<string>(&rank_str), "the rank of stonefs, default(0) (e.g. -r stonefs_a:0)")
      ("minfo", po::value<string>(&minfo), "specify metapool, datapools and rank (e.g. stonefs_metadata_a:stonefs_data_a:0)")
  ...
  if (vm.count("help")) {
    ...
    std::cout << "usage : \n"
              << "  stonefs-meta-injection <listc|showm|showfn|amend|amendfn> -r <fsname:rank> -i <ino>"
              << std::endl;
    std::cout << "example : \n"
              << "  amend info of inode(1099531628828)\n"
              << "    stonefs-meta-injection showm -r stonefs_a:0 -i 1099531628828 -o out\n"
              << "    alter file\n"
              << "    stonefs-meta-injection amend -r stonefs_a:0 -i 1099531628828 --in out --yes-i-really-mean-it"
              << std::endl;
