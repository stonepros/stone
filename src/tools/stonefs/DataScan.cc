...
#include "cls/stonefs/cls_stonefs_client.h"
...
{
  std::cout << "Usage: \n"
    << "  stonefs-data-scan init [--force-init]\n"
    << "  stonefs-data-scan scan_extents [--force-pool] [--worker_n N --worker_m M] <data pool name>\n"
    << "  stonefs-data-scan scan_inodes [--force-pool] [--force-corrupt] [--worker_n N --worker_m M] <data pool name>\n"
    << "  stonefs-data-scan pg_files <path> <pg id> [<pg id>...]\n"
    << "  stonefs-data-scan scan_links\n"
    << "\n"
    ...
    << "\n"
    << "  stonefs-data-scan scan_frags [--force-corrupt]\n"
    << "  stonefs-data-scan cleanup <data pool name>\n"
    << std::endl;
    ...
    inodeno_t inono, int mode, int64_t data_pool_id)
{
  ...
  inode->ctime = inode->mtime = stone_clock_now();
  inode->nlink = 1;
  ...
  if (inono == STONE_INO_ROOT || MDS_INO_IS_MDSDIR(inono)) {
    sr_t srnode;
    srnode.seq = 1;
  ...
  // Serialize
  bufferlist inode_bl;
  encode(std::string(STONE_FS_ONDISK_MAGIC), inode_bl);
  inode_data.encode(inode_bl, STONE_FEATURES_SUPPORTED_DEFAULT);
...
{
  int r = 0;
  r = inject_unlinked_inode(STONE_INO_ROOT, S_IFDIR|0755, data_pool_id);
  if (r != 0) {
    return r;
  }
...
{
  int r;
  r = root_exists(STONE_INO_ROOT, result);
  if (r != 0) {
    return r;
  }
  if (!*result) {
...
int DataScan::scan_extents()
{
        ...
        uint64_t obj_name_offset) -> int
  {
    ...
    r = ClsStoneFSClient::accumulate_inode_metadata(
        data_io,
        obj_name_ino,
        obj_name_offset,
        size,
...
int DataScan::probe_filter(librados::IoCtx &ioctx)
{
  ...
  ClsStoneFSClient::build_tag_filter("test", &filter_bl);
    ...
    )
{
  ...
  if (untagged_only) {
    ...
    // the stonefs pgls filtering mode
    legacy_filtering = !probe_filter(ioctx);
    if (!legacy_filtering) {
      ClsStoneFSClient::build_tag_filter(filter_tag, &filter_bl);
    }
  }
  ...
  while(range_i < range_end) {
    ...
    for (const auto &i : result) {
      ...
      if (untagged_only && legacy_filtering) {
        ...
        stone_assert(obj_name_offset == 0);
...
int DataScan::scan_inodes()
{
        ...
        uint64_t obj_name_offset) -> int
  {
    ...
    r = ClsStoneFSClient::fetch_inode_accumulate_result(
...
int DataScan::cleanup()
{
  ...
  return forall_objects(data_io, true, [this](
      ...
      {
      int r = 0;
      r = ClsStoneFSClient::delete_inode_accumulate_result(data_io, oid);
      if (r < 0) {
...
int DataScan::scan_links()
{
  ...
  for (int step = SCAN_INOS; step <= CHECK_LINK; step++) {
	...
	try {
	  ...
	  if (dentry_type == 'I' || dentry_type == 'i') {
		...
		std::string key;
		dentry_key_t dn_key(STONE_NOSNAP, dname.c_str());
		dn_key.encode(key);
  ...
  for (auto& p : dup_primaries) {
	...
	continue;
      ...
      std::string key;
      dentry_key_t dn_key(STONE_NOSNAP, q.name.c_str());
      dn_key.encode(key);
    ...
    inodeno_t ino, const InodeStore &dentry)
{
  ...
  bool created = false;
  int r = find_or_create_dirfrag(STONE_INO_ROOT, frag_t(), &created);
  if (r < 0) {
    return r;
  }
  InodeStore lf_ino;
  r = read_dentry(STONE_INO_ROOT, frag_t(), "lost+found", &lf_ino);
  if (r == -ENOENT || r == -EINVAL) {
    ...
    fragstat.nfiles = 1,
    build_dir_dentry(STONE_INO_LOST_AND_FOUND, fragstat, inherit_layout, &lf_ino);

    // Inject link to LF inode in the root dir
    r = inject_linkage(STONE_INO_ROOT, "lost+found", frag_t(), lf_ino);
    if (r < 0) {
      return r;
    }
  } else {
  ...
  r = find_or_create_dirfrag(STONE_INO_LOST_AND_FOUND, frag_t(), &created);
  if (r < 0) {
    return r;
  }
    ...
    const frag_t fragment, const InodeStore &inode, const snapid_t dnfirst)
{
  ...
  encode('I', dentry_bl);
  inode.encode_bare(dentry_bl, STONE_FEATURES_SUPPORTED_DEFAULT);
    ...
    const file_layout_t &layout, InodeStore *out)
{
  ...
  inode->inline_data.version = STONE_INLINE_NONE;
