                   ...
                   )
{
  ...
  if (!manual_mode) {
    ...
    if (features == 0)
      features = STONE_FEATURES_SUPPORTED_DEFAULT;
    else if (features != STONE_FEATURES_SUPPORTED_DEFAULT) {
...
void MetaTool::inode_meta_t::encode(::stone::bufferlist& bl, uint64_t features)
{
    ::encode(_f, bl);
    ::encode(_t, bl);
