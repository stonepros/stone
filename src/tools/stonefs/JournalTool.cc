...
{
  std::cout << "Usage: \n"
    << "  stonefs-journal-tool [options] journal <command>\n"
    << "    <command>:\n"
    << "      inspect\n"
    << "      import <path> [--force]\n"
    << "      export <path>\n"
    << "      reset [--force]\n"
    << "  stonefs-journal-tool [options] header <get|set> <field> <value>\n"
    << "    <field>: [trimmed_pos|expire_pos|write_pos|pool_id]\n"
    << "  stonefs-journal-tool [options] event <effect> <selector> <output> [special options]\n"
    << "    <selector>:\n"
    << "      --range=<start>..<end>\n"
    << "      --path=<substring>\n"
    << "      --inode=<integer>\n"
...
int JournalTool::main(std::vector<const char*> &argv)
{
  ...
  std::string rank_str;
  if (!stone_argparse_witharg(argv, arg, &rank_str, "--rank", (char*)NULL)) {
    derr << "missing mandatory \"--rank\" argument" << dendl;
    return -EINVAL;
  }

  if (!stone_argparse_witharg(argv, arg, &type, "--journal", (char*)NULL)) {
    // Default is mdlog
    type = "mdlog";
  }
...
int JournalTool::main_event(std::vector<const char*> &argv)
{
  ...
  if (command == "recover_dentries") {
    if (type != "mdlog") {
      ...
      if (arg != argv.end() && stone_argparse_flag(argv, arg, "--dry_run", (char*)NULL)) {
        dry_run = true;
      }
    }
  }
  ...
  while(arg != argv.end()) {
    std::string arg_str;
    if (stone_argparse_witharg(argv, arg, &arg_str, "--path", (char*)NULL)) {
      output_path = arg_str;
    } else if (stone_argparse_witharg(argv, arg, &arg_str, "--alternate-pool",
				     nullptr)) {
  ...
  if (command == "get") {
	...
	found.insert(std::string(s));
      ...
      object_t frag_oid;
      frag_oid = InodeStore::get_object_name(STONE_INO_LOST_AND_FOUND,
					     frag_t(), "");
...
int JournalTool::recover_dentries(
    ...
    std::set<inodeno_t> *consumed_inos)
{
  stone_assert(consumed_inos != NULL);
  ...
  for (const auto& fb : metablob.roots) {
    ...
    if (write_root_ino && !dry_run) {
      ...
      encode(std::string(STONE_FS_ONDISK_MAGIC), new_root_ino_bl);
...
int JournalTool::erase_region(JournalScanner const &js, uint64_t const pos, uint64_t const length)
{
  ...
  if (type == "mdlog") {
    ENoOp enoop(0);
    enoop.encode_with_header(tmp, STONE_FEATURES_SUPPORTED_DEFAULT);
  ...
  if (type == "mdlog") {
    ...
    ENoOp enoop(padding);
    enoop.encode_with_header(entry, STONE_FEATURES_SUPPORTED_DEFAULT);
  } else if (type == "purge_queue") {
    PurgeItem pi;
    pi.pad_size = padding;
    pi.encode(entry);
  ...
  stone_assert(log_data.length() == length);
  ...
  bufferlist *out_bl)
{
  ...
  if (bare) {
    new_inode.encode_bare(*out_bl, STONE_FEATURES_SUPPORTED_DEFAULT);
  } else {
    new_inode.encode(*out_bl, STONE_FEATURES_SUPPORTED_DEFAULT);
  }
}
