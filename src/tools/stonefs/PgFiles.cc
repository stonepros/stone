...
int PgFiles::init()
{
  int r = stone_create_with_context(&cmount, g_stone_context);
  if (r != 0) {
    return r;
  }

  return stone_init(cmount);
}
...
void PgFiles::hit_dir(std::string const &path)
{
  ...
  stone_dir_result *dr = nullptr;
  int r = stone_opendir(cmount, path.c_str(), &dr);
  if (r != 0) {
  ...
  struct dirent de;
  while((r = stone_readdir_r(cmount, dr, &de)) != 0) {
    if (r < 0) {
      derr << "Error reading path " << path << ": " << cpp_strerror(r)
           << dendl;
      stone_closedir(cmount, dr); // best effort, ignore r
      return;
    }
    ...
    r = stone_statx(cmount, de_path.c_str(), &stx,
		    STONE_STATX_INO|STONE_STATX_SIZE, 0);
    if (r != 0) {
...
void PgFiles::hit_file(std::string const &path, const struct stone_statx &stx)
{
  stone_assert(S_ISREG(stx.stx_mode));
  ...
  int l_object_size = 0;
  int l_pool_id = 0;
  int r = stone_get_path_layout(cmount, path.c_str(), &l_stripe_unit,
  ...
  for (uint64_t i = 0; i < num_objects; ++i) {
    ...
    target.m_seed = stone_stable_mod(target.ps(), pg_num, pg_num_mask);
