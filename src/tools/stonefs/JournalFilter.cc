  ...
  std::vector<const char*>::iterator &arg)
{
  while(arg != argv.end()) {
    std::string arg_str;
    if (stone_argparse_witharg(argv, arg, &arg_str, "--range", (char*)NULL)) {
    ...
    } else if (stone_argparse_witharg(argv, arg, &arg_str, "--path", (char*)NULL)) {
	...
	return -EINVAL;
      ...
      path_expr = arg_str;
    } else if (stone_argparse_witharg(argv, arg, &arg_str, "--inode", (char*)NULL)) {
    ...
    } else if (stone_argparse_witharg(argv, arg, &arg_str, "--type", (char*)NULL)) {
      try {
    ...
    } else if (stone_argparse_witharg(argv, arg, &arg_str, "--frag", (char*)NULL)) {
      if (!type.compare("purge_queue")) {
    ...
    } else if (stone_argparse_witharg(argv, arg, &arg_str, "--dname", (char*)NULL)) {
      if (!type.compare("purge_queue")) {
    ...
    } else if (stone_argparse_witharg(argv, arg, &arg_str, "--client", (char*)NULL)) {
      if (!type.compare("purge_queue")) {
