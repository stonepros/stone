...
struct EstimateResult {
  ...
  stone::mutex lock = stone::make_mutex("EstimateResult::lock");
  ...
  void add_chunk(bufferlist& chunk, const std::string& fp_algo) {
    ...
    if (fp_algo == "sha1") {
      ...
      stone_assert(0 == "no support fingerperint algorithm");
    }
...
stone::mutex glock = stone::make_mutex("glock");
...
class CrawlerThread : public Thread
{
  ...
  ObjectCursor begin;
  ObjectCursor end;
  stone::mutex m_lock = stone::make_mutex("CrawlerThread::Locker");
			  ...
			  std::vector<const char*> &nargs)
{
  ...
  if (op_name == "chunk-get-ref" ||
    ...
    hobject_t oid(sobject_t(target_object_name, STONE_NOSNAP), "", hash, pool_id, "");
...
int main(int argc, const char **argv)
{
  ...
  for (i = args.begin(); i != args.end(); ) {
    if (stone_argparse_double_dash(args, i)) {
      break;
    } else if (stone_argparse_witharg(args, i, &val, "--op", (char*)NULL)) {
      opts["op_name"] = val;
      op_name = val;
    } else if (stone_argparse_witharg(args, i, &val, "--pool", (char*)NULL)) {
      opts["pool"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--object", (char*)NULL)) {
      opts["object"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--chunk-algorithm", (char*)NULL)) {
      opts["chunk-algorithm"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--chunk-size", (char*)NULL)) {
      opts["chunk-size"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--fingerprint-algorithm", (char*)NULL)) {
      opts["fingerprint-algorithm"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--chunk-pool", (char*)NULL)) {
      opts["chunk-pool"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--target-ref", (char*)NULL)) {
      opts["target-ref"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--target-ref-pool-id", (char*)NULL)) {
      opts["target-ref-pool-id"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--max-thread", (char*)NULL)) {
      opts["max-thread"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--report-period", (char*)NULL)) {
      opts["report-period"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--max-read-size", (char*)NULL)) {
      opts["max-seconds"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--max-seconds", (char*)NULL)) {
      opts["max-seconds"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--min-chunk-size", (char*)NULL)) {
      opts["min-chunk-size"] = val;
    } else if (stone_argparse_witharg(args, i, &val, "--max-chunk-size", (char*)NULL)) {
