...
#include "StonexServiceHandler.h"
#include "StonexProtocol.h"
#include "StonexKeyServer.h"
#include <errno.h>
#include <sstream>
...
#undef dout_prefix
#define dout_prefix *_dout << "stonex server " << entity_name << ": "
...
int StonexServiceHandler::do_start_session(
  ...
  AuthCapsInfo *caps)
{
  ...
  server_challenge = stone::util::generate_random_number<uint64_t>(min, max);
...
int StonexServiceHandler::verify_old_ticket(
  const StoneXAuthenticate& req,
  ...
  bool& should_enc_ticket)
{
  ...
  stone_assert(global_id_status != global_id_status_t::NONE);
  ...
  if (!stonex_decode_ticket(cct, key_server, STONE_ENTITY_TYPE_AUTH,
...
int StonexServiceHandler::handle_request(
  ...
  std::string *pconnection_secret)
{
  ...
  struct StoneXRequestHeader stonex_header;
  try {
    decode(stonex_header, indata);
  } catch (stone::buffer::error& e) {
    ldout(cct, 0) << __func__ << " failed to decode StoneXRequestHeader: "
		  << e.what() << dendl;
    return -EPERM;
  }

  switch (stonex_header.request_type) {
  case STONEX_GET_AUTH_SESSION_KEY:
	...
	ldout(cct, 0) << __func__ << " failed to decode StoneXAuthenticate: "
		      << e.what() << dendl;
	ret = -EPERM;
	break;
      }
	...
	break;
      ...
      if (!error.empty()) {
	ldout(cct, 0) << " stonex_calc_client_server_challenge error: " << error << dendl;
	ret = -EACCES;
	break;
      }
	...
	break;
      ...
      double ttl;
      if (!key_server->get_service_secret(STONE_ENTITY_TYPE_AUTH,
      ...
      info.service_id = STONE_ENTITY_TYPE_AUTH;
      info.ticket.name = entity_name;
      info.ticket.global_id = global_id;
      info.ticket.init_timestamps(stone_clock_now(), ttl);
	...
	*psession_key = session_key;
      ...
      vector<StoneXSessionAuthInfo> info_vec;
      info_vec.push_back(info);

      build_stonex_response_header(stonex_header.request_type, 0, *result_bl);
      if (!stonex_build_service_ticket_reply(
	...
	break;
      ...
      if (!key_server->get_service_caps(entity_name, STONE_ENTITY_TYPE_MON,
					*caps)) {
	...
	if (req.other_keys) {
	  ...
	  if (!info_vec.empty()) {
	    CryptoKey no_key;
	    stonex_build_service_ticket_reply(
  ...
  case STONEX_GET_PRINCIPAL_SESSION_KEY:
	...
	break;
      ...
      StoneXServiceTicketRequest ticket_req;
      try {
	decode(ticket_req, indata);
      } catch (stone::buffer::error& e) {
	ldout(cct, 0) << __func__
		      << " failed to decode StoneXServiceTicketRequest: "
		      << e.what() << dendl;
	ret = -EPERM;
	break;
      }
      ...
      for (uint32_t service_id = 1; service_id <= ticket_req.keys;
	   service_id <<= 1) {
        // skip STONE_ENTITY_TYPE_AUTH: auth ticket must be obtained with
        // STONEX_GET_AUTH_SESSION_KEY
	...
	ret = service_err;
      ...
      CryptoKey no_key;
      build_stonex_response_header(stonex_header.request_type, ret, *result_bl);
      stonex_build_service_ticket_reply(cct, auth_ticket_info.session_key,
  ...
  default:
    ldout(cct, 10) << "handle_request unknown op " << stonex_header.request_type << dendl;
    return -EINVAL;
  }
  return ret;
}

void StonexServiceHandler::build_stonex_response_header(int request_type, int status, bufferlist& bl)
{
