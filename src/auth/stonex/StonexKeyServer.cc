...
#undef dout_prefix
#define dout_prefix *_dout << "stonex keyserverdata: "
...
bool KeyServerData::get_service_secret(StoneContext *cct, uint32_t service_id,
				       CryptoKey& secret, uint64_t& secret_id,
				       double& ttl) const
{
  auto iter = rotating_secrets.find(service_id);
  if (iter == rotating_secrets.end()) { 
    ldout(cct, 10) << "get_service_secret service " << stone_entity_type_name(service_id) << " not found " << dendl;
    return false;
  }
  ...
  ttl = service_id == STONE_ENTITY_TYPE_AUTH ?
  ...
  ldout(cct, 30) << __func__ << " service "
		 << stone_entity_type_name(service_id) << " secret_id "
				...
				uint64_t secret_id, CryptoKey& secret) const
{
  ...
  if (riter == secrets.secrets.end()) {
    ldout(cct, 10) << "get_service_secret service " << stone_entity_type_name(service_id)
...
bool KeyServerData::get_caps(StoneContext *cct, const EntityName& name,
...
#undef dout_prefix
#define dout_prefix *_dout << "stonex keyserver: "


KeyServer::KeyServer(StoneContext *cct_, KeyRing *extra_secrets)
  : cct(cct_),
    data(extra_secrets),
    lock{stone::make_mutex("KeyServer::lock")}
{
}
...
void KeyServer::_dump_rotating_secrets()
{
  ...
  for (auto iter = data.rotating_secrets.begin();
    ...
    for (auto mapiter = key.secrets.begin();
	 ...
	 ++mapiter)
      ldout(cct, 30) << "service " << stone_entity_type_name(iter->first)
	             << " id " << mapiter->first
...
int KeyServer::_rotate_secret(uint32_t service_id, KeyServerData &pending_data)
{
  ...
  int added = 0;
  utime_t now = stone_clock_now();
  double ttl = service_id == STONE_ENTITY_TYPE_AUTH ? cct->_conf->auth_mon_ticket_ttl : cct->_conf->auth_service_ticket_ttl;
  ...
  while (r.need_new_secrets(now)) {
    ...
    ldout(cct, 10) << "_rotate_secret adding " << stone_entity_type_name(service_id) << dendl;
    ldout(cct, 30) << "_rotate_secret adding " << stone_entity_type_name(service_id)
	           << " id " << secret_id << " " << ek
	           << dendl;
    added++;
  }
...
{
  bufferptr bp;
  CryptoHandler *crypto = cct->get_crypto_handler(STONE_CRYPTO_AES);
  if (!crypto)
    return false;
  ...
  secret.set_secret(STONE_CRYPTO_AES, bp, stone_clock_now());
...
bool KeyServer::prepare_rotating_update(bufferlist& rotating_bl)
{
  ...
  int added = 0;
  added += _rotate_secret(STONE_ENTITY_TYPE_AUTH, pending_data);
  added += _rotate_secret(STONE_ENTITY_TYPE_MON, pending_data);
  added += _rotate_secret(STONE_ENTITY_TYPE_OSD, pending_data);
  added += _rotate_secret(STONE_ENTITY_TYPE_MDS, pending_data);
  added += _rotate_secret(STONE_ENTITY_TYPE_MGR, pending_data);
  if (!added) {
    return false;
  }
...
bool KeyServer::_get_service_caps(const EntityName& name, uint32_t service_id,
				  AuthCapsInfo& caps_info) const
{
  string s = stone_entity_type_name(service_id);
