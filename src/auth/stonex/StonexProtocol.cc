...
#undef dout_prefix
#define dout_prefix *_dout << "stonex: "
...
void stonex_calc_client_server_challenge(StoneContext *cct, CryptoKey& secret, uint64_t server_challenge, 
		  uint64_t client_challenge, uint64_t *key, std::string &error)
{
  StoneXChallengeBlob b;
  ...
  uint64_t k = 0;
  const stone_le64 *p = (const stone_le64 *)enc.c_str();
...
bool stonex_build_service_ticket_blob(StoneContext *cct, StoneXSessionAuthInfo& info,
				     StoneXTicketBlob& blob)
{
  ...
  ldout(cct, 10) << "build_service_ticket service "
		 << stone_entity_type_name(info.service_id)
  ...
  if (!error.empty()) {
    ldout(cct, -1) << "stonex_build_service_ticket_blob failed with error "
	  << error << dendl;
    return false;
  }
  return true;
 ...
 */
bool stonex_build_service_ticket_reply(StoneContext *cct,
                     CryptoKey& principal_secret,
                     vector<StoneXSessionAuthInfo> ticket_info_vec,
 ...
 */
bool StoneXTicketHandler::verify_service_ticket_reply(
  CryptoKey& secret,
...
bool StoneXTicketHandler::have_key()
{
  if (have_key_flag) {
    have_key_flag = stone_clock_now() < expires;
  }

  return have_key_flag;
}

bool StoneXTicketHandler::need_key() const
{
  if (have_key_flag) {
    return (!expires.is_zero()) && (stone_clock_now() >= renew_after);
  }

  return true;
}

bool StoneXTicketManager::have_key(uint32_t service_id)
{
...
bool StoneXTicketManager::need_key(uint32_t service_id) const
{
  auto iter = tickets_map.find(service_id);
  if (iter == tickets_map.end())
    return true;
...
void StoneXTicketManager::set_have_need_key(uint32_t service_id, uint32_t& have, uint32_t& need)
{
  auto iter = tickets_map.find(service_id);
  if (iter == tickets_map.end()) {
    have &= ~service_id;
  ...
  //ldout(cct, 10) << "set_have_need_key service " << stone_entity_type_name(service_id)
  //<< " (" << service_id << ")"
...
void StoneXTicketManager::invalidate_ticket(uint32_t service_id)
{
  auto iter = tickets_map.find(service_id);
  if (iter != tickets_map.end())
 ...
 * this ServiceTicket with the result.
 */
bool StoneXTicketManager::verify_service_ticket_reply(CryptoKey& secret,
						     bufferlist::const_iterator& indata)
{
  ...
  for (int i=0; i<(int)num; i++) {
    ...
    ldout(cct, 10) << "got key for service_id " << stone_entity_type_name(type) << dendl;
    StoneXTicketHandler& handler = get_handler(type);
 ...
 */
StoneXAuthorizer *StoneXTicketHandler::build_authorizer(uint64_t global_id) const
{
  StoneXAuthorizer *a = new StoneXAuthorizer(cct);
 ...
 * ticket, {timestamp}^session_key
 */
StoneXAuthorizer *StoneXTicketManager::build_authorizer(uint32_t service_id) const
{
  auto iter = tickets_map.find(service_id);
  if (iter == tickets_map.end()) {
  ...
  const StoneXTicketHandler& handler = iter->second;
  return handler.build_authorizer(global_id);
}

void StoneXTicketManager::validate_tickets(uint32_t mask, uint32_t& have, uint32_t& need)
{
  uint32_t i;
  need = 0;
  for (i = 1; i<=mask; i<<=1) {
			 ...
			 StoneXServiceTicketInfo& ticket_info)
{
  ...
  if (secret_id == (uint64_t)-1) {
    if (!keys->get_secret(cct->_conf->name, service_secret)) {
      ldout(cct, 0) << "stone_decode_ticket could not get general service secret for service_id="
	      << stone_entity_type_name(service_id) << " secret_id=" << secret_id << dendl;
      return false;
    }
  } else {
    if (!keys->get_service_secret(service_id, secret_id, service_secret)) {
      ldout(cct, 0) << "stone_decode_ticket could not get service secret for service_id=" 
	      << stone_entity_type_name(service_id) << " secret_id=" << secret_id << dendl;
      return false;
    }
  }
  ...
  if (!error.empty()) {
    ldout(cct, 0) << "stone_decode_ticket could not decrypt ticket info. error:" 
	<< error << dendl;
    return false;
  }
 ...
 */
bool stonex_verify_authorizer(StoneContext *cct, const KeyStore& keys,
			     bufferlist::const_iterator& indata,
			     ...
			     bufferlist *reply_bl)
{
  ...
  } catch (stone::buffer::end_of_buffer &e) {
    // Unable to decode!
    return false;
  }
  ...
  if (ticket.secret_id == (uint64_t)-1) {
    ...
    if (!keys.get_secret(name, service_secret)) {
      ldout(cct, 0) << "verify_authorizer could not get general service secret for service "
	      << stone_entity_type_name(service_id) << " secret_id=" << ticket.secret_id << dendl;
      return false;
    }
  } else {
	...
	stone_abort_msg("got secret_id=0");
      return false;
    }
  }
  std::string error;
  ...
  if (challenge) {
    auto *c = static_cast<StoneXAuthorizeChallenge*>(challenge->get());
    if (!auth_msg.have_challenge || !c) {
      c = new StoneXAuthorizeChallenge;
      challenge->reset(c);
...
bool StoneXAuthorizer::verify_reply(bufferlist::const_iterator& indata,
				   std::string *connection_secret)
{
  StoneXAuthorizeReply reply;
...
bool StoneXAuthorizer::add_challenge(StoneContext *cct,
