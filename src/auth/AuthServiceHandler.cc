...
#include "stonex/StonexServiceHandler.h"
#ifdef HAVE_GSSAPI
...
int AuthServiceHandler::start_session(const EntityName& entity_name,
				      ...
				      AuthCapsInfo *caps)
{
  stone_assert(!this->entity_name.get_type() && !this->global_id &&
...
AuthServiceHandler *get_auth_service_handler(int type, StoneContext *cct, KeyServer *ks)
{
  switch (type) {
  case STONE_AUTH_STONEX:
    return new StonexServiceHandler(cct, ks);
