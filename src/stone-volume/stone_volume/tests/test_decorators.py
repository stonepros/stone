import os
import pytest
from stone_volume import exceptions, decorators, terminal
...
class TestNeedsRoot(object):
    ...
    def test_is_not_root_env_var_skip_needs_root(self, monkeypatch):
        ...
        monkeypatch.setattr(decorators.os, 'environ', {'STONE_VOLUME_SKIP_NEEDS_ROOT': '1'})
        assert decorators.needs_root(func)() is True
...
class TestCatches(object):
    ...
    def teardown(self):
        try:
            del(os.environ['STONE_VOLUME_DEBUG'])
        except KeyError:
            pass

    def test_stone_volume_debug_enabled(self):
        os.environ['STONE_VOLUME_DEBUG'] = '1'
    ...
    def test_stone_volume_debug_disabled_no_exit(self, capsys):
    ...
    def test_stone_volume_debug_exits(self, capsys):
        @decorators.catches()
        def func():
            raise RuntimeError()
