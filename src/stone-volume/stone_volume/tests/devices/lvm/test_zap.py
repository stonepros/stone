...
from stone_volume.devices.lvm import zap
...
class TestFindAssociatedDevices(object):
    ...
    def test_no_lvs_found_that_match_id(self, monkeypatch, device_info):
        tags = 'stone.osd_id=9,stone.journal_uuid=x,stone.type=data'
    ...
    def test_no_lvs_found_that_match_fsid(self, monkeypatch, device_info):
        tags = 'stone.osd_id=9,stone.osd_fsid=asdf-lkjh,stone.journal_uuid=x,'+\
               'stone.type=data'
    ...
    def test_no_stone_lvs_found(self, monkeypatch):
    ...
    def test_lv_is_matched_fsid(self, monkeypatch):
        tags = 'stone.osd_id=0,stone.osd_fsid=asdf-lkjh,stone.journal_uuid=x,' +\
               'stone.type=data'
        osd = api.Volume(lv_name='volume1', lv_uuid='y', vg_name='',
                         lv_path='/dev/VolGroup/lv', lv_tags=tags)
        volumes = []
...
class TestEnsureAssociatedLVs(object):
    ...
    def test_data_is_found(self):
        tags = 'stone.osd_id=0,stone.osd_fsid=asdf-lkjh,stone.journal_uuid=x,stone.type=data'
        osd = api.Volume(
    ...
    def test_block_is_found(self):
        tags = 'stone.osd_id=0,stone.osd_fsid=asdf-lkjh,stone.journal_uuid=x,stone.type=block'
        osd = api.Volume(
    ...
    def test_journal_is_found(self):
        tags = 'stone.osd_id=0,stone.osd_fsid=asdf-lkjh,stone.journal_uuid=x,stone.type=journal'
        osd = api.Volume(
    ...
    def test_multiple_dbs_are_found(self):
        tags = 'stone.osd_id=0,stone.osd_fsid=asdf-lkjh,stone.journal_uuid=x,stone.type=db'
        volumes = []
        for i in range(3):
            osd = api.Volume(
                lv_name='volume%s' % i, lv_uuid='y', vg_name='', lv_path='/dev/VolGroup/lv%s' % i, lv_tags=tags)
    ...
    def test_multiple_wals_are_found(self):
        tags = 'stone.osd_id=0,stone.osd_fsid=asdf-lkjh,stone.wal_uuid=x,stone.type=wal'
        volumes = []
        for i in range(3):
            osd = api.Volume(
                lv_name='volume%s' % i, lv_uuid='y', vg_name='', lv_path='/dev/VolGroup/lv%s' % i, lv_tags=tags)
    ...
    def test_multiple_backing_devs_are_found(self):
        ...
        for _type in ['journal', 'db', 'wal']:
            tags = 'stone.osd_id=0,stone.osd_fsid=asdf-lkjh,stone.wal_uuid=x,stone.type=%s' % _type
            osd = api.Volume(
    ...
    @patch('stone_volume.devices.lvm.zap.api.get_lvs')
    def test_ensure_associated_lvs(self, m_get_lvs):
        zap.ensure_associated_lvs([], lv_tags={'stone.osd_id': '1'})
        calls = [
            call(tags={'stone.type': 'journal', 'stone.osd_id': '1'}),
            call(tags={'stone.type': 'db', 'stone.osd_id': '1'}),
            call(tags={'stone.type': 'wal', 'stone.osd_id': '1'})
        ]
...
class TestWipeFs(object):
    ...
    def setup(self):
        os.environ['STONE_VOLUME_WIPEFS_INTERVAL'] = '0'

    def test_works_on_second_try(self, stub_call):
        os.environ['STONE_VOLUME_WIPEFS_TRIES'] = '2'
