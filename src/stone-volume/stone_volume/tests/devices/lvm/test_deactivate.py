...
from stone_volume.devices.lvm import deactivate

class TestDeactivate(object):

    @patch("stone_volume.devices.lvm.deactivate.get_lvs_by_tag")
    ...
    def test_unmount_tmpfs_called_osd_uuid(self, p_u_tmpfs, p_get_lvs):
        FooVolume = lvm.Volume(
            lv_name='foo', lv_path='/dev/vg/foo',
            lv_tags="stone.osd_fsid=0,stone.osd_id=1,stone.cluster_name=foo,stone.type=data")
        p_get_lvs.return_value = [FooVolume]

        deactivate.deactivate_osd(None, 0)
        p_u_tmpfs.assert_called_with(
            '/var/lib/stone/osd/{}-{}'.format('foo', 1))

    @patch("stone_volume.devices.lvm.deactivate.get_lvs_by_tag")
    @patch("stone_volume.util.system.unmount_tmpfs")
    @patch("stone_volume.util.encryption.dmcrypt_close")
    ...
    def test_crypt_dmclose(self, p_dm_close, p_u_tmpfs, p_get_lvs):
        FooVolume = lvm.Volume(
            ...
            lv_tags="stone.osd_id=0,stone.encrypted=1,stone.cluster_name=foo,stone.type=data")
        p_get_lvs.return_value = [FooVolume]
