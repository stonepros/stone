...
class TestOSDs(object):
    ...
    def test_stone_osd_package_is_installed(self, node, host):
        assert host.package("stone-osd").is_installed
    ...
    def test_osds_listen_on_public_network(self, node, host):
        ...
        assert host.check_output(
            "netstat -lntp | grep stone-osd | grep %s | wc -l" % (node["address"])) == str(nb_port)  # noqa E501

    def test_osds_listen_on_cluster_network(self, node, host):
        # TODO: figure out way to paramaterize this test
        nb_port = (node["num_osds"] * node["num_osd_ports"])
        assert host.check_output("netstat -lntp | grep stone-osd | grep %s | wc -l" %  # noqa E501
    ...
    def test_osd_services_are_running(self, node, host):
        ...
        for osd in node["osds"]:
            assert host.service("stone-osd@%s" % osd).is_running
    ...
    def test_osd_are_mounted(self, node, host):
        ...
        for osd_id in node["osd_ids"]:
            osd_path = "/var/lib/stone/osd/{cluster}-{osd_id}".format(
                cluster=node["cluster_name"],
                osd_id=osd_id,
            )
            assert host.mount_point(osd_path).exists

    def test_stone_volume_is_installed(self, node, host):
        host.exists('stone-volume')

    def test_stone_volume_systemd_is_installed(self, node, host):
        host.exists('stone-volume-systemd')
    ...
    def test_all_osds_are_up_and_in(self, node, host):
        cmd = "sudo stone --cluster={cluster} --connect-timeout 5 --keyring /var/lib/stone/bootstrap-osd/{cluster}.keyring -n client.bootstrap-osd osd tree -f json".format(  # noqa E501
            cluster=node["cluster_name"])
