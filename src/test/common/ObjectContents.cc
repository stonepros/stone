...
bool test_object_contents()
{
  ...
  stone_assert(bl2.length() == 20);

  for (ObjectContents::Iterator iter3 = c.get_iterator();
       iter.valid();
       ++iter) {
    stone_assert(bl2[iter3.get_pos()] == *iter3);
  }

  stone_assert(bl2[0] == '\0');
  stone_assert(bl2[7] == '\0');

  interval_set<uint64_t> to_clone;
  to_clone.insert(5, 10);
  d.clone_range(c, to_clone);
  stone_assert(d.size() == 15);
  ...
  for (uint64_t i = 5; i < 15; ++i, ++iter2) {
    ...
    stone_assert(iter2.get_pos() == i);
    stone_assert(*iter2 == bl2[i]);
  }
  return true;
}
