...
/*
 *  STONE messenger engine
 *
 *  FIO engine which uses stone messenger as a transport.  See corresponding
...
enum stone_msgr_type {
  STONE_MSGR_TYPE_UNDEF,
  STONE_MSGR_TYPE_POSIX,
  STONE_MSGR_TYPE_DPDK,
  STONE_MSGR_TYPE_RDMA,
};

const char *stone_msgr_types[] = { "undef", "async+posix",
				  "async+dpdk", "async+rdma" };

struct stone_msgr_options {
  ...
  const char *hostname;
  const char *conffile;
  enum stone_msgr_type ms_type;
};

class FioDispatcher;

struct stone_msgr_data {
  stone_msgr_data(struct stone_msgr_options *o_, unsigned iodepth) :
    o(o_) {
...
struct stone_msgr_io {
  struct flist_head list;
  struct stone_msgr_data *data;
  struct io_u *io_u;
  MOSDOp *req_msg; /** Cached request, valid only for sender */
};

struct stone_msgr_reply_io {
  struct flist_head list;
  MOSDOpReply *rep;
};
...
static void create_or_get_stone_context(struct stone_msgr_options *o)
{
  if (g_stone_context) {
    g_stone_context->get();
    ctx_ref++;
  ...
  cct = global_init(NULL, args, STONE_ENTITY_TYPE_CLIENT,
		    CODE_ENVIRONMENT_UTILITY,
		    CINIT_FLAG_NO_DEFAULT_CONFIG_FILE);
  /* Will use g_stone_context instead */
  cct.detach();

  common_init_finish(g_stone_context);
  g_stone_context->_conf.apply_changes(NULL);
  g_dummy_auth = new DummyAuthClientServer(g_stone_context);
  g_dummy_auth->auth_registry.refresh_config();
}

static void put_stone_context(void)
{
  if (--ctx_ref == 0) {
    ostringstream ostr;
    Formatter* f;
    ...
    g_stone_context->get_perfcounters_collection()->dump_formatted(f, false);
...
static void stone_msgr_sender_on_reply(const object_t &oid)
{
  struct stone_msgr_data *data;
  struct stone_msgr_io *io;
...
class ReplyCompletion : public Message::CompletionHook {
  struct stone_msgr_io *m_io;

public:
  ReplyCompletion(MOSDOpReply *rep, struct stone_msgr_io *io) :
    Message::CompletionHook(rep),
    m_io(io) {
  }
  void finish(int err) override {
    struct stone_msgr_data *data = m_io->data;

    ring_buffer_enqueue(&data->io_completed_q, (void *)m_io);
  }
};

static void stone_msgr_receiver_on_request(struct stone_msgr_data *data,
					  MOSDOp *req)
{
  MOSDOpReply *rep;
  ...
  if (data->io_inflight_nr) {
    ...
    struct stone_msgr_reply_io *rep_io;
...
class FioDispatcher : public Dispatcher {
  struct stone_msgr_data *m_data;

public:
  FioDispatcher(struct stone_msgr_data *data):
  ...
  void ms_fast_dispatch(Message *m) override {
    if (m_data->o->is_receiver) {
      ...
      stone_msgr_receiver_on_request(m_data, req);
    } else {
      MOSDOpReply *rep;
      ...
      stone_msgr_sender_on_reply(rep->get_oid());
    }
    m->put();
  }
...
static entity_addr_t hostname_to_addr(struct stone_msgr_options *o)
{
  entity_addr_t addr;
...
static Messenger *create_messenger(struct stone_msgr_options *o)
{
  ...
  std::string ms_type = o->ms_type != STONE_MSGR_TYPE_UNDEF ?
    stone_msgr_types[o->ms_type] :
    g_stone_context->_conf.get_val<std::string>("ms_type");

  /* o->td__>pid doesn't set value, so use getpid() instead*/
  auto nonce = o->is_receiver ? 0 : (getpid() + o->td__->thread_number);
  Messenger *msgr = Messenger::create(g_stone_context, ms_type.c_str(),
				      ename, lname, nonce);
  if (o->is_receiver) {
...
static void init_messenger(struct stone_msgr_data *data)
{
  struct stone_msgr_options *o = data->o;
  FioDispatcher *disp;
  Messenger *msgr;
...
static void free_messenger(struct stone_msgr_data *data)
{
  data->msgr->shutdown();
  data->msgr->wait();
  delete data->msgr;
...
static void put_messenger(struct stone_msgr_data *data)
{
  struct stone_msgr_options *o = data->o;
...
static int fio_stone_msgr_setup(struct thread_data *td)
{
  struct stone_msgr_options *o = (decltype(o))td->eo;
  o->td__ = td;
  stone_msgr_data *data;

  /* We have to manage global resources so we use threads */
  td->o.use_thread = 1;

  create_or_get_stone_context(o);

  if (!td->io_ops_data) {
    data = new stone_msgr_data(o, td->o.iodepth);
    init_messenger(data);
...
static void fio_stone_msgr_cleanup(struct thread_data *td)
{
  struct stone_msgr_data *data;
  unsigned nr;
...
static int fio_stone_msgr_io_u_init(struct thread_data *td, struct io_u *io_u)
{
  struct stone_msgr_options *o = (decltype(o))td->eo;
  struct stone_msgr_io *io;
  MOSDOp *req_msg = NULL;
  ...
  if (!o->is_receiver) {
    ...
    hobject_t hobj(oid, oloc.key, STONE_NOSNAP, pgid.ps(),
...
static void fio_stone_msgr_io_u_free(struct thread_data *td, struct io_u *io_u)
{
  struct stone_msgr_io *io;
...
static enum fio_q_status stone_msgr_sender_queue(struct thread_data *td,
						struct io_u *io_u)
{
  struct stone_msgr_data *data;
  struct stone_msgr_io *io;
...
static int fio_stone_msgr_getevents(struct thread_data *td, unsigned int min,
...
static struct io_u *fio_stone_msgr_event(struct thread_data *td, int event)
{
  struct stone_msgr_data *data;
  struct stone_msgr_io *io;
...
static enum fio_q_status stone_msgr_receiver_queue(struct thread_data *td,
						  struct io_u *io_u)
{
  struct stone_msgr_data *data;
  struct stone_msgr_io *io;
...
static enum fio_q_status fio_stone_msgr_queue(struct thread_data *td,
					     struct io_u *io_u)
{
  struct stone_msgr_options *o = (decltype(o))td->eo;

  if (o->is_receiver)
    return stone_msgr_receiver_queue(td, io_u);
  else
    return stone_msgr_sender_queue(td, io_u);
}

static int fio_stone_msgr_open_file(struct thread_data *td, struct fio_file *f)
{
  return 0;
}

static int fio_stone_msgr_close_file(struct thread_data *, struct fio_file *)
{
  return 0;
}
...
static std::vector<fio_option> options {
  make_option([] (fio_option& o) {
    o.name  = "receiver";
    o.lname = "STONE messenger is receiver";
    o.type  = FIO_OPT_BOOL;
    o.off1  = offsetof(struct stone_msgr_options, is_receiver);
    o.help  = "STONE messenger is sender or receiver";
    o.def   = "0";
  }),
  make_option([] (fio_option& o) {
    o.name  = "single_instance";
    o.lname = "Single instance of STONE messenger ";
    o.type  = FIO_OPT_BOOL;
    o.off1  = offsetof(struct stone_msgr_options, is_single);
    o.help  = "STONE messenger is a created once for all threads";
    o.def   = "0";
  }),
  make_option([] (fio_option& o) {
    o.name  = "hostname";
    o.lname = "STONE messenger hostname";
    o.type  = FIO_OPT_STR_STORE;
    o.off1  = offsetof(struct stone_msgr_options, hostname);
    o.help  = "Hostname for STONE messenger engine";
  }),
  make_option([] (fio_option& o) {
    o.name   = "port";
    o.lname  = "STONE messenger engine port";
    o.type   = FIO_OPT_INT;
    o.off1   = offsetof(struct stone_msgr_options, port);
    o.maxval = 65535;
    o.minval = 1;
    o.help   = "Port to use for STONE messenger";
  }),
  make_option([] (fio_option& o) {
    o.name  = "ms_type";
    o.lname = "STONE messenger transport type: async+posix, async+dpdk, async+rdma";
    o.type  = FIO_OPT_STR;
    o.off1  = offsetof(struct stone_msgr_options, ms_type);
    o.help  = "Transport type for STONE messenger, see 'ms async transport type' corresponding STONE documentation page";
    o.def   = "undef";

    o.posval[0].ival = "undef";
    o.posval[0].oval = STONE_MSGR_TYPE_UNDEF;

    o.posval[1].ival = "async+posix";
    o.posval[1].oval = STONE_MSGR_TYPE_POSIX;
    o.posval[1].help = "POSIX API";

    o.posval[2].ival = "async+dpdk";
    o.posval[2].oval = STONE_MSGR_TYPE_DPDK;
    o.posval[2].help = "DPDK";

    o.posval[3].ival = "async+rdma";
    o.posval[3].oval = STONE_MSGR_TYPE_RDMA;
    o.posval[3].help = "RDMA";
  }),
  make_option([] (fio_option& o) {
    o.name  = "stone_conf_file";
    o.lname = "STONE configuration file";
    o.type  = FIO_OPT_STR_STORE;
    o.off1  = offsetof(struct stone_msgr_options, conffile);
    o.help  = "Path to STONE configuration file";
  }),
  {} /* Last NULL */
};
...
void get_ioengine(struct ioengine_ops** ioengine_ptr)
{
   ...
   */
  ioengine.name	= "stone-msgr";
  ioengine.version	= FIO_IOOPS_VERSION;
  ioengine.flags	= FIO_DISKLESSIO | FIO_UNIDIR | FIO_PIPEIO;
  ioengine.setup	= fio_stone_msgr_setup;
  ioengine.queue	= fio_stone_msgr_queue;
  ioengine.getevents	= fio_stone_msgr_getevents;
  ioengine.event	= fio_stone_msgr_event;
  ioengine.cleanup	= fio_stone_msgr_cleanup;
  ioengine.open_file	= fio_stone_msgr_open_file;
  ioengine.close_file	= fio_stone_msgr_close_file;
  ioengine.io_u_init	= fio_stone_msgr_io_u_init;
  ioengine.io_u_free	= fio_stone_msgr_io_u_free;
  ioengine.option_struct_size = sizeof(struct stone_msgr_options);
