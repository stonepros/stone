...
/*
 *  Stone ObjectStore engine
 *
 * IO engine using Stone's ObjectStore class to test low-level performance of
 * Stone OSDs.
 *
 */
...
#include "include/stone_assert.h" // fio.h clobbers our assert.h
#include <algorithm>
...
static std::vector<fio_option> stone_options{
  make_option([] (fio_option& o) {
    o.name   = "conf";
    o.lname  = "stone configuration file";
    o.type   = FIO_OPT_STR_STORE;
    o.help   = "Path to a stone configuration file";
		      ...
		      uint64_t count)
{
...

  {
    ...
    // need it. E.g. stone-objectstore-tool
...
/// includes g_stone_context and the ObjectStore instance
struct Engine {
  /// the initial g_stone_context reference to be dropped on destruction
...
Engine::Engine(thread_data* td)
  : ref_count(0),
    ...
    perf_output_file(
      ...
      std::nullopt)
{
  // add the stone command line arguments
  auto o = static_cast<Options*>(td->eo);
  if (!o->conf) {
    throw std::runtime_error("missing conf option for stone configuration file");
  }
  ...
  // claim the g_stone_context reference and release it on destruction
  cct = global_init(nullptr, args, STONE_ENTITY_TYPE_OSD,
  ...
  os.reset(ObjectStore::create(g_stone_context,
...
struct Object {
  ...
  Object(const char* name, Collection& coll)
    : oid(hobject_t(name, "", STONE_NOSNAP, coll.pg.ps(), coll.pg.pool(), "")),
      coll(coll) {}
};
...
struct Job {
  ...
  std::vector<io_u*> events; //< completions for fio_stone_os_event()
  const bool unlink; //< unlink objects on destruction
  ...
  mono_clock::time_point last = stone::mono_clock::zero();
  unsigned index = 0;
  ...
  static vector<unsigned> parse_throttle_str(const char *p) {
    ...
    stone::for_each_substr(p, ",\"", [&ret] (auto &&s) mutable {
      if (s.size() > 0) {
      ...
      static_cast<Options*>(td->eo)->cycle_throttle_period)
{
  ...
  if (o->check_files) {
    derr << "fio_stone_objectstore checked " << checked_or_preallocated
	 << " files"<< dendl;
  }
  if (o->preallocate_files ){
    derr << "fio_stone_objectstore preallocated " << checked_or_preallocated
	 << " files"<< dendl;
  }
}
...
void Job::check_throttle()
{
  ...
  if (stone::mono_clock::is_zero(last) ||
      ((cycle_throttle_period != cycle_throttle_period.zero()) &&
       (stone::mono_clock::now() - last) > cycle_throttle_period)) {
    ...
    index++;
    index %= tvals * dtvals;
    last = stone::mono_clock::now();
  }
}

int fio_stone_os_setup(thread_data* td)
{
...
void fio_stone_os_cleanup(thread_data* td)
{
...
io_u* fio_stone_os_event(thread_data* td, int event)
{
  // return the requested event from fio_stone_os_getevents()
  auto job = static_cast<Job*>(td->io_ops_data);
  return job->events[event];
}

int fio_stone_os_getevents(thread_data* td, unsigned int min,
...
class UnitComplete : public Context {
 ...
 public:
  ...
  void finish(int r) {
    // mark the pointer to indicate completion for fio_stone_os_getevents()
    u->engine_data = reinterpret_cast<void*>(1ull);
  }
};

enum fio_q_status fio_stone_os_queue(thread_data* td, io_u* u)
{
  fio_ro_check(td, u);
  ...
  if (u->ddir == DDIR_WRITE) {
    ...
    const int flags = td_rw(td) ? STONE_OSD_OP_FLAG_FADVISE_WILLNEED : 0;
    ...
    if (o->_fastinfo_omap_len_high) {
      stone_assert(o->_fastinfo_omap_len_high >= o->_fastinfo_omap_len_low);
      // fill with the garbage as we do not care of the actual content...
      job->one_for_all_data.set_length(
	stone::util::generate_random_number(
	...
	if (o->pglog_omap_len_high &&
      ...
      if (o->pglog_omap_len_high) {
	stone_assert(o->pglog_omap_len_high >= o->pglog_omap_len_low);
	...
	omaps[ver_key].append(job->one_for_all_data);
      ...
      if (o->pglog_dup_omap_len_high) {
	//insert dup
	stone_assert(o->pglog_dup_omap_len_high >= o->pglog_dup_omap_len_low);
...
int fio_stone_os_commit(thread_data* td)
{
...
int fio_stone_os_open(thread_data* td, fio_file* f) { return 0; }
int fio_stone_os_close(thread_data* td, fio_file* f) { return 0; }

int fio_stone_os_io_u_init(thread_data* td, io_u* u)
{
...
void fio_stone_os_io_u_free(thread_data* td, io_u* u)
{
  u->engine_data = nullptr;
}
...
struct stone_ioengine : public ioengine_ops {
  stone_ioengine() : ioengine_ops({}) {
    name        = "stone-os";
    version     = FIO_IOOPS_VERSION;
    flags       = FIO_DISKLESSIO;
    setup       = fio_stone_os_setup;
    queue       = fio_stone_os_queue;
    commit      = fio_stone_os_commit;
    getevents   = fio_stone_os_getevents;
    event       = fio_stone_os_event;
    cleanup     = fio_stone_os_cleanup;
    open_file   = fio_stone_os_open;
    close_file  = fio_stone_os_close;
    io_u_init   = fio_stone_os_io_u_init;
    io_u_free   = fio_stone_os_io_u_free;
    options     = stone_options.data();
...
void get_ioengine(struct ioengine_ops** ioengine_ptr) {
  static stone_ioengine ioengine;
