...
#undef dout_prefix
#define dout_prefix *_dout << "unittest_libstonesqlite: "
...
static boost::intrusive_ptr<StoneContext> cct;

class StoneSQLiteTest : public ::testing::Test {
public:
  inline static const std::string pool = "stonesqlite";
...
protected:
  int db_open()
  {
    ...
    auto&& name = get_uri();
    sqlcatch(sqlite3_open_v2(name.c_str(), &db, SQLITE_OPEN_CREATE|SQLITE_OPEN_READWRITE|SQLITE_OPEN_URI, "stone"));
...
out:
  ...
  virtual std::string get_uri() const {
    auto uri = fmt::format("file:{}:/{}?vfs=stone", pool, get_name());
    return uri;
  }
...
TEST_F(StoneSQLiteTest, InsertBulk4096) {
  static const char SQL[] =
...
TEST_F(StoneSQLiteTest, InsertBulk) {
  static const char SQL[] =
    "CREATE TABLE foo (a INT);"
    "WITH RECURSIVE c(x) AS"
    "  ("
...
TEST_F(StoneSQLiteTest, UpdateBulk) {
  static const char SQL[] =
    "CREATE TABLE foo (a INT);"
    "WITH RECURSIVE c(x) AS"
    "  ("
...
TEST_F(StoneSQLiteTest, InsertRate) {
...
TEST_F(StoneSQLiteTest, DatabaseShrink) {
  static const char SQL[] =
    "CREATE TABLE foo (a INT);"
    "WITH RECURSIVE c(x) AS"
    "  ("
...
TEST_F(StoneSQLiteTest, InsertExclusiveRate) {
  using clock = stone::coarse_mono_clock;
  using time = stone::coarse_mono_time;
...
TEST_F(StoneSQLiteTest, InsertExclusiveWALRate) {
  using clock = stone::coarse_mono_clock;
  using time = stone::coarse_mono_time;
...
TEST_F(StoneSQLiteTest, WALTransactionSync) {
  static const char SQL[] =
    "PRAGMA locking_mode=EXCLUSIVE;"
    "PRAGMA journal_mode=WAL;"
    ...
    "    WHERE a.i = ? AND"
    "          b.i = ? AND"
    "          a.fullkey = '$.libstonesqlite_vfs.opf_sync.avgcount' AND"
    "          b.fullkey = '$.libstonesqlite_vfs.opf_sync.avgcount';"
    ;
...
TEST_F(StoneSQLiteTest, PersistTransactionSync) {
  static const char SQL[] =
    "BEGIN TRANSACTION;"
    "CREATE TABLE foo (a INT);"
    "INSERT INTO foo (a) VALUES (RANDOM());"
...
TEST_F(StoneSQLiteTest, InsertExclusiveLock) {
  static const char SQL[] =
    "PRAGMA locking_mode=EXCLUSIVE;"
    "CREATE TABLE foo (a INT);"
    "INSERT INTO foo (a) VALUES (RANDOM());"
    ...
    "    WHERE a.i = ? AND"
    "          b.i = ? AND"
    "          a.fullkey = '$.libstonesqlite_vfs.opf_lock.avgcount' AND"
    "          b.fullkey = '$.libstonesqlite_vfs.opf_lock.avgcount';"
    "SELECT a.atom, b.atom, a.atom-b.atom"
    "    FROM p AS a, p AS b"
    "    WHERE a.i = ? AND"
    "          b.i = ? AND"
    "          a.fullkey = '$.libstonesqlite_striper.lock' AND"
    "          b.fullkey = '$.libstonesqlite_striper.lock';"
    ;
...
TEST_F(StoneSQLiteTest, TransactionSizeUpdate) {
  static const char SQL[] =
    "BEGIN TRANSACTION;"
    "CREATE TABLE foo (a INT);"
    "INSERT INTO foo (a) VALUES (RANDOM());"
    ...
    "    WHERE a.i = ? AND"
    "          b.i = ? AND"
    "          a.fullkey = '$.libstonesqlite_striper.update_size' AND"
    "          b.fullkey = '$.libstonesqlite_striper.update_size';"
    ;
...
TEST_F(StoneSQLiteTest, AllocatedGrowth) {
  static const char SQL[] =
    ...
    "    WHERE a.i = ? AND"
    "          b.i = ? AND"
    "          a.fullkey = '$.libstonesqlite_striper.update_allocated' AND"
    "          b.fullkey = '$.libstonesqlite_striper.update_allocated';"
    ;
...
TEST_F(StoneSQLiteTest, DeleteBulk) {
  static const char SQL[] =
    "CREATE TABLE foo (a INT);"
    "WITH RECURSIVE c(x) AS"
    "  ("
...
TEST_F(StoneSQLiteTest, DropMassive) {
  static const char SQL[] =
    "CREATE TABLE foo (a BLOB);"
    "WITH RECURSIVE c(x) AS"
    "  ("
    ...
    "    WHERE a.i = ? AND"
    "          b.i = ? AND"
    "          a.fullkey = '$.libstonesqlite_striper.shrink' AND"
    "          b.fullkey = '$.libstonesqlite_striper.shrink';"
    "SELECT a.atom-b.atom"
    "    FROM p AS a, p AS b"
    "    WHERE a.i = ? AND"
    "          b.i = ? AND"
    "          a.fullkey = '$.libstonesqlite_striper.shrink_bytes' AND"
    "          b.fullkey = '$.libstonesqlite_striper.shrink_bytes';"
    ;
...
TEST_F(StoneSQLiteTest, InsertMassiveVerify) {
  static const char SQL[] =
    "CREATE TABLE foo (a BLOB);"
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
      ...
      auto digest = stone::crypto::digest<stone::crypto::SHA1>(bl);
...
TEST_F(StoneSQLiteTest, PerfValid) {
  static const char SQL[] =
    "SELECT json_valid(stone_perf());"
    ;
...
TEST_F(StoneSQLiteTest, StatusValid) {
  static const char SQL[] =
    "SELECT json_valid(stone_status());"
    ;
...
TEST_F(StoneSQLiteTest, CurrentTime) {
  static const char SQL[] =
...
TEST_F(StoneSQLiteTest, StatusFields) {
  static const char SQL[] =
    "SELECT json_extract(stone_status(), '$.addr');"
    "SELECT json_extract(stone_status(), '$.id');"
    ;
...
int main(int argc, char **argv) {
  ...
  std::string cluster;
  StoneInitParameters iparams = stone_argparse_early_args(args, STONE_ENTITY_TYPE_CLIENT, &cluster, &conf_file_list);
  cct = boost::intrusive_ptr<StoneContext>(common_preinit(iparams, CODE_ENVIRONMENT_UTILITY, 0), false);
  ...
  if (int rc = stonesqlite_setcct(cct.get(), nullptr); rc < 0) {
