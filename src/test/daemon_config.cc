...
TEST(DaemonConfig, SimpleSet) {
  int ret;
  ret = g_stone_context->_conf.set_val("log_graylog_port", "21");
  ASSERT_EQ(0, ret);
  g_stone_context->_conf.apply_changes(nullptr);
  char buf[128];
  memset(buf, 0, sizeof(buf));
  char *tmp = buf;
  ret = g_stone_context->_conf.get_val("log_graylog_port", &tmp, sizeof(buf));
  ASSERT_EQ(0, ret);
  ASSERT_EQ(string("21"), string(buf));
  g_stone_context->_conf.rm_val("log_graylog_port");
}

TEST(DaemonConfig, Substitution) {
  int ret;
  g_conf()._clear_safe_to_start_threads();
  ret = g_stone_context->_conf.set_val("host", "foo");
  ASSERT_EQ(0, ret);
  ret = g_stone_context->_conf.set_val("public_network", "bar$host.baz");
  ASSERT_EQ(0, ret);
  g_stone_context->_conf.apply_changes(nullptr);
  char buf[128];
  memset(buf, 0, sizeof(buf));
  char *tmp = buf;
  ret = g_stone_context->_conf.get_val("public_network", &tmp, sizeof(buf));
  ASSERT_EQ(0, ret);
...
TEST(DaemonConfig, SubstitutionTrailing) {
  ...
  ret = g_stone_context->_conf.set_val("host", "foo");
  ASSERT_EQ(0, ret);
  ret = g_stone_context->_conf.set_val("public_network", "bar$host");
  ASSERT_EQ(0, ret);
  g_stone_context->_conf.apply_changes(nullptr);
  char buf[128];
  memset(buf, 0, sizeof(buf));
...
// config: variable substitution happen only once http://tracker.stone.com/issues/7103
TEST(DaemonConfig, SubstitutionMultiple) {
  int ret;
  ret = g_stone_context->_conf.set_val("mon_host", "localhost");
  ASSERT_EQ(0, ret);
  ret = g_stone_context->_conf.set_val("keyring", "$mon_host/$cluster.keyring,$mon_host/$cluster.mon.keyring");
  ASSERT_EQ(0, ret);
  g_stone_context->_conf.apply_changes(nullptr);
  char buf[512];
  memset(buf, 0, sizeof(buf));
  char *tmp = buf;
  ret = g_stone_context->_conf.get_val("keyring", &tmp, sizeof(buf));
  ASSERT_EQ(0, ret);
  ASSERT_EQ(string("localhost/stone.keyring,localhost/stone.mon.keyring"), tmp);
...
TEST(DaemonConfig, ArgV) {
  ...
  g_stone_context->_conf.parse_argv(args);
  g_stone_context->_conf.apply_changes(nullptr);

  char buf[128];
  char *tmp = buf;
  memset(buf, 0, sizeof(buf));
  ret = g_stone_context->_conf.get_val("key", &tmp, sizeof(buf));
  ASSERT_EQ(0, ret);
...
TEST(DaemonConfig, InjectArgs) {
  ...
  ret = g_stone_context->_conf.injectargs(injection, &cout);
  ASSERT_EQ(0, ret);

  char buf[128];
  char *tmp = buf;
  memset(buf, 0, sizeof(buf));
  ret = g_stone_context->_conf.get_val("leveldb_max_open_files", &tmp, sizeof(buf));
  ASSERT_EQ(0, ret);
...
TEST(DaemonConfig, InjectArgsReject) {
  ...
  memset(buf, 0, sizeof(buf));
  ret = g_stone_context->_conf.get_val("osd_data", &tmp, sizeof(buf));
  ASSERT_EQ(0, ret);
  ...
  ret = g_stone_context->_conf.injectargs(injection2, &cout);
  ASSERT_EQ(-EPERM, ret);

  // It should be unchanged.
  memset(buf2, 0, sizeof(buf2));
  ret = g_stone_context->_conf.get_val("osd_data", &tmp2, sizeof(buf2));
  ASSERT_EQ(0, ret);
  ...
  ret = g_stone_context->_conf.injectargs(injection3, &cout);
  ASSERT_EQ(-EINVAL, ret);
}
...
TEST(DaemonConfig, InjectArgsBooleans) {
  ...
  memset(buf, 0, sizeof(buf));
  ret = g_stone_context->_conf.get_val("log_to_syslog", &tmp, sizeof(buf));
  ASSERT_EQ(0, ret);
...
TEST(DaemonConfig, InjectArgsLogfile) {
  ...
  char *tmp = buf;
  memset(buf, 0, sizeof(buf));
  ret = g_stone_context->_conf.get_val("log_file", &tmp, sizeof(buf));
  ASSERT_EQ(0, ret);
  ...
  ret = g_stone_context->_conf.set_val("log_file", "");
  ASSERT_EQ(0, ret);
  g_stone_context->_conf.apply_changes(nullptr);
  ret = g_stone_context->_conf.get_val("log_file", &tmp, sizeof(buf));
  ASSERT_EQ(0, ret);
...
TEST(DaemonConfig, ThreadSafety1) {
  ...
  // been set.
  ret = g_stone_context->_conf.set_val("osd_data", "");
  ASSERT_EQ(-EPERM, ret);
  ...
  // world.
  ret = g_stone_context->_conf.set_val("osd_data", "/tmp/crazydata");
  ASSERT_EQ(0, ret);
...
TEST(DaemonConfig, InvalidIntegers) {
  {
    int ret = g_stone_context->_conf.set_val("log_graylog_port", "rhubarb");
    ASSERT_EQ(-EINVAL, ret);
  }

  {
    ...
    int ret = g_stone_context->_conf.set_val("log_graylog_port", str);
    ASSERT_EQ(-EINVAL, ret);
  }
...
TEST(DaemonConfig, InvalidFloats) {
  {
    ...
    int ret = g_stone_context->_conf.set_val("log_stop_at_utilization", str);
    ASSERT_EQ(-EINVAL, ret);
  }
  {
    double bad_value = 2 * (double)std::numeric_limits<float>::max();
  ...
  {
    int ret = g_stone_context->_conf.set_val("log_stop_at_utilization", "not a float");
    ASSERT_EQ(-EINVAL, ret);
  }
}
