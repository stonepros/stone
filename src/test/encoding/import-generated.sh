...
ver=`bin/stone-dencoder version`
echo "version $ver"
...
echo "numgen\ttype"
for type in `bin/stone-dencoder list_types`; do

    [ -d "$archive/$ver/objects/$type" ] || mkdir -p "$archive/$ver/objects/$type"

    num=`bin/stone-dencoder type $type count_tests`
    echo "$num\t$type"
    max=$(($num - 1))
    for n in `seq 0 $max`; do
	bin/stone-dencoder type $type select_test $n encode export $tmp1
