...
TEST(ACL, SetACL) {
  ...
  ASSERT_EQ(0, stone_conf_set(cmount, "client_acl_type", "posix_acl"));
  ...
  ASSERT_EQ(0, stone_conf_set(cmount, "client_permissions", "1"));
  ...
  ASSERT_EQ(stone_fsetxattr(cmount, fd, ACL_EA_DEFAULT, acl_buf, acl_buf_size, 0), -EACCES);
  ASSERT_EQ(stone_fsetxattr(cmount, fd, ACL_EA_ACCESS, acl_buf, acl_buf_size, 0), 0);

  int tmpfd = stone_open(cmount, test_file, O_RDWR, 0);
  ASSERT_GT(tmpfd, 0);
  stone_close(cmount, tmpfd);

  struct stone_statx stx;
  ASSERT_EQ(stone_fstatx(cmount, fd, &stx, STONE_STATX_MODE, 0), 0);
  ...
  ASSERT_EQ(stone_fsetxattr(cmount, fd, ACL_EA_ACCESS, acl_buf, acl_buf_size, 0), 0);
  // ACL was deleted
  ASSERT_EQ(stone_fgetxattr(cmount, fd, ACL_EA_ACCESS, NULL, 0), -ENODATA);
...
TEST(ACL, Chmod) {
  ...
  // change mode
  ASSERT_EQ(stone_fchmod(cmount, fd, 0640), 0);

  ASSERT_EQ(stone_fstatx(cmount, fd, &stx, STONE_STATX_MODE, 0), 0);
  ASSERT_EQ(stx.stx_mode & 0777u, 0640u);

  // ACL was updated according to mode
  ASSERT_EQ(stone_fgetxattr(cmount, fd, ACL_EA_ACCESS, acl_buf, acl_buf_size), acl_buf_size);
  ASSERT_EQ(check_acl_and_mode(acl_buf, acl_buf_size, stx.stx_mode), 0);
...
TEST(ACL, DefaultACL) {
  ...
  ASSERT_EQ(stone_mkdir(cmount, test_dir1, 0750), 0);

  // set default acl
  ASSERT_EQ(stone_setxattr(cmount, test_dir1, ACL_EA_DEFAULT, acl1_buf, acl_buf_size, 0), 0);

  char test_dir2[262];
  sprintf(test_dir2, "%s/dir2", test_dir1);
  ASSERT_EQ(stone_mkdir(cmount, test_dir2, 0755), 0);

  // inherit default acl
  ASSERT_EQ(stone_getxattr(cmount, test_dir2, ACL_EA_DEFAULT, acl2_buf, acl_buf_size), acl_buf_size);
  ASSERT_EQ(memcmp(acl1_buf, acl2_buf, acl_buf_size), 0);

  // mode and ACL are updated
  ASSERT_EQ(stone_getxattr(cmount, test_dir2, ACL_EA_ACCESS, acl2_buf, acl_buf_size), acl_buf_size);
  {
    struct stone_statx stx;
    ASSERT_EQ(stone_statx(cmount, test_dir2, &stx, STONE_STATX_MODE, 0), 0);
  ...
  int fd = stone_open(cmount, test_file1, O_CREAT|O_RDWR, 0666);
  ASSERT_GT(fd, 0);

  // no default acl
  ASSERT_EQ(stone_fgetxattr(cmount, fd, ACL_EA_DEFAULT, NULL, 0), -ENODATA);

  // mode and ACL are updated
  ASSERT_EQ(stone_fgetxattr(cmount, fd, ACL_EA_ACCESS, acl2_buf, acl_buf_size), acl_buf_size);
  {
    struct stone_statx stx;
    ASSERT_EQ(stone_statx(cmount, test_file1, &stx, STONE_STATX_MODE, 0), 0);
    // other bits of mode &= acl other perm
...
TEST(ACL, Disabled) {
  ...
  ASSERT_EQ(0, stone_conf_read_file(cmount, NULL));
  ASSERT_EQ(0, stone_mount(cmount, "/"));
  ASSERT_EQ(0, stone_conf_set(cmount, "client_acl_type", ""));
  ...
  ASSERT_EQ(stone_mkdir(cmount, test_dir, 0750), 0);

  ASSERT_EQ(stone_setxattr(cmount, test_dir, ACL_EA_DEFAULT, acl_buf, acl_buf_size, 0), -EOPNOTSUPP);
  ASSERT_EQ(stone_setxattr(cmount, test_dir, ACL_EA_ACCESS, acl_buf, acl_buf_size, 0), -EOPNOTSUPP);
  ASSERT_EQ(stone_getxattr(cmount, test_dir, ACL_EA_DEFAULT, acl_buf, acl_buf_size), -EOPNOTSUPP);
  ASSERT_EQ(stone_getxattr(cmount, test_dir, ACL_EA_ACCESS, acl_buf, acl_buf_size), -EOPNOTSUPP);
