...
#include "stone_pthread_self.h"

// Startup common: create and mount stone fs
#define STARTUP_STONE() do {				\
...
// Cleanup common: unmount and release stone fs
#define CLEANUP_STONE() do {			\
...
/* Basic locking */
TEST(LibStoneFS, BasicLocking) {
  struct stone_mount_info *cmount = NULL;
  STARTUP_STONE();

  char c_file[1024];
  sprintf(c_file, "/flock_test_%d", getpid());
  const int fd = stone_open(cmount, c_file, O_RDWR | O_CREAT, fileMode);
  ASSERT_GE(fd, 0); 

  // Lock exclusively twice
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_EX, 42));
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, 43));
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, 44));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, 42));

  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, 43));
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, 44));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, 43));

  // Lock shared three times
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_SH, 42));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_SH, 43));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_SH, 44));
  // And then attempt to lock exclusively
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, 45));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, 42));
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, 45));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, 44));
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, 45));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, 43));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, 45));
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_SH | LOCK_NB, 42));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, 45));
  ...
  ASSERT_EQ(0, stone_unlink(cmount, c_file));
  CLEANUP_STONE();
}
...
struct str_ConcurrentLocking {
  const char *file;
  struct stone_mount_info *cmount;  // !NULL if shared
  sem_t sem[2];
  sem_t semReply[2];
...
static void thread_ConcurrentLocking(str_ConcurrentLocking& s) {
  struct stone_mount_info *const cmount = s.cmount;
  struct timespec ts;

  const int fd = stone_open(cmount, s.file, O_RDWR | O_CREAT, fileMode);
  ASSERT_GE(fd, 0); 

  ASSERT_EQ(-EWOULDBLOCK,
	    stone_flock(cmount, fd, LOCK_EX | LOCK_NB, stone_pthread_self()));
  PING_MAIN(1); // (1)
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_EX, stone_pthread_self()));
  PING_MAIN(2); // (2)

  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, stone_pthread_self()));
  PING_MAIN(3); // (3)

  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_SH, stone_pthread_self()));
  PING_MAIN(4); // (4)
...
TEST(LibStoneFS, ConcurrentLocking) {
  ...
  ASSERT_EQ(-EWOULDBLOCK,
	    stone_flock(cmount, fd, LOCK_EX | LOCK_NB, stone_pthread_self()));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_SH | LOCK_NB, stone_pthread_self()));
  ...
  ASSERT_EQ(-EWOULDBLOCK,
	    stone_flock(cmount, fd, LOCK_EX | LOCK_NB, stone_pthread_self()));
  ASSERT_EQ(-EWOULDBLOCK,
	    stone_flock(cmount, fd, LOCK_SH | LOCK_NB, stone_pthread_self()));
  ...
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, stone_pthread_self()));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, stone_pthread_self()));
...
TEST(LibStoneFS, ThreesomeLocking) {
  const pid_t mypid = getpid();
  struct stone_mount_info *cmount;
  STARTUP_STONE();
...
static void process_ConcurrentLocking(str_ConcurrentLocking& s) {
  ...
  ASSERT_EQ(-EWOULDBLOCK,
	    stone_flock(cmount, fd, LOCK_EX | LOCK_NB, mypid));
  PING_MAIN(1); // (1)
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_EX, mypid));
  PING_MAIN(2); // (2)

  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, mypid));
  PING_MAIN(3); // (3)

  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_SH, mypid));
  PING_MAIN(4); // (4)
...
// Disabled because of fork() issues (http://tracker.stone.com/issues/16556)
TEST(LibStoneFS, DISABLED_InterProcessLocking) {
  PROCESS_SLOW_MS();
  ...
  WAIT_WORKER(4); // (4)
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, mypid));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_SH | LOCK_NB, mypid));
  ...
  // We no longer have the lock
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, mypid));
  ASSERT_EQ(-EWOULDBLOCK, stone_flock(cmount, fd, LOCK_SH | LOCK_NB, mypid));
  ...
  // We can lock it again
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_EX | LOCK_NB, mypid));
  ASSERT_EQ(0, stone_flock(cmount, fd, LOCK_UN, mypid));
...
// Disabled because of fork() issues (http://tracker.stone.com/issues/16556)
TEST(LibStoneFS, DISABLED_ThreesomeInterProcessLocking) {
  PROCESS_SLOW_MS();
  // Process synchronization
  char c_file[1024];
  const pid_t mypid = getpid();
  ...
  TWICE(WAIT_WORKER(6); // (6)
	...
	ASSERT_EQ(-EWOULDBLOCK,
		  stone_flock(cmount, fd, LOCK_EX | LOCK_NB, mypid));
	ASSERT_EQ(-EWOULDBLOCK,
		  stone_flock(cmount, fd, LOCK_SH | LOCK_NB, mypid));
