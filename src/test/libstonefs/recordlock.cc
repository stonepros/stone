...
TEST(LibStoneFS, BasicRecordLocking) {
  ...
  // Get the root inode
  rc = stone_ll_lookup_root(cmount, &root);
  ASSERT_EQ(rc, 0); 

  // Get the inode and Fh corresponding to c_file
  rc = stone_ll_create(cmount, root, c_file, fileMode, O_RDWR | O_CREAT,
  ...
  lock1.l_len = 1024;
  lock1.l_pid = getpid();
  ASSERT_EQ(0, stone_ll_setlk(cmount, fh, &lock1, 42, false));
  ...
  lock2.l_len = 1024;
  lock2.l_pid = getpid();
  ASSERT_EQ(-EAGAIN, stone_ll_setlk(cmount, fh, &lock2, 43, false));
  ...
  // Now do a getlk
  ASSERT_EQ(0, stone_ll_getlk(cmount, fh, &lock2, 43));
  ...
  ASSERT_EQ(0, stone_ll_close(cmount, fh));
  ASSERT_EQ(0, stone_ll_unlink(cmount, root, c_file, perms));
  CLEANUP_STONE();
}
...
static void thread_ConcurrentRecordLocking(str_ConcurrentRecordLocking& s) {
  ...
  // Get the inode and Fh corresponding to c_file
  rc = stone_ll_create(cmount, root, s.file, fileMode, O_RDWR | O_CREAT,
		      &inode, &fh, &stx, 0, 0, stone_mount_perms(cmount));
  ASSERT_EQ(rc, 0); 
  ...
  lock1.l_len = 1024;
  lock1.l_pid = getpid();
  ASSERT_EQ(-EAGAIN, stone_ll_setlk(cmount, fh, &lock1, stone_pthread_self(), false));
  ...
  lock1.l_len = 1024;
  lock1.l_pid = getpid();
  ASSERT_EQ(0, stone_ll_setlk(cmount, fh, &lock1, stone_pthread_self(), true));
  PING_MAIN(2); // (2)
  ...
  lock1.l_len = 1024;
  lock1.l_pid = getpid();
  ASSERT_EQ(0, stone_ll_setlk(cmount, fh, &lock1, stone_pthread_self(), false));
  PING_MAIN(3); // (3)
...
TEST(LibStoneFS, ConcurrentRecordLocking) {
...
TEST(LibStoneFS, ThreesomeRecordLocking) {
  const pid_t mypid = getpid();
  struct stone_mount_info *cmount;
  STARTUP_STONE();
...
static void process_ConcurrentRecordLocking(str_ConcurrentRecordLocking& s) {
  ...
  lock1.l_len = 1024;
  lock1.l_pid = getpid();
  ASSERT_EQ(-EAGAIN, stone_ll_setlk(cmount, fh, &lock1, mypid, false));
  PING_MAIN(1); // (1)
  lock1.l_type = F_WRLCK;
  lock1.l_whence = SEEK_SET;
  lock1.l_start = 0;
  lock1.l_len = 1024;
  lock1.l_pid = getpid();
  ASSERT_EQ(0, stone_ll_setlk(cmount, fh, &lock1, mypid, true));
  PING_MAIN(2); // (2)
  ...
  lock1.l_len = 1024;
  lock1.l_pid = getpid();
  ASSERT_EQ(0, stone_ll_setlk(cmount, fh, &lock1, mypid, false));
  PING_MAIN(3); // (3)
...
TEST(LibStoneFS, DISABLED_InterProcessRecordLocking) {
  PROCESS_SLOW_MS();
...
// Disabled because of fork() issues (http://tracker.stone.com/issues/16556)
TEST(LibStoneFS, DISABLED_ThreesomeInterProcessRecordLocking) {
  PROCESS_SLOW_MS();
  // Process synchronization
  char c_file[1024];
  const pid_t mypid = getpid();
