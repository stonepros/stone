...
static int stone_ll_delegation_wait(struct stone_mount_info *cmount, Fh *fh,
				   unsigned cmd, stone_deleg_cb_t cb, void *priv)
{
  ...
  /* Wait 10s at most */
  do {
    ret = stone_ll_delegation(cmount, fh, cmd, cb, priv);
    usleep(10000);
...
static int set_default_deleg_timeout(struct stone_mount_info *cmount)
{
  uint32_t session_timeout = stone_get_cap_return_timeout(cmount);
  return stone_set_deleg_timeout(cmount, session_timeout - 1);
}
...
static void open_breaker_func(struct stone_mount_info *cmount, const char *filename, int flags, std::atomic_bool *opened)
{
  ...
  if (!cmount) {
    ...
    ASSERT_EQ(stone_conf_parse_env(cmount, NULL), 0);
  ...
  ASSERT_EQ(stone_ll_lookup(cmount, root, filename, &file, &stx, STONE_STATX_ALL_STATS, 0, perms), 0);
  int ret, i = 0;
  for (;;) {
    ASSERT_EQ(stone_ll_getattr(cmount, file, &stx, STONE_STATX_ALL_STATS, 0, perms), 0);
    ret = stone_ll_open(cmount, file, flags, &fh, perms);
    if (ret != -EAGAIN)
      break;
    ASSERT_LT(i++, MAX_WAIT);
    usleep(1000);
...
static void namespace_breaker_func(struct stone_mount_info *cmount, int cmd, const char *oldname, const char *newname)
{
  bool do_shutdown = false;
  ...
  for (;;) {
    ...
    case DelegTestRename:
      ret = stone_ll_rename(cmount, root, oldname, root, newname, perms);
      break;
    case DelegTestLink:
      if (!file) {
	ASSERT_EQ(stone_ll_lookup(cmount, root, oldname, &file, &stx, 0, 0, perms), 0);
      }
      ret = stone_ll_link(cmount, file, root, newname, perms);
      break;
    case DelegTestUnlink:
      ret = stone_ll_unlink(cmount, root, oldname, perms);
      break;
    default:
      // Bad command
      stone_abort();
...
static void simple_deleg_test(struct stone_mount_info *cmount, struct stone_mount_info *tcmount)
{
  Inode *root, *file;
  ...
  ASSERT_EQ(stone_ll_delegation_wait(cmount, fh, STONE_DELEGATION_WR, dummy_deleg_cb, &recalled), 0);
  ...
  ASSERT_EQ(stone_ll_delegation(cmount, fh, STONE_DELEGATION_NONE, dummy_deleg_cb, &recalled), 0);
  breaker1.join();
  ASSERT_EQ(stone_ll_close(cmount, fh), 0);
  ASSERT_EQ(stone_ll_unlink(cmount, root, filename, perms), 0);
  ...
  recalled.store(false);
  ASSERT_EQ(stone_ll_delegation_wait(cmount, fh, STONE_DELEGATION_RD, dummy_deleg_cb, &recalled), 0);
  ...
  ASSERT_EQ(stone_ll_close(cmount, fh), 0);
  ASSERT_EQ(stone_ll_unlink(cmount, root, filename, perms), 0);
  ASSERT_EQ(stone_ll_unlink(cmount, root, newname, perms), 0);
...
TEST(LibStoneFS, DelegMultiClient) {
...
TEST(LibStoneFS, DelegSingleClient) {
  struct stone_mount_info *cmount;
...
TEST(LibStoneFS, DelegTimeout) {
  ...
  ASSERT_EQ(stone_set_deleg_timeout(cmount, 2), 0);
  ...
  /* Reopen read-only */
  ASSERT_EQ(stone_ll_close(cmount, fh), 0);
  ASSERT_EQ(stone_ll_open(cmount, file, O_RDONLY, &fh, perms), 0);
  ...
  ASSERT_EQ(stone_ll_getattr(cmount, root, &stx, 0, 0, perms), -ENOTCONN);
  stone_release(cmount);
}

TEST(LibStoneFS, RecalledGetattr) {
  struct stone_mount_info *cmount1;
  ...
  Inode *root, *file;
  ASSERT_EQ(stone_ll_lookup_root(cmount1, &root), 0);
  ...
  Fh *fh;
  struct stone_statx stx;
  UserPerm *perms = stone_mount_perms(cmount1);

  ASSERT_EQ(stone_ll_create(cmount1, root, filename, 0666,
		    O_RDWR|O_CREAT|O_EXCL, &file, &fh, &stx, 0, 0, perms), 0);
  ASSERT_EQ(stone_ll_write(cmount1, fh, 0, sizeof(filename), filename),
	    static_cast<int>(sizeof(filename)));
  ASSERT_EQ(stone_ll_close(cmount1, fh), 0);

  /* New mount for read delegation */
  struct stone_mount_info *cmount2;
  ...
  ASSERT_EQ(stone_ll_lookup_root(cmount2, &root), 0);
  perms = stone_mount_perms(cmount2);
  ASSERT_EQ(stone_ll_lookup(cmount2, root, filename, &file, &stx, 0, 0, perms), 0);

  ASSERT_EQ(stone_ll_open(cmount2, file, O_WRONLY, &fh, perms), 0);
  ASSERT_EQ(stone_ll_write(cmount2, fh, 0, sizeof(filename), filename),
	    static_cast<int>(sizeof(filename)));
  ASSERT_EQ(stone_ll_close(cmount2, fh), 0);

  ASSERT_EQ(stone_ll_open(cmount2, file, O_RDONLY, &fh, perms), 0);

  /* Break delegation */
  std::atomic_bool recalled(false);
  ASSERT_EQ(stone_ll_delegation_wait(cmount2, fh, STONE_DELEGATION_RD, dummy_deleg_cb, &recalled), 0);
  ASSERT_EQ(stone_ll_read(cmount2, fh, 0, sizeof(filename), filename),
	    static_cast<int>(sizeof(filename)));
  ASSERT_EQ(stone_ll_getattr(cmount2, file, &stx, STONE_STATX_ALL_STATS, 0, perms), 0);
  std::atomic_bool opened(false);
  ...
  ASSERT_EQ(opened.load(), false);
  ASSERT_EQ(stone_ll_getattr(cmount2, file, &stx, STONE_STATX_ALL_STATS, 0, perms), 0);
  ASSERT_EQ(stone_ll_delegation(cmount2, fh, STONE_DELEGATION_NONE, dummy_deleg_cb, nullptr), 0);
  breaker1.join();
  ASSERT_EQ(stone_ll_close(cmount2, fh), 0);
  stone_unmount(cmount2);
  stone_release(cmount2);
  stone_unmount(cmount1);
  stone_release(cmount1);
}
