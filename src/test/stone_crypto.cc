...
TEST(MD5, Simple) {
  ...
  unsigned char digest[STONE_CRYPTO_MD5_DIGESTSIZE];
  h.Final(digest);
  int err;
  unsigned char want_digest[STONE_CRYPTO_MD5_DIGESTSIZE] = {
    0xac, 0xbd, 0x18, 0xdb, 0x4c, 0xc2, 0xf8, 0x5c,
    0xed, 0xef, 0x65, 0x4f, 0xcc, 0xc4, 0xa4, 0xd8,
  };
  err = memcmp(digest, want_digest, STONE_CRYPTO_MD5_DIGESTSIZE);
  ASSERT_EQ(0, err);
}
...
TEST(HMACSHA1, Simple) {
  stone::crypto::HMACSHA1 h((const unsigned char*)"sekrit", 6);
  h.Update((const unsigned char*)"foo", 3);
  unsigned char digest[STONE_CRYPTO_HMACSHA1_DIGESTSIZE];
  h.Final(digest);
  int err;
  unsigned char want_digest[STONE_CRYPTO_HMACSHA1_DIGESTSIZE] = {
    0x04, 0xbc, 0x52, 0x66, 0xb6, 0xff, 0xad, 0xad, 0x9d, 0x57,
    0xce, 0x13, 0xea, 0x8c, 0xf5, 0x6b, 0xf9, 0x95, 0x2f, 0xd6,
  };
  err = memcmp(digest, want_digest, STONE_CRYPTO_HMACSHA1_DIGESTSIZE);
  ASSERT_EQ(0, err);
}
...
TEST(Digest, SHA1) {
  auto digest = [](const bufferlist& bl) {
    return stone::crypto::digest<stone::crypto::SHA1>(bl);
  };
  {
    bufferlist bl;
...
TEST(Digest, SHA256) {
  auto digest = [](const bufferlist& bl) {
    return stone::crypto::digest<stone::crypto::SHA256>(bl);
  };
  {
    bufferlist bl;
...
TEST(Digest, SHA512) {
  auto digest = [](const bufferlist& bl) {
    return stone::crypto::digest<stone::crypto::SHA512>(bl);
  };
  {
    bufferlist bl;
...
class ForkDeathTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    // will be cleaned up with stone::crypto::shutdown(false), so we need to
    // keep them around after fork.
    stone::crypto::shutdown(true);
  }
...
void do_simple_crypto() {
  ...
  // fork, and if you comment out the stone::crypto::init, or if the
