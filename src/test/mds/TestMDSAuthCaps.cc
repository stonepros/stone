...
TEST(MDSAuthCaps, ParseGood) {
  for (int i=0; parse_good[i]; i++) {
    ...
    ASSERT_TRUE(cap.parse(g_stone_context, str, &cout));
  }
}
...
TEST(MDSAuthCaps, ParseBad) {
  for (int i=0; parse_bad[i]; i++) {
    ...
    ASSERT_FALSE(cap.parse(g_stone_context, str, &cout));
  }
}
...
TEST(MDSAuthCaps, AllowAll) {
  ...
  ASSERT_TRUE(cap.parse(g_stone_context, "allow r", NULL));
  ASSERT_FALSE(cap.allow_all());
  cap = MDSAuthCaps();

  ASSERT_TRUE(cap.parse(g_stone_context, "allow rw", NULL));
  ASSERT_FALSE(cap.allow_all());
  cap = MDSAuthCaps();

  ASSERT_TRUE(cap.parse(g_stone_context, "allow", NULL));
  ASSERT_FALSE(cap.allow_all());
  cap = MDSAuthCaps();
...
TEST(MDSAuthCaps, AllowUid) {
  MDSAuthCaps cap(g_stone_context);
  ASSERT_TRUE(cap.parse(g_stone_context, "allow * uid=10", NULL));
  ASSERT_FALSE(cap.allow_all());
...
TEST(MDSAuthCaps, AllowUidGid) {
  MDSAuthCaps cap(g_stone_context);
  ASSERT_TRUE(cap.parse(g_stone_context, "allow * uid=10 gids=10,11,12; allow * uid=12 gids=12,10", NULL));
  ASSERT_FALSE(cap.allow_all());
...
TEST(MDSAuthCaps, AllowPath) {
  MDSAuthCaps cap;
  ASSERT_TRUE(cap.parse(g_stone_context, "allow * path=/sandbox", NULL));
  ASSERT_FALSE(cap.allow_all());
...
TEST(MDSAuthCaps, AllowPathChars) {
  ...
  ASSERT_TRUE(unquo_cap.parse(g_stone_context, "allow * path=/sandbox-._foo", NULL));
...
TEST(MDSAuthCaps, AllowPathCharsQuoted) {
  MDSAuthCaps quo_cap;
  ASSERT_TRUE(quo_cap.parse(g_stone_context, "allow * path=\"/sandbox-._foo\"", NULL));
...
TEST(MDSAuthCaps, RootSquash) {
  MDSAuthCaps rs_cap;
  ASSERT_TRUE(rs_cap.parse(g_stone_context, "allow rw root_squash, allow rw path=/sandbox", NULL));
...
TEST(MDSAuthCaps, OutputParsed) {
  ...
  for (size_t i = 0; i < num_tests; ++i) {
    ...
    ASSERT_TRUE(cap.parse(g_stone_context, test_values[i].input, &cout));
...
TEST(MDSAuthCaps, network) {
  ...
  MDSAuthCaps cap;
  ASSERT_TRUE(cap.parse(g_stone_context, "allow * network 192.168.0.0/16, allow * network 10.0.0.0/8", NULL));
