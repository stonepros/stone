...
#include "stone_erasure_code_benchmark.h"
...
int ErasureCodeBench::setup(int argc, char** argv) {
  ...
  vector<string> stone_option_strings = po::collect_unrecognized(
  ...
  cct = global_init(
    NULL, stone_options, STONE_ENTITY_TYPE_CLIENT,
...
int ErasureCodeBench::encode()
{
  ...
  utime_t begin_time = stone_clock_now();
  ...
  utime_t end_time = stone_clock_now();
...
/*
 * Local Variables:
 * compile-command: "cd ../../../build ; make -j4 stone_erasure_code_benchmark &&
 *   valgrind --tool=memcheck --leak-check=full \
 *      ./bin/stone_erasure_code_benchmark \
 *      --plugin jerasure \
