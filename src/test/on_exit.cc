...
int main(int argc, char **argv)
{
  ...
  stone_assert(func_scope_val == 0);
  func_scope();
  stone_assert(func_scope_val == 8);

  // shared mem for exit tests
  shared_val = (int*)mmap(NULL, sizeof(int),
      PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
  stone_assert(shared_val != MAP_FAILED);
  ...
  *shared_val = 0;
  ...
  if (pid) {
    ...
    stone_assert(ret == pid); // should be our child
    stone_assert(status == 0);
    stone_assert(*shared_val == MAIN_SCOPE_VAL);
  } else {
  ...
  *shared_val = 0;
  ...
  if (pid) {
    ...
    int ret = waitpid(pid, &status, 0);
    stone_assert(ret == pid); // should be our child
    stone_assert(WEXITSTATUS(status) == 3);
    stone_assert(*shared_val == EXIT_FUNC_VAL);
  } else {
    // child adds a callback to the static scope callback manager and then
    // exits via exit().
    stone_assert(*shared_val == 0);
