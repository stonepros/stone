...
static void usage()
{
  cout << "usage: stone-osd -i <ID> [flags]\n"
...
int main(int argc, const char **argv)
{
  ...
  auto cct = global_init(
    &defaults,
    args, STONE_ENTITY_TYPE_OSD,
    CODE_ENVIRONMENT_DAEMON, 0);
  stone_heap_profiler_init();
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ) {
    if (stone_argparse_double_dash(args, i)) {
      break;
    } else if (stone_argparse_flag(args, i, "--mkfs", (char*)NULL)) {
      mkfs = true;
    } else if (stone_argparse_witharg(args, i, &val, "--osdspec-affinity", (char*)NULL)) {
     osdspec_affinity = val;
    } else if (stone_argparse_flag(args, i, "--mkjournal", (char*)NULL)) {
      mkjournal = true;
    } else if (stone_argparse_flag(args, i, "--check-allows-journal", (char*)NULL)) {
      check_allows_journal = true;
    } else if (stone_argparse_flag(args, i, "--check-wants-journal", (char*)NULL)) {
      check_wants_journal = true;
    } else if (stone_argparse_flag(args, i, "--check-needs-journal", (char*)NULL)) {
      check_needs_journal = true;
    } else if (stone_argparse_flag(args, i, "--mkkey", (char*)NULL)) {
      mkkey = true;
    } else if (stone_argparse_flag(args, i, "--flush-journal", (char*)NULL)) {
      flushjournal = true;
    } else if (stone_argparse_flag(args, i, "--convert-filestore", (char*)NULL)) {
      convertfilestore = true;
    } else if (stone_argparse_witharg(args, i, &val, "--dump-pg-log", (char*)NULL)) {
      dump_pg_log = val;
    } else if (stone_argparse_flag(args, i, "--dump-journal", (char*)NULL)) {
      dump_journal = true;
    } else if (stone_argparse_flag(args, i, "--get-cluster-fsid", (char*)NULL)) {
      get_cluster_fsid = true;
    } else if (stone_argparse_flag(args, i, "--get-osd-fsid", "--get-osd-uuid", (char*)NULL)) {
      get_osd_fsid = true;
    } else if (stone_argparse_flag(args, i, "--get-journal-fsid", "--get-journal-uuid", (char*)NULL)) {
      get_journal_fsid = true;
    } else if (stone_argparse_witharg(args, i, &device_path,
  ...
  if (get_device_fsid) {
    uuid_d uuid;
    int r = ObjectStore::probe_block_device_fsid(g_stone_context, device_path,
						 &uuid);
    if (r < 0) {
  ...
  ObjectStore *store = ObjectStore::create(g_stone_context,
					   store_type,
					   data_path,
					   journal_path,
                                           flags);
  ...
  if (mkkey) {
    ...
    int ret = keyring.load(g_stone_context, keyring_path);
    if (ret == 0 &&
	keyring.get_auth(ename, eauth)) {
      derr << "already have key in keyring " << keyring_path << dendl;
    } else {
      eauth.key.create(g_stone_context, STONE_CRYPTO_AES);
  ...
  if (mkfs) {
    ...
    int err = OSD::mkfs(g_stone_context, store, g_conf().get_val<uuid_d>("fsid"),
...
flushjournal_out:
  ...
  if (strcmp(magic.c_str(), STONE_OSD_ONDISK_MAGIC)) {
    derr << "OSD magic " << magic << " != my " << STONE_OSD_ONDISK_MAGIC
	 << dendl;
    forker.exit(1);
  }
  ...
  Messenger *ms_public = Messenger::create(g_stone_context, public_msg_type,
					   entity_name_t::OSD(whoami), "client", nonce);
  Messenger *ms_cluster = Messenger::create(g_stone_context, cluster_msg_type,
					    entity_name_t::OSD(whoami), "cluster", nonce);
  Messenger *ms_hb_back_client = Messenger::create(g_stone_context, cluster_msg_type,
					     entity_name_t::OSD(whoami), "hb_back_client", nonce);
  Messenger *ms_hb_front_client = Messenger::create(g_stone_context, public_msg_type,
					     entity_name_t::OSD(whoami), "hb_front_client", nonce);
  Messenger *ms_hb_back_server = Messenger::create(g_stone_context, cluster_msg_type,
						   entity_name_t::OSD(whoami), "hb_back_server", nonce);
  Messenger *ms_hb_front_server = Messenger::create(g_stone_context, public_msg_type,
						    entity_name_t::OSD(whoami), "hb_front_server", nonce);
  Messenger *ms_objecter = Messenger::create(g_stone_context, public_msg_type,
  ...
  if (!ms_public || !ms_cluster || !ms_hb_front_client || !ms_hb_back_client || !ms_hb_back_server || !ms_hb_front_server || !ms_objecter)
    forker.exit(1);
  ms_cluster->set_cluster_protocol(STONE_OSD_PROTOCOL);
  ms_hb_front_client->set_cluster_protocol(STONE_OSD_PROTOCOL);
  ms_hb_back_client->set_cluster_protocol(STONE_OSD_PROTOCOL);
  ms_hb_back_server->set_cluster_protocol(STONE_OSD_PROTOCOL);
  ms_hb_front_server->set_cluster_protocol(STONE_OSD_PROTOCOL);
  ...
  boost::scoped_ptr<Throttle> client_byte_throttler(
    new Throttle(g_stone_context, "osd_client_bytes", message_size));
  uint64_t message_cap = g_conf().get_val<uint64_t>("osd_client_message_cap");
  boost::scoped_ptr<Throttle> client_msg_throttler(
    new Throttle(g_stone_context, "osd_client_messages", message_cap));
  ...
  ms_objecter->set_default_policy(Messenger::Policy::lossy_client(STONE_FEATURE_OSDREPLYMUX));

  entity_addrvec_t public_addrs, cluster_addrs;
  r = pick_addresses(g_stone_context, STONE_PICK_ADDRESS_PUBLIC, &public_addrs,
		     iface_preferred_numa_node);
  if (r < 0) {
    derr << "Failed to pick public address." << dendl;
    forker.exit(1);
  }
  r = pick_addresses(g_stone_context, STONE_PICK_ADDRESS_CLUSTER, &cluster_addrs,
		     iface_preferred_numa_node);
  if (r < 0) {
  ...
  TracepointProvider::initialize<osd_tracepoint_traits>(g_stone_context);
  TracepointProvider::initialize<os_tracepoint_traits>(g_stone_context);
  TracepointProvider::initialize<bluestore_tracepoint_traits>(g_stone_context);
#ifdef WITH_OSD_INSTRUMENT_FUNCTIONS
  TracepointProvider::initialize<cyg_profile_traits>(g_stone_context);
#endif
  ...
  MonClient mc(g_stone_context, poolctx);
  if (mc.build_initial_monmap() < 0)
    return -1;
  global_init_chdir(g_stone_context);

  if (global_init_preload_erasure_code(g_stone_context) < 0) {
    forker.exit(1);
  }

  osdptr = new OSD(g_stone_context,
		   store,
		   whoami,
		   ms_cluster,
		   ms_public,
  ...
  if ((mkdir(s, 0755) == 0) && (chdir(s) == 0)) {
    dout(0) << "stone-osd: gmon.out should be in " << s << dendl;
  }
