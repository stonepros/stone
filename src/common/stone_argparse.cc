...
void stone_arg_value_type(const char * nextargstr, bool *bool_option, bool *bool_numeric)
{
...
// The defaults for StoneInitParameters
StoneInitParameters::StoneInitParameters(uint32_t module_type_)
...
static bool va_stone_argparse_binary_flag(std::vector<const char*> &args,
	...
	std::ostream *oss, ...)
{
  ...
  va_list ap;
  va_start(ap, oss);
  r = va_stone_argparse_binary_flag(args, i, ret, oss, ap);
  va_end(ap);
  return r;
}
	...
	std::ostream &oss, ...)
{
  ...
  std::string str;
  va_start(ap, oss);
  r = va_stone_argparse_witharg(args, i, &str, oss, ap);
  va_end(ap);
  if (r == 0) {
    return false;
  } else if (r < 0) {
  ...
  stone_arg_value_type(str.c_str(), &is_option, &is_numeric);
...
template bool stone_argparse_witharg<int>(std::vector<const char*> &args,
	std::vector<const char*>::iterator &i, int *ret,
	std::ostream &oss, ...);

template bool stone_argparse_witharg<long long>(std::vector<const char*> &args,
	std::vector<const char*>::iterator &i, long long *ret,
	std::ostream &oss, ...);

template bool stone_argparse_witharg<float>(std::vector<const char*> &args,
	...
	std::ostream &oss, ...)
{
  ...
  va_list ap;
  va_start(ap, oss);
  r = va_stone_argparse_witharg(args, i, ret, oss, ap);
  va_end(ap);
  return r != 0;
}
	   ...
	   std::string *cluster, std::string *conf_file_list)
{
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ) {
    ...
    else if (stone_argparse_flag(args, i, "--version", "-v", (char*)NULL)) {
      std::cout << pretty_version_to_str() << std::endl;
      _exit(0);
    }
    else if (stone_argparse_witharg(args, i, &val, "--conf", "-c", (char*)NULL)) {
      *conf_file_list = val;
    }
    else if (stone_argparse_flag(args, i, "--no-config-file", (char*)NULL)) {
      iparams.no_config_file = true;
    }
    else if (stone_argparse_witharg(args, i, &val, "--cluster", (char*)NULL)) {
      *cluster = val;
    }
    else if ((module_type != STONE_ENTITY_TYPE_CLIENT) &&
	     (stone_argparse_witharg(args, i, &val, "-i", (char*)NULL))) {
      iparams.name.set_id(val);
    }
    else if (stone_argparse_witharg(args, i, &val, "--id", "--user", (char*)NULL)) {
      iparams.name.set_id(val);
    }
    else if (stone_argparse_witharg(args, i, &val, "--name", "-n", (char*)NULL)) {
    ...
    else if (stone_argparse_flag(args, i, "--show_args", (char*)NULL)) {
      std::cout << "args: ";
...
static void generic_usage(bool is_server)
{
  std::cout <<
    ...
    "  --cluster NAME    set cluster name (default: stone)" << std::endl <<
...
bool stone_argparse_need_usage(const std::vector<const char*>& args)
{
  if (args.empty()) {
    return true;
  }
