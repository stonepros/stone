...
using stone::HeartbeatMap;


#if defined(WITH_SEASTAR) && !defined(WITH_ALIEN)
namespace crimson::common {
StoneContext::StoneContext()
...
uint32_t StoneContext::get_module_type() const
{
  return STONE_ENTITY_TYPE_OSD;
}

CryptoRandom* StoneContext::random() const
{
  return _crypto_random.get();
}

StoneContext* StoneContext::get()
{
  ++nref;
  return this;
}

void StoneContext::put()
{
  if (--nref == 0) {
    delete this;
  }
...
PerfCountersCollectionImpl* StoneContext::get_perfcounters_collection()
{
...
public:
  explicit LockdepObs(StoneContext *cct)
    : m_cct(cct), m_registered(false), lock(stone::make_mutex("lock_dep_obs")) {
  }
  ~LockdepObs() override {
    if (m_registered) {
      lockdep_unregister_stone_context(m_cct);
    }
  }
  ...
  void handle_conf_change(const ConfigProxy& conf,
    ...
    if (conf->lockdep && !m_registered) {
      lockdep_register_stone_context(m_cct);
      m_registered = true;
...
public:
  explicit MempoolObs(StoneContext *cct)
    : cct(cct), lock(stone::make_mutex("mem_pool_obs")) {
...
class StoneContextServiceThread : public Thread
{
public:
  explicit StoneContextServiceThread(StoneContext *cct)
    : _reopen_logs(false), _exit_thread(false), _cct(cct)
  {
  }

  ~StoneContextServiceThread() override {}
  ...
  void *entry() override
  {
    while (1) {
      ...
      if (_cct->_conf->heartbeat_interval) {
        auto interval = stone::make_timespan(_cct->_conf->heartbeat_interval);
...
private:
  stone::mutex _lock = stone::make_mutex("StoneContextServiceThread::_lock");
 ...
 *
 * The logging subsystem sits below most of the stone code, including
...
class LogObs : public md_config_obs_t {
  stone::logging::Log *log;
  stone::mutex lock;

public:
  explicit LogObs(stone::logging::Log *l)
    : log(l), lock(stone::make_mutex("log_obs")) {
  }
...
namespace stone::common {
// cct config watcher
class StoneContextObs : public md_config_obs_t {
  StoneContext *cct;

public:
  explicit StoneContextObs(StoneContext *cct) : cct(cct) {}
...
class StoneContextHook : public AdminSocketHook {
  StoneContext *m_cct;

public:
  explicit StoneContextHook(StoneContext *cct) : m_cct(cct) {}
...
bool StoneContext::check_experimental_feature_enabled(const std::string& feat)
{
...
bool StoneContext::check_experimental_feature_enabled(const std::string& feat,
						     std::ostream *message)
{
  std::unique_lock<stone::spinlock> lg(_feature_lock);
  ...
  if (enabled) {
    ...
    (*message) << "To enable this feature, add this to your stone.conf:\n";
    (*message) << "  enable experimental unrecoverable data corrupting features = " << feat << "\n";
  }
  return enabled;
}

int StoneContext::do_command(std::string_view command, const cmdmap_t& cmdmap,
			    Formatter *f,
			    std::ostream& ss,
			    bufferlist *out)
{
  ...
  bufferlist *out)
{
  ...
  lgeneric_dout(this, 1) << "do_command '" << command << "' '" << cmdmap << "'"
			 << dendl;
  stone_assert_always(!(command == "assert" && _conf->debug_asok_assert_abort));
  if (command == "abort") {
  ...
  if (command == "leak_some_memory") {
	...
	auto schema = _conf.get_schema(key);
	  ...
	  r = -ENOENT;
        ...
        for (const auto &option : stone_options) {
	...
	r = _conf.injectargs(args, &ss);
      ...
      stone_abort_msg("registered under wrong command?");    
    }
    f->close_section();
  }
  lgeneric_dout(this, 1) << "do_command '" << command << "' '" << cmdmap
...
StoneContext::StoneContext(uint32_t module_type_,
  ...
  : nref(1),
    ...
    crush_location(this)
{
  _log = new stone::logging::Log(&_conf->subsys);

  _log_obs = new LogObs(_log);
  _conf.add_observer(_log_obs);

  _cct_obs = new StoneContextObs(this);
  ...
  _admin_hook = new StoneContextHook(this);
  ...
  _admin_socket->register_command("perf dump name=logger,type=StoneString,req=false name=counter,type=StoneString,req=false", _admin_hook, "dump perfcounters value");
  _admin_socket->register_command("perfcounters_schema", _admin_hook, "");
  _admin_socket->register_command("perf histogram dump name=logger,type=StoneString,req=false name=counter,type=StoneString,req=false", _admin_hook, "dump perf histogram values");
  ...
  _admin_socket->register_command("perf reset name=var,type=StoneString", _admin_hook, "perf reset <name>: perf reset all or one perfcounter name");
  _admin_socket->register_command("config show", _admin_hook, "dump current config settings");
  _admin_socket->register_command("config help name=var,type=StoneString,req=false", _admin_hook, "get config setting schema and descriptions");
  _admin_socket->register_command("config set name=var,type=StoneString name=val,type=StoneString,n=N",  _admin_hook, "config set <field> <val> [<val> ...]: set a config variable");
  _admin_socket->register_command("config unset name=var,type=StoneString",  _admin_hook, "config unset <field>: unset a config variable");
  _admin_socket->register_command("config get name=var,type=StoneString", _admin_hook, "config get <field>: get the config value");
  _admin_socket->register_command(
      "config diff", _admin_hook,
      "dump diff of current config and default config");
  _admin_socket->register_command(
      "config diff get name=var,type=StoneString", _admin_hook,
      "dump diff get <field>: dump diff of current and default config setting <field>");
  _admin_socket->register_command("injectargs name=injected_args,type=StoneString,n=N", _admin_hook, "inject configuration arguments into running daemon"),
  ...
  _crypto_none = CryptoHandler::create(STONE_CRYPTO_NONE);
  _crypto_aes = CryptoHandler::create(STONE_CRYPTO_AES);
...
StoneContext::~StoneContext()
{
  ...
  delete _crypto_aes;
  if (_crypto_inited > 0) {
    stone_assert(_crypto_inited == 1);  // or else someone explicitly did
...
void StoneContext::put() {
  if (--nref == 0) {
    ...
    if (g_stone_context == this)
      g_stone_context = nullptr;
    delete this;
  } else {
...
void StoneContext::shutdown_crypto()
{
  if (--_crypto_inited == 0) {
...
void StoneContext::start_service_thread()
{
  {
    ...
    _service_thread = new StoneContextServiceThread(this);
...
void StoneContext::join_service_thread()
{
  std::unique_lock<stone::spinlock> lg(_service_thread_lock);

  StoneContextServiceThread *thread = _service_thread;
  if (!thread) {
    return;
  }
  _service_thread = NULL;
...
void StoneContext::set_init_flags(int flags)
{
  _init_flags = flags;
}

int StoneContext::get_init_flags() const
{
  return _init_flags;
}

PerfCountersCollection *StoneContext::get_perfcounters_collection()
{
  return _perf_counters_collection;
}

void StoneContext::_enable_perf_counter()
{
  assert(!_cct_perf);
...
void StoneContext::_disable_perf_counter()
{
  if (!_cct_perf) {
    return;
  }
...
void StoneContext::_refresh_perf_values()
{
  if (_cct_perf) {
...
AdminSocket *StoneContext::get_admin_socket()
{
  return _admin_socket;
}

CryptoHandler *StoneContext::get_crypto_handler(int type)
{
  switch (type) {
...
void StoneContext::notify_pre_fork()
{
  {
...
void StoneContext::notify_post_fork()
{
  stone::spin_unlock(&_fork_watchers_lock);
  for (auto &&t : _fork_watchers)
    t->handle_post_fork();
}

void StoneContext::set_mon_addrs(const MonMap& mm) {
