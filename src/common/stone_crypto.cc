...
static auto ssl_mutexes = stone::make_lock_container<stone::shared_mutex>(
  static_cast<size_t>(std::max(CRYPTO_num_locks(), 0)),
  [](const size_t i) {
    return stone::make_shared_mutex(
...
static struct {
  ...
  stone::mutex lock = stone::make_mutex("crypto::ssl::init_records::lock");;
} init_records;
  ...
  [[maybe_unused]] const int line)
{
  if (mutex_num < 0 || static_cast<size_t>(mutex_num) >= ssl_mutexes.size()) {
    stone_assert_always("openssl passed wrong mutex index" == nullptr);
  }
