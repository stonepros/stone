...
void real_clock::to_stone_timespec(const time_point& t,
				  struct stone_timespec& ts) {
  ts.tv_sec = to_time_t(t);
  ts.tv_nsec = (t.time_since_epoch() % 1s).count();
}
struct stone_timespec real_clock::to_stone_timespec(const time_point& t) {
  struct stone_timespec ts;
  to_stone_timespec(t, ts);
  return ts;
}
real_clock::time_point real_clock::from_stone_timespec(
  const struct stone_timespec& ts) {
  return time_point(seconds(ts.tv_sec) + nanoseconds(ts.tv_nsec));
}

void coarse_real_clock::to_stone_timespec(const time_point& t,
					 struct stone_timespec& ts) {
  ts.tv_sec = to_time_t(t);
  ts.tv_nsec = (t.time_since_epoch() % seconds(1)).count();
}
struct stone_timespec coarse_real_clock::to_stone_timespec(
  const time_point& t) {
  struct stone_timespec ts;
  to_stone_timespec(t, ts);
  return ts;
}
coarse_real_clock::time_point coarse_real_clock::from_stone_timespec(
  const struct stone_timespec& ts) {
  return time_point(seconds(ts.tv_sec) + nanoseconds(ts.tv_nsec));
}
...
template ostream&
operator<< <::stone::timespan::rep,
            ::stone::timespan::period> (ostream&, const ::stone::timespan&);

template ostream&
operator<< <::stone::signedspan::rep,
            ::stone::signedspan::period> (ostream&, const ::stone::signedspan&);
