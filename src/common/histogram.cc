...
// -- pow2_hist_t --
void pow2_hist_t::dump(stone::Formatter *f) const
{
...
void pow2_hist_t::encode(stone::buffer::list& bl) const
{
  ENCODE_START(1, 1, bl);
  encode(h, bl);
  ENCODE_FINISH(bl);
...
void pow2_hist_t::decode(stone::buffer::list::const_iterator& p)
{
  DECODE_START(1, p);
  decode(h, p);
  DECODE_FINISH(p);
