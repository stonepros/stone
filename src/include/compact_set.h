 ...
 *
 */
#ifndef STONE_COMPACT_SET_H
#define STONE_COMPACT_SET_H
...
template<class T, class Set>
inline void encode(const compact_set_base<T, Set>& m, stone::buffer::list& bl) {
  m.encode(bl);
}
template<class T, class Set>
inline void decode(compact_set_base<T, Set>& m, stone::buffer::list::const_iterator& p) {
  m.decode(p);
}
