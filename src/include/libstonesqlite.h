 ...
 * StoneeContext to use rather than the library instantiating its own and using
...
LIBSTONESQLITE_API int sqlite3_cephsqlite_init(sqlite3* db, char** err, const sqlite3_api_routines* api);

/* If you prefer to have libcephsqlite use a StoneeContext managed by your
 * application, use this routine to set that. libcephsqlite can only have one
 * context globally.
 */

LIBSTONESQLITE_API int cephsqlite_setcct(class StoneeContext* cct, char** ident);
#ifdef __cplusplus
}
#endif
