...
int DBObjectMap::is_buggy_ghobject_key_v1(StoneContext* cct,
					  const string &in)
{
  int dots = 5;  // skip 5 .'s
...
int DBObjectMap::DBObjectMapIteratorImpl::init()
{
  ...
  stone_assert(key_iter);
  complete_iter = map->db->get_iterator(map->complete_prefix(header));
  stone_assert(complete_iter);
  cur_iter = key_iter;
  stone_assert(cur_iter);
  ready = true;
  return 0;
}
...
bool DBObjectMap::DBObjectMapIteratorImpl::valid()
{
  ...
  stone_assert(!valid || cur_iter->valid());
  return valid;
}
							     ...
							     string *end)
{
  ...
  stone_assert(complete_iter->key() <= to_test);
  stone_assert(complete_iter->value().length() >= 1);
  ...
  if (_end.empty() || _end > to_test) {
    ...
    complete_iter->next();
    stone_assert(!complete_iter->valid() || complete_iter->key() > to_test);
    return false;
  }
}
...
int DBObjectMap::DBObjectMapIteratorImpl::adjust()
{
  ...
  stone_assert(invalid || cur_iter->valid());
  return 0;
}
		       ...
		       const SequencerPosition *spos)
{
  ...
  stone_assert(header->num_children > 0);
  header->num_children--;
			...
			KeyValueDB::Transaction t)
{
  while (1) {
    ...
    stone_assert(parent->num_children > 0);
    parent->num_children--;
    header.swap(parent);
  }
  return 0;
...
int DBObjectMap::write_state(KeyValueDB::Transaction _t) {
  stone_assert(stone_mutex_is_locked_by_me(header_lock));
...
DBObjectMap::Header DBObjectMap::lookup_parent(Header input)
{
  ...
  header->decode(iter);
  stone_assert(header->seq == input->parent);
