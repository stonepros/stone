...
void mount_stone_debug(const char *fmt, ...)
{
	if (verboseflag) {
		va_list args;
...
static int parse_src(const char *orig_str, struct stone_mount_info *cmi)
{
	size_t len;
	char *mount_path;
...
static char *finalize_src(struct stone_mount_info *cmi)
{
	int pos, len;
	char *src;
 ...
 */
static int fetch_config_info(struct stone_mount_info *cmi)
{
	int ret = 0;
	pid_t pid;
	struct stone_config_info *cci;
	...
	if (cci == MAP_FAILED) {
		mount_stone_debug("Unable to allocate memory: %s\n",
				 strerror(errno));
		return EX_SYSERR;
	}
	...
	pid = fork();
	if (pid < 0) {
		mount_stone_debug("fork() failure: %s\n", strerror(errno));
		ret = EX_SYSERR;
		goto out;
	}
	...
	if (pid == 0) {
		...
		if (ret)
			exit(1);
		mount_stone_get_config_info(cmi->cmi_conf, cmi->cmi_name, v2_addrs, cci);
		exit(0);
	} else {
		/* parent */
		pid = wait(&ret);
		if (!WIFEXITED(ret)) {
			mount_stone_debug("Child process terminated abnormally.\n");
			ret = EX_SYSERR;
			goto out;
		}
		ret = WEXITSTATUS(ret);
		if (ret) {
			mount_stone_debug("Child exited with status %d\n", ret);
			ret = EX_SYSERR;
			goto out;
		}
		...
		if (!cmi->cmi_secret[0] && cci->cci_secret[0]) {
			...
			if (len < SECRET_BUFSIZE) {
				...
				mount_stone_debug("secret is too long (len=%zu max=%zu)!\n", len, SECRET_BUFSIZE);
			}
		}
 ...
 */
static int parse_options(const char *data, struct stone_mount_info *cmi)
{
	...
	mount_stone_debug("parsing options: %s\n", data);
...
static void modprobe(void)
{
	...
	r = module_load("stone", NULL);
	if (r)
		printf("failed to load stone kernel module (%d)\n", r);
}

static void usage(const char *prog_name)
{
	printf("usage: %s [src] [mount-point] [-n] [-v] [-o stone-options]\n",
		prog_name);
	printf("options:\n");
	printf("\t-h: Print this help\n");
	printf("\t-n: Do not update /etc/mtab\n");
	printf("\t-v: Verbose\n");
	printf("\tstone-options: refer to mount.stone(8)\n");
	printf("\n");
}
 ...
 * pointers inside.
 */
static void stone_mount_info_free(struct stone_mount_info *cmi)
{
	free(cmi->cmi_opts);
	free(cmi->cmi_name);
	free(cmi->cmi_path);
...
static int append_key_or_secret_option(struct stone_mount_info *cmi)
{
	...
	if (cmi->cmi_secret[0] && (strcmp(cmi->cmi_secret, "<hidden>") != 0)) {
		...
		if (ret < 0) {
			...
			fprintf(stderr, "adding stone secret key to kernel failed: %s\n",
				strerror(-ret));
			return ret;
		}
	}
...
int main(int argc, char *argv[])
{
	...
	if (retval) {
		fprintf(stderr, "failed to parse stone_options: %d\n", retval);
		retval = EX_USAGE;
		goto out;
	}
	...
	/* Ensure the stone key_type is available */
	modprobe();
	...
	if (mount(rsrc, node, "stone", cmi.cmi_flags, cmi.cmi_opts)) {
		retval = EX_FAIL;
		switch (errno) {
		case ENODEV:
			fprintf(stderr, "mount error: stone filesystem not supported by the system\n");
			break;
		case EHOSTUNREACH:
		...
		if (!skip_mtab_flag) {
			update_mtab_entry(rsrc, node, "stone", cmi.cmi_opts, cmi.cmi_flags, 0, 0);
		}
	}

	block_signals(SIG_UNBLOCK);
out:
	stone_mount_info_free(&cmi);
	free(rsrc);
	return retval;
}
