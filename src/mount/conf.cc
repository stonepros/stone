...
extern "C" void mount_stone_get_config_info(const char *config_file,
					   const char *name,
					   bool v2_addrs,
					   struct stone_config_info *cci)
{
  ...
  stone::async::io_context_pool ioc(1);
  ...
  if (monaddrs.length())
    ...
    mount_stone_debug("Could not discover monitor addresses\n");

scrape_keyring:
  err = keyring.from_stone_context(cct.get());
  if (err) {
    mount_stone_debug("keyring.from_stone_context failed: %d\n", err);
    return;
  }

  if (!keyring.get_secret(conf->name, secret)) {
    mount_stone_debug("keyring.get_secret failed\n");
    return;
  }
  ...
  if (secret_str.length() + 1 > sizeof(cci->cci_secret)) {
    mount_stone_debug("secret is too long\n");
    return;
  }
