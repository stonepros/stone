...
void Request<I>::finish(int r) {
  ...
  stone_assert(!m_appended_op_event || m_committed_op_event);
...
bool Request<I>::commit_op_event(int r) {
  ...
  stone_assert(m_op_tid != 0);
  stone_assert(!m_committed_op_event);
  ...
  if (image_ctx.journal != nullptr &&
    ...
    stone_assert(image_ctx.journal->is_journal_ready());
