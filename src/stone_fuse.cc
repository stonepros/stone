...
stone::async::io_context_pool icp;
...
{
  cout <<
"usage: stone-fuse [-n client.username] [-m mon-ip-addr:mon-port] <mount point> [OPTIONS]\n"
"  --client_mountpoint/-r <sub_directory>\n"
"                    use sub_directory as the mounted root, rather than the full Stone tree.\n"
"\n";
  fuse_usage();
...
int main(int argc, const char **argv, const char *envp[]) {
  int filer_flags = 0;
  //cerr << "stone-fuse starting " << myrank << "/" << world << std::endl;
  ...
  for (auto i = args.begin(); i != args.end();) {
    if (stone_argparse_double_dash(args, i)) {
      break;
    } else if (stone_argparse_flag(args, i, "--localize-reads", (char*)nullptr)) {
      cerr << "setting STONE_OSD_FLAG_LOCALIZE_READS" << std::endl;
      filer_flags |= STONE_OSD_FLAG_LOCALIZE_READS;
    } else if (stone_argparse_flag(args, i, "-V", (char*)nullptr)) {
      const char* tmpargv[] = {
	"stone-fuse",
	"-V"
      };
...
#ifndef __LP64__
    cerr << std::endl;
    cerr << "WARNING: Stone inode numbers are 64 bits wide, and FUSE on 32-bit kernels does" << std::endl;
...

  {
    ...
    public:
      ...
      void init(StoneFuse *cf, Client *cl) {
	cfuse = cf;
	client = cl;
      }
	...
	if (tr != 0 && client_die_on_failed_dentry_invalidate) {
	  ...
	  if (umount_r) {
		...
		cerr << "got error " << umount_r
		     << " when unmounting Stone on failed remount test!" << std::endl;
	      } else {
    ...
    MonClient *mc = new MonClient(g_stone_context, icp);
    ...
    cfuse = new StoneFuse(client, forker.get_signal_fd());

    r = cfuse->init(newargc, newargv);
    if (r != 0) {
      cerr << "stone-fuse[" << getpid() << "]: fuse failed to initialize" << std::endl;
      goto out_messenger_start_failed;
    }

    cerr << "stone-fuse[" << getpid() << "]: starting stone client" << std::endl;
    r = messenger->start();
    if (r < 0) {
      cerr << "stone-fuse[" << getpid() << "]: stone messenger failed with " << cpp_strerror(-r) << std::endl;
      goto out_messenger_start_failed;
    }
    ...
    r = client->init();
    if (r < 0) {
      cerr << "stone-fuse[" << getpid() << "]: stone client failed with " << cpp_strerror(-r) << std::endl;
    ...
    perms = client->pick_my_perms();
    {
      ...
      if (r < 0) {
        if (r == STONE_FUSE_NO_MDS_UP) {
          cerr << "stone-fuse[" << getpid() << "]: probably no MDS server is up?" << std::endl;
        }
        cerr << "stone-fuse[" << getpid() << "]: stone mount failed with " << cpp_strerror(-r) << std::endl;
        r = EXIT_FAILURE;
        goto out_shutdown;
      }
    }
    ...
    r = cfuse->start();
    if (r != 0) {
      cerr << "stone-fuse[" << getpid() << "]: fuse failed to start" << std::endl;
      goto out_client_unmount;
    }

    cerr << "stone-fuse[" << getpid() << "]: starting fuse" << std::endl;
    ...
    cerr << "stone-fuse[" << getpid() << "]: fuse finished with error " << r
