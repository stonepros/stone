...
public:
  stripe_info_t(uint64_t stripe_size, uint64_t stripe_width)
    ...
    stone_assert(stripe_width % stripe_size == 0);
  }
...
int decode(
  ...
  std::map<int, stone::buffer::list> &to_decode,
...
int decode(
  ...
  stone::ErasureCodeInterfaceRef &ec_impl,
  std::map<int, stone::buffer::list> &to_decode,
  std::map<int, stone::buffer::list*> &out);
...
public:
  ...
  void append(uint64_t old_size, std::map<int, stone::buffer::list> &to_append);
  void clear() {
    total_chunk_size = 0;
  ...
  uint32_t get_chunk_hash(int shard) const {
    stone_assert((unsigned)shard < cumulative_shard_hashes.size());
  ...
  void set_projected_total_logical_size(
    ...
    stone_assert(sinfo.logical_offset_is_stripe_aligned(logical_size));
