...
void OnodeBlock::apply_delta(const stone::bufferlist &bl)
{
  assert(deltas.empty());

  auto p = bl.cbegin();
  uint8_t n_deltas = 0;
  stone::decode(n_deltas, p);
...
void OnodeBlock::apply_pending_changes(bool do_cleanup)
{
  ...
  if (share_buffer) {
    ...
    get_bptr() = stone::bufferptr{get_bptr().c_str(),
				 get_bptr().length()};
    share_buffer = false;
  }
  assert(mutate_func);
