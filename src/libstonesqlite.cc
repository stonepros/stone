...
#include "include/libstonesqlite.h"
#include "SimpleRADOSStriper.h"

#define dout_subsys stone_subsys_stonesqlite
#undef dout_prefix
#define dout_prefix *_dout << "stonesqlite: " << __func__ << ": "
...
struct stonesqlite_appdata {
  ...
  int setup_perf() {
    stone_assert(cct);
    PerfCountersBuilder plb(cct.get(), "libstonesqlite_vfs", P_FIRST, P_LAST);
    ...
    if (int rc = SimpleRADOSStriper::config_logger(cct.get(), "libstonesqlite_striper", &striper_logger); rc < 0) {
      return rc;
    }
...
struct stonesqlite_fileloc {
  std::string pool;
  std::string radosns;
  std::string name;
};

struct stonesqlite_fileio {
  librados::IoCtx ioctx;
  std::unique_ptr<SimpleRADOSStriper> rs;
};

std::ostream& operator<<(std::ostream &out, const stonesqlite_fileloc& fileloc) {
  return out
    << "["
    << fileloc.pool
    << ":"
...
struct stonesqlite_file {
  sqlite3_file base;
  struct sqlite3_vfs* vfs = nullptr;
  int flags = 0;
  // There are 5 lock states: https://sqlite.org/c3ref/c_lock_exclusive.html
  int lock = 0;
  struct stonesqlite_fileloc loc{};
  struct stonesqlite_fileio io{};
};


#define getdata(vfs) (*((stonesqlite_appdata*)((vfs)->pAppData)))

static StoneContext* getcct(sqlite3_vfs* vfs)
{
  ...
  env_to_vec(env_args, "STONE_ARGS");
  std::string cluster, conf_file_list; // unused
  StoneInitParameters iparams = stone_argparse_early_args(env_args, STONE_ENTITY_TYPE_CLIENT, &cluster, &conf_file_list);
  cct = boost::intrusive_ptr<StoneContext>(common_preinit(iparams, CODE_ENVIRONMENT_LIBRARY, 0), false);
  ...
  if (int rc = appd.setup_perf(); rc < 0) {
    stone_abort("cannot setup perf counters");
  }

  if (int rc = appd.init_cluster(); rc < 0) {
    stone_abort("cannot setup RADOS cluster handle");
  }
...
{
  auto f = (stonesqlite_file*)file;
  auto start = stone::coarse_mono_clock::now();
  df(5) << std::hex << ilock << dendl;

  auto& lock = f->lock;
  stone_assert(!f->io.rs->is_locked() || lock > SQLITE_LOCK_NONE);
  stone_assert(lock <= ilock);
...
static int Unlock(sqlite3_file *file, int ilock)
{
  ...
  auto& lock = f->lock;
  stone_assert(lock == SQLITE_LOCK_NONE || (lock > SQLITE_LOCK_NONE && f->io.rs->is_locked()));
  stone_assert(lock >= ilock);
...
static bool parsepath(std::string_view path, struct stonesqlite_fileloc* fileloc)
{
...
static int makestriper(sqlite3_vfs* vfs, const stonesqlite_fileloc& loc, stonesqlite_fileio* io)
{
  auto&& appd = getdata(vfs);
  auto& cct = appd.cct;
...
enoent_retry:
  ...
  io->rs->set_lock_timeout(cct->_conf.get_val<std::chrono::milliseconds>("stonesqlite_lock_renewal_timeout"));
  io->rs->set_lock_interval(cct->_conf.get_val<std::chrono::milliseconds>("stonesqlite_lock_renewal_interval"));
  io->rs->set_blocklist_the_dead(cct->_conf.get_val<bool>("stonesqlite_blocklist_dead_locker"));
                ...
                int flags,  int *oflags)
{
  ...
  auto f = new (file)stonesqlite_file();
  f->vfs = vfs;
  if (!parsepath(path, &f->loc)) {
    stone_assert(0); /* xFullPathname validates! */
  }
  f->flags = flags;
...
LIBSTONESQLITE_API int stonesqlite_setcct(StoneContext* cct, char** ident)
{
  ldout(cct, 1) << "cct: " << cct << dendl;

  if (sqlite3_api == nullptr) {
    lderr(cct) << "API violation: must have sqlite3 init libstonesqlite" << dendl;
    return -EINVAL;
  }

  auto vfs = sqlite3_vfs_find("stone");
  if (!vfs) {
    lderr(cct) << "API violation: must have sqlite3 init libstonesqlite" << dendl;
    return -EINVAL;
  }
...
static void f_perf(sqlite3_context* ctx, int argc, sqlite3_value** argv)
{
  ...
  f.open_object_section("stone_perf");
...
static void f_status(sqlite3_context* ctx, int argc, sqlite3_value** argv)
{
  ...
  auto&& appd = getdata(vfs);
  JSONFormatter f(false);
  f.open_object_section("stone_status");
...
static int autoreg(sqlite3* db, char** err, const struct sqlite3_api_routines* thunk)
{
  auto vfs = sqlite3_vfs_find("stone");
  if (!vfs) {
    stone_abort("stone vfs not found");
  }

  if (int rc = sqlite3_create_function(db, "stone_perf", 0, SQLITE_UTF8, vfs, f_perf, nullptr, nullptr); rc) {
    return rc;
  }

  if (int rc = sqlite3_create_function(db, "stone_status", 0, SQLITE_UTF8, vfs, f_status, nullptr, nullptr); rc) {
    return rc;
  }
...
LIBSTONESQLITE_API int sqlite3_stonesqlite_init(sqlite3* db, char** err, const sqlite3_api_routines* api)
{
  ...
  auto vfs = sqlite3_vfs_find("stone");
  if (!vfs) {
    auto appd = new stonesqlite_appdata;
    vfs = &appd->vfs;
    vfs->iVersion = 2;
    vfs->szOsFile = sizeof(struct stonesqlite_file);
    vfs->mxPathname = 4096;
    vfs->zName = "stone";
    vfs->pAppData = appd;
    vfs->xOpen = Open;
