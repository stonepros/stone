...
stone::real_time rgw_error_repo_decode_value(const bufferlist& bl)
{
  uint64_t value;
  try {
    using stone::decode;
  ...
  return stone::real_clock::zero() + stone::timespan(value);
}
...
class RGWErrorRepoWriteCR : public RGWSimpleCoroutine {
 ...
 public:
  RGWErrorRepoWriteCR(RGWSI_RADOS* rados, const rgw_raw_obj& raw_obj,
                      const std::string& key, stone::real_time timestamp)
