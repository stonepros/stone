...
static void usage()
{
  cout << "usage: stone-mds -i <ID> [flags]\n"
       << "  -m monitorip:port\n"
...
int main(int argc, const char **argv)
{
  ...
  auto cct = global_init(NULL, args,
			 STONE_ENTITY_TYPE_MDS, CODE_ENVIRONMENT_DAEMON, 0);
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ) {
    ...
    else if (stone_argparse_witharg(args, i, &val, "--hot-standby", (char*)NULL)) {
  ...
  pick_addresses(g_stone_context, STONE_PICK_ADDRESS_PUBLIC, &addrs);

  // Normal startup
  if (g_conf()->name.has_default_id()) {
    derr << "must specify '-i name' with the stone-mds instance name" << dendl;
    exit(1);
  }
  ...
  if (!msgr)
    forker.exit(1);
  msgr->set_cluster_protocol(STONE_MDS_PROTOCOL);
  ...
  msgr->set_policy(entity_name_t::TYPE_MON,
                   Messenger::Policy::lossy_client(STONE_FEATURE_UID |
                                                   STONE_FEATURE_PGID64));
  msgr->set_policy(entity_name_t::TYPE_MDS,
                   Messenger::Policy::lossless_peer(STONE_FEATURE_UID));
  ...
  // get monmap
  stone::async::io_context_pool ctxpool(2);
  MonClient mc(g_stone_context, ctxpool);
 ...
 shutdown:
  ...
  if ((mkdir(s, 0755) == 0) && (chdir(s) == 0)) {
    cerr << "stone-mds: gmon.out should be in " << s << std::endl;
  }
