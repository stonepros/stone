...
KernelDevice::KernelDevice(StoneContext* cct, aio_callback_t cb, void *cbpriv, aio_callback_t d_cb, void *d_cbpriv)
...
int KernelDevice::_lock()
{
  ...
  for (;;) {
    ...
    std::this_thread::sleep_for(stone::make_timespan(retry_interval));
  }
}
...
int KernelDevice::_sync_write(uint64_t off, bufferlist &bl, bool buffered, int write_hint)
{
  ...
  do {
    ...
    if (left) {
      ...
      if (r) {
        stone_assert(idx < iov.size());
        stone_assert((size_t)r < iov[idx].iov_len);
  ...
  *_dout << dendl;
  ...
  if (aio && dio && !buffered) {
	...
	rand() % cct->_conf->bdev_inject_crash == 0) {
      ...
      bufferptr p = stone::buffer::create_small_page_aligned(len);
		      ...
		      bool buffered)
{
  ...
  auto p = stone::buffer::ptr_node::create(stone::buffer::create_small_page_aligned(len));
...
int KernelDevice::direct_read_unaligned(uint64_t off, uint64_t len, char *buf)
{
  ...
  bufferptr p = stone::buffer::create_small_page_aligned(aligned_len);
  int r = 0;
