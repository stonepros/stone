/*
 * stone-dokan - Win32 StoneFS client based on Dokan
 *
...
#undef dout_prefix
#define dout_prefix *_dout << "stone-dokan: "
...
#define STONE_DOKAN_MAX_FILE_SZ (1LL << 40) // 1TB
#define STONE_DOKAN_MAX_IO_SZ (128 * 1024 * 1024) // 128MB
  ...
  fd_context* fdc)
{
  ...
  int fd = stone_open(cmount, path.c_str(), flags, mode);
  if (fd < 0) {
    dout(2) << __func__ << " " << path
            << ": stone_open failed. Error: " << fd << dendl;
    return stonefs_errno_to_ntsatus(fd);
  }
...
static NTSTATUS WinStoneCreateDirectory(
  LPCWSTR FileName,
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  int ret = stone_mkdir(cmount, path.c_str(), 0755);
  if (ret < 0) {
    dout(2) << __func__ << " " << path
            << ": stone_mkdir failed. Error: " << ret << dendl;
    return stonefs_errno_to_ntsatus(ret);
  }
  return 0;
}

static NTSTATUS WinStoneCreateFile(
  LPCWSTR FileName,
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  *fdc = { 0 };
  ...
  struct stone_statx stbuf;
  unsigned int requested_attrs = STONE_STATX_BASIC_STATS;
  int ret = stone_statx(cmount, path.c_str(), &stbuf, requested_attrs, 0);
  if (!ret) { /* File Exists */
    ...
    if (DokanFileInfo->IsDirectory) {
      ...
      if ((st = WinStoneCreateDirectory(FileName, DokanFileInfo)))
        return st;
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  int ret = stone_close(cmount, fdc->fd);
  if (ret) {
    dout(2) << __func__ << " " << path
            << " failed. fd: " << fdc->fd
            << ". Error: " << ret << dendl;
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  if (DokanFileInfo->DeleteOnClose) {
    ...
    if (DokanFileInfo->IsDirectory) {
      int ret = stone_rmdir(cmount, path.c_str());
      if (ret)
        derr << __func__ << " " << path
             << ": stone_rmdir failed. Error: " << ret << dendl;
    } else {
      int ret = stone_unlink(cmount, path.c_str());
      if (ret != 0) {
        derr << __func__ << " " << path
             << ": stone_unlink failed. Error: " << ret << dendl;
      }
    }
  }
}

static NTSTATUS WinStoneReadFile(
  LPCWSTR FileName,
  LPVOID Buffer,
  DWORD BufferLength,
  LPDWORD ReadLength,
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  if (Offset > STONE_DOKAN_MAX_FILE_SZ ||
      BufferLength > STONE_DOKAN_MAX_IO_SZ) {
  ...
  if (!fdc->fd) {
    ...
    string path = get_path(FileName);
    int fd_new = stone_open(cmount, path.c_str(), O_RDONLY, 0);
    if (fd_new < 0) {
      dout(2) << __func__ << " " << path
              << ": stone_open failed. Error: " << fd_new << dendl;
      return stonefs_errno_to_ntsatus(fd_new);
    }

    int ret = stone_read(cmount, fd_new, (char*) Buffer, BufferLength, Offset);
    if (ret < 0) {
      dout(2) << __func__ << " " << path
              << ": stone_read failed. Error: " << ret
              << ". Offset: " << Offset
              << "Buffer length: " << BufferLength << dendl;
      stone_close(cmount, fd_new);
      return stonefs_errno_to_ntsatus(ret);
    }
    *ReadLength = ret;
    stone_close(cmount, fd_new);
    ...
    int ret = stone_read(cmount, fdc->fd, (char*) Buffer, BufferLength, Offset);
    if (ret < 0) {
      dout(2) << __func__ << " " << get_path(FileName)
              << ": stone_read failed. Error: " << ret
              << ". Offset: " << Offset
...
static NTSTATUS WinStoneWriteFile(
  LPCWSTR FileName,
  LPCVOID Buffer,
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  if (Offset < 0) {
    if (DokanFileInfo->WriteToEndOfFile) {
      ...
      if (ret) {
        dout(2) << __func__ << " " << path
                << ": stone_statx failed. Error: " << ret << dendl;
        return stonefs_errno_to_ntsatus(ret);
      }
  ...
  if (Offset > STONE_DOKAN_MAX_FILE_SZ ||
      NumberOfBytesToWrite > STONE_DOKAN_MAX_IO_SZ) {
  ...
  if (!fdc->fd) {
    ...
    int fd_new = stone_open(cmount, path.c_str(), O_RDWR, 0);
    if (fd_new < 0) {
      dout(2) << __func__ << " " << path
              << ": stone_open failed. Error: " << fd_new << dendl;
      return stonefs_errno_to_ntsatus(fd_new);
    ...
    int ret = stone_write(cmount, fd_new, (char*) Buffer,
                         NumberOfBytesToWrite, Offset);
    if (ret < 0) {
      dout(2) << __func__ << " " << path
              << ": stone_write failed. Error: " << ret
              << ". Offset: " << Offset
    ...
    *NumberOfBytesWritten = ret;
    ...
    int ret = stone_write(cmount, fdc->fd, (char*) Buffer,
                         NumberOfBytesToWrite, Offset);
    if (ret < 0) {
      dout(2) << __func__ << " " << get_path(FileName)
              << ": stone_write failed. Error: " << ret
...
static NTSTATUS WinStoneFlushFileBuffers(
  LPCWSTR FileName,
  PDOKAN_FILE_INFO DokanFileInfo)
{
  pfd_context fdc = (pfd_context) &(DokanFileInfo->Context);
  ...
  int ret = stone_fsync(cmount, fdc->fd, 0);
  if (ret) {
    dout(2) << __func__ << " " << get_path(FileName)
            << ": stone_sync failed. Error: " << ret << dendl;
    return stonefs_errno_to_ntsatus(ret);
  }
  return 0;
}

static NTSTATUS WinStoneGetFileInformation(
  LPCWSTR FileName,
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  if (!fdc->fd) {
    ...
    int ret = stone_fstatx(cmount, fdc->fd, &stbuf, requested_attrs, 0);
    if (ret) {
      dout(2) << __func__ << " " << path
              << ": stone_fstatx failed. Error: " << ret << dendl;
      return stonefs_errno_to_ntsatus(ret);
    }
  }
...
static NTSTATUS WinStoneFindFiles(
  LPCWSTR FileName,
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  int ret = stone_opendir(cmount, path.c_str(), &dirp);
  if (ret != 0) {
    dout(2) << __func__ << " " << path
            << ": stone_mkdir failed. Error: " << ret << dendl;
    return stonefs_errno_to_ntsatus(ret);
  ...
  while (1) {
    ...
    unsigned int requested_attrs = STONE_STATX_BASIC_STATS;
    ret = stone_readdirplus_r(cmount, dirp, &result, &stbuf,
                             requested_attrs,
    ...
    if (ret < 0) {
      dout(2) << __func__ << " " << path
              << ": stone_readdirplus_r failed. Error: " << ret << dendl;
      return stonefs_errno_to_ntsatus(ret);
    }
 ...
 * allowed. The actual file deletion will be performed by WinStoneCleanup
 */
static NTSTATUS WinStoneDeleteFile(
  LPCWSTR FileName,
  PDOKAN_FILE_INFO DokanFileInfo)
{
  string path = get_path(FileName);
  ...
  if (stone_may_delete(cmount, path.c_str()) < 0) {
    return STATUS_ACCESS_DENIED;
  }
...
static NTSTATUS WinStoneDeleteDirectory(
  LPCWSTR FileName,
  PDOKAN_FILE_INFO DokanFileInfo)
{
  string path = get_path(FileName);
  ...
  if (ret != 0) {
    dout(2) << __func__ << " " << path
            << ": stone_opendir failed. Error: " << ret << dendl;
    return stonefs_errno_to_ntsatus(ret);
  }
  ...
  while (1) {
    memset(&findData, 0, sizeof(findData));
    struct dirent *result = stone_readdir(cmount, dirp);
    if (result) {
...
static NTSTATUS WinStoneMoveFile(
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  int ret = stone_rename(cmount, path.c_str(), new_path.c_str());
  if (ret) {
    dout(2) << __func__ << " " << path << " -> " << new_path
            << ": stone_rename failed. Error: " << ret << dendl;
  }

  return stonefs_errno_to_ntsatus(ret);
}

static NTSTATUS WinStoneSetEndOfFile(
  LPCWSTR FileName,
  LONGLONG ByteOffset,
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  int ret = stone_ftruncate(cmount, fdc->fd, ByteOffset);
  if (ret) {
    dout(2) << __func__ << " " << get_path(FileName)
            << ": stone_ftruncate failed. Error: " << ret
...
static NTSTATUS WinStoneSetAllocationSize(
  LPCWSTR FileName,
  LONGLONG AllocSize,
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  if ((unsigned long long) AllocSize < stbuf.stx_size) {
    int ret = stone_ftruncate(cmount, fdc->fd, AllocSize);
    if (ret) {
      dout(2) << __func__ << " " << get_path(FileName)
              << ": stone_ftruncate failed. Error: " << ret << dendl;
      return stonefs_errno_to_ntsatus(ret);
    }
    return 0;
  }
...
static NTSTATUS WinStoneSetFileAttributes(
  LPCWSTR FileName,
...
static NTSTATUS WinStoneSetFileTime(
  LPCWSTR FileName,
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  if (LastAccessTime) {
    mask |= STONE_SETATTR_ATIME;
    to_unix_time(*LastAccessTime, &stbuf.stx_atime.tv_sec);
  }
  if (LastWriteTime) {
    mask |= STONE_SETATTR_MTIME;
    to_unix_time(*LastWriteTime, &stbuf.stx_mtime.tv_sec);
  }

  int ret = stone_setattrx(cmount, path.c_str(), &stbuf, mask, 0);
  if (ret) {
    dout(2) << __func__ << " " << path
            << ": stone_setattrx failed. Error: " << ret << dendl;
    return stonefs_errno_to_ntsatus(ret);
  }
  return 0;
}

static NTSTATUS WinStoneSetFileSecurity(
  LPCWSTR FileName,
...
static NTSTATUS WinStoneGetVolumeInformation(
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  *FileSystemFlags = FILE_CASE_SENSITIVE_SEARCH |
  ...
  wcscpy(FileSystemNameBuffer, L"Stone");
  return 0;
}

static NTSTATUS WinStoneGetDiskFreeSpace(
  ...
  PDOKAN_FILE_INFO DokanFileInfo)
{
  ...
  int ret = stone_statfs(cmount, "/", &vfsbuf);
  if (ret) {
    derr << "stone_statfs failed. Error: " << ret << dendl;
    return stonefs_errno_to_ntsatus(ret);;
  }
...
int do_unmap(wstring& mountpoint) {
  if (!DokanRemoveMountPoint(mountpoint.c_str())) {
    wcerr << "Couldn't remove the specified StoneFS mount: "
          << mountpoint << std::endl;
    return -EINVAL;
  }
  return 0;
...
static NTSTATUS WinStoneUnmount(
  PDOKAN_FILE_INFO  DokanFileInfo)
{
  cleanup_mount();
...
NTSTATUS get_volume_serial(PDWORD serial) {
  int64_t fs_cid = stone_get_fs_cid(cmount);

  char fsid_str[64] = { 0 };
  int ret = stone_getxattr(cmount, "/", "stone.cluster_fsid",
...
int do_map() {
  ...
  dokan_operations->ZwCreateFile = WinStoneCreateFile;
  dokan_operations->Cleanup = WinStoneCleanup;
  dokan_operations->CloseFile = WinStoneCloseFile;
  dokan_operations->ReadFile = WinStoneReadFile;
  dokan_operations->WriteFile = WinStoneWriteFile;
  dokan_operations->FlushFileBuffers = WinStoneFlushFileBuffers;
  dokan_operations->GetFileInformation = WinStoneGetFileInformation;
  dokan_operations->FindFiles = WinStoneFindFiles;
  dokan_operations->SetFileAttributes = WinStoneSetFileAttributes;
  dokan_operations->SetFileTime = WinStoneSetFileTime;
  dokan_operations->DeleteFile = WinStoneDeleteFile;
  dokan_operations->DeleteDirectory = WinStoneDeleteDirectory;
  dokan_operations->MoveFile = WinStoneMoveFile;
  dokan_operations->SetEndOfFile = WinStoneSetEndOfFile;
  dokan_operations->SetAllocationSize = WinStoneSetAllocationSize;
  dokan_operations->SetFileSecurity = WinStoneSetFileSecurity;
  dokan_operations->GetDiskFreeSpace = WinStoneGetDiskFreeSpace;
  dokan_operations->GetVolumeInformation = WinStoneGetVolumeInformation;
  dokan_operations->Unmounted = WinStoneUnmount;

  stone_create_with_context(&cmount, g_stone_context);

  r = stone_mount(cmount, g_cfg->root_path.c_str());
  if (r) {
    derr << "stone_mount failed. Error: " << r << dendl;
    return stonefs_errno_to_ntsatus(r);
  }

  if (g_cfg->win_vol_name.empty()) {
    string stone_fs_name = g_conf().get_val<string>("client_fs");

    g_cfg->win_vol_name = L"Stone";
    if (!stone_fs_name.empty()) {
      g_cfg->win_vol_name += L" - " + to_wstring(stone_fs_name);
    }
  }
  ...
  dout(0) << "Mounted stonefs directory: " << g_cfg->root_path.c_str()
