...
class AES128GCM_OnWireTxHandler : public stone::crypto::onwire::TxHandler {
...
public:
  AES128GCM_OnWireTxHandler(StoneContext* const cct,
			    const key_t& key,
			    const nonce_t& nonce,
    ...
    stone_assert_always(ectx);
    stone_assert_always(key.size() * CHAR_BIT == 128);
  ...
  void authenticated_encrypt_update(const stone::bufferlist& plaintext) override;
  stone::bufferlist authenticated_encrypt_final() override;
};
                                                 ...
                                                 const uint32_t* last)
{
  if (nonce == initial_nonce) {
    if (used_initial_nonce) {
      throw stone::crypto::onwire::TxHandlerError("out of nonces");
    }
  ...
  stone_assert(buffer.get_append_buffer_unused_tail_length() == 0);
...
void AES128GCM_OnWireTxHandler::authenticated_encrypt_update(
  const stone::bufferlist& plaintext)
{
  stone_assert(buffer.get_append_buffer_unused_tail_length() >=
              plaintext.length());
  ...
  for (const auto& plainbuf : plaintext.buffers()) {
    ...
    stone_assert_always(update_len >= 0);
    stone_assert(static_cast<unsigned>(update_len) == plainbuf.length());
...
stone::bufferlist AES128GCM_OnWireTxHandler::authenticated_encrypt_final()
{
  int final_len = 0;
  stone_assert(buffer.get_append_buffer_unused_tail_length() ==
              AESGCM_BLOCK_LEN);
  ...
  stone_assert_always(final_len == 0);
...
// RX PART
class AES128GCM_OnWireRxHandler : public stone::crypto::onwire::RxHandler {
  std::unique_ptr<EVP_CIPHER_CTX, decltype(&::EVP_CIPHER_CTX_free)> ectx;
  nonce_t nonce;
  bool new_nonce_format;  // 64-bit counter?
  static_assert(sizeof(nonce) == AESGCM_IV_LEN);
...
public:
  AES128GCM_OnWireRxHandler(StoneContext* const cct,
			    const key_t& key,
			    const nonce_t& nonce,
			    bool new_nonce_format)
    : ectx(EVP_CIPHER_CTX_new(), EVP_CIPHER_CTX_free),
  ...
  void authenticated_decrypt_update(stone::bufferlist& bl) override;
  void authenticated_decrypt_update_final(stone::bufferlist& bl) override;
};
  ...
  stone::bufferlist& bl)
{
  ...
  for (auto& buf : bl.buffers()) {
    ...
    stone_assert_always(update_len >= 0);
    stone_assert(static_cast<unsigned>(update_len) == buf.length());
  }
}
  ...
  stone::bufferlist& bl)
{
  ...
  stone_assert(orig_len >= AESGCM_TAG_LEN);
  ...
  // authenticate the message.
  {
    ...
    stone_assert_always(final_len == 0);
    stone_assert(bl.length() + AESGCM_TAG_LEN == orig_len);
  }
}

stone::crypto::onwire::rxtx_t stone::crypto::onwire::rxtx_t::create_handler_pair(
  StoneContext* cct,
  ...
  bool crossed)
{
  if (auth_meta.is_mode_secure()) {
    stone_assert_always(auth_meta.connection_secret.length() >= \
