...
class NFSGaneshaUiControllerTest(ControllerTestCase):
    ...
    @patch('dashboard.controllers.nfs.StoneFS')
    def test_lsdir(self, stonefs_class):
        stonefs_class.return_value.ls_dir.return_value = [
            {'path': '/foo'},
            {'path': '/foo/bar'}
        ]
        mocked_ls_dir = stonefs_class.return_value.ls_dir

        reqs = [
            {
                'params': None,
                'stonefs_ls_dir_args': ['/', 1],
                'path0': '/',
                'status': 200
            },
            {
            ...
            },
            {
                ...
                'stonefs_ls_dir_args': ['/foo', 3],
                'path0': '/foo',
                'status': 200
            },
            {
                'params': {'root_dir': 'foo', 'depth': '6'},
                'stonefs_ls_dir_args': ['/foo', 5],
                'path0': '/foo',
                'status': 200
            },
            {
        ...
        for req in reqs:
            ...
            if req['status'] == 200:
                ...
                stonefs_class.assert_called_once_with('a')

            # Check the arguments passed to `StoneFS.ls_dir`.
            if req.get('stonefs_ls_dir_args'):
                mocked_ls_dir.assert_called_once_with(*req['stonefs_ls_dir_args'])
            else:
                mocked_ls_dir.assert_not_called()
            mocked_ls_dir.reset_mock()
            stonefs_class.reset_mock()

    @patch('dashboard.controllers.nfs.stonefs')
    @patch('dashboard.controllers.nfs.StoneFS')
    def test_lsdir_non_existed_dir(self, stonefs_class, stonefs):
        stonefs.ObjectNotFound = Exception
        stonefs.PermissionError = Exception
        stonefs_class.return_value.ls_dir.side_effect = stonefs.ObjectNotFound()
        self._get(self._create_ls_dir_url('a', {'root_dir': '/foo', 'depth': '3'}))
        stonefs_class.assert_called_once_with('a')
        stonefs_class.return_value.ls_dir.assert_called_once_with('/foo', 3)
        self.assertStatus(200)
