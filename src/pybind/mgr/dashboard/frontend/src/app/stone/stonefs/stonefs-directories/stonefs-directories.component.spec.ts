...
import {
  ...
  StonefsQuotas,
  StonefsSnapshot
} from '~/app/shared/models/stonefs-directory-models';
...
import { StonefsDirectoriesComponent } from './stonefs-directories.component';

describe('StonefsDirectoriesComponent', () => {
  let component: StonefsDirectoriesComponent;
  let fixture: ComponentFixture<StonefsDirectoriesComponent>;
  let stonefsService: StonefsService;
  ...
  const get = {
    nodeIds: (): { [path: string]: StonefsDir } => component['nodeIds'],
    dirs: (): StonefsDir[] => component['dirs'],
  ...
  let mockData: {
    nodes: any;
    parent: any;
    createdSnaps: StonefsSnapshot[] | any[];
    deletedSnaps: StonefsSnapshot[] | any[];
    updatedQuotas: { [path: string]: StonefsQuotas };
    createdDirs: StonefsDir[];
  };

  // Object contains mock functions
  const mockLib = {
    quotas: (max_bytes: number, max_files: number): StonefsQuotas => ({ max_bytes, max_files }),
    snapshots: (dirPath: string, howMany: number): StonefsSnapshot[] => {
    ...
    dir: (parentPath: string, name: string, modifier: number): StonefsDir => {
    ...
    lsDir: (_id: number, path = ''): Observable<StonefsDir[]> => {
    ...
    updateQuota: (_id: any, path: string, updated: StonefsQuotas): Observable<string> => {
    ...
    deleteSnapshotsThroughModal: (snapshots: StonefsSnapshot[]) => {
  ...
  configureTestBed(
    {
      ...
      declarations: [StonefsDirectoriesComponent],
  ...
  beforeEach(() => {
    ...
    stonefsService = TestBed.inject(StonefsService);
    lsDirSpy = spyOn(stonefsService, 'lsDir').and.callFake(mockLib.lsDir);
    spyOn(stonefsService, 'mkSnapshot').and.callFake(mockLib.mkSnapshot);
    spyOn(stonefsService, 'rmSnapshot').and.callFake(mockLib.rmSnapshot);
    spyOn(stonefsService, 'quota').and.callFake(mockLib.updateQuota);
    ...
    fixture = TestBed.createComponent(StonefsDirectoriesComponent);
  ...
  describe('mock self test', () => {
    ...
    describe('test quota update mock', () => {
      ...
      const updateQuota = (quotas: StonefsQuotas) => mockLib.updateQuota(ID, PATH, quotas);
  ...
  describe('snapshots', () => {
    ...
    it('should create a snapshot', () => {
      ...
      expect(stonefsService.mkSnapshot).toHaveBeenCalledWith(1, '/a', 'newSnap');
    ...
    describe('update modal', () => {
      describe('max_files', () => {
        ...
        it('should update max_files correctly', () => {
          expect(stonefsService.quota).toHaveBeenCalledWith(1, '/a/c/b', { max_files: 5 });
        ...
        it('shows the right texts', () => {
          assert.quotaUpdateModalTexts(
            `Update StoneFS files quota for '/a/c/b'`,
            `The inherited files quota 10 from '/a' is the maximum value to be used.`,
            `Updated StoneFS files quota for '/a/c/b'`
          );
        });
      });
      ...
      describe('max_bytes', () => {
        ...
        it('should update max_files correctly', () => {
          expect(stonefsService.quota).toHaveBeenCalledWith(1, '/a/c/b', { max_bytes: 512 });
        ...
        it('shows the right texts', () => {
          assert.quotaUpdateModalTexts(
            `Update StoneFS size quota for '/a/c/b'`,
            `The inherited size quota 1 KiB from '/a' is the maximum value to be used.`,
            `Updated StoneFS size quota for '/a/c/b'`
          );
        });
      });
      ...
      describe('action behaviour', () => {
        ...
        it(`uses 'Set' action instead of 'Update' if the quota is not set (0)`, () => {
          ...
          assert.quotaUpdateModalTexts(
            `Set StoneFS size quota for '/a/c/b'`,
            `The inherited size quota 1 KiB from '/a' is the maximum value to be used.`,
            `Set StoneFS size quota for '/a/c/b'`
          );
        });
    ...
    describe('unset modal', () => {
      describe('max_files', () => {
        ...
        it('should unset max_files correctly', () => {
          expect(stonefsService.quota).toHaveBeenCalledWith(1, '/a/c/b', { max_files: 0 });
          assert.dirQuotas(2048, 0);
        });

        it('shows the right texts', () => {
          assert.quotaUnsetModalTexts(
            `Unset StoneFS files quota for '/a/c/b'`,
      ...
      describe('max_bytes', () => {
        ...
        it('should unset max_files correctly', () => {
          expect(stonefsService.quota).toHaveBeenCalledWith(1, '/a/c/b', { max_bytes: 0 });
          assert.dirQuotas(0, 20);
        });

        it('shows the right texts', () => {
          assert.quotaUnsetModalTexts(
            `Unset StoneFS size quota for '/a/c/b'`,
      ...
      describe('action behaviour', () => {
        it('uses different Text if no quota is inherited', () => {
          ...
          mockLib.unsetQuotaThroughModal('max_bytes');
          assert.quotaUnsetModalTexts(
            `Unset StoneFS size quota for '/a'`,
