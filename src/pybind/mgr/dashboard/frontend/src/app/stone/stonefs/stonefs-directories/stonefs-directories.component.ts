...
@Component({
  selector: 'cd-stonefs-directories',
  templateUrl: './stonefs-directories.component.html',
  styleUrls: ['./stonefs-directories.component.scss']
})
export class StonefsDirectoriesComponent implements OnInit, OnChanges {
  ...
  private dirs: StonefsDir[];
  private nodeIds: { [path: string]: StonefsDir };
  ...
  constructor(
    ...
    private stonefsService: StonefsService,
  ...
  ngOnInit() {
    this.permission = this.authStorageService.getPermissions().stonefs;
  ...
  updateDirectory(path: string): Promise<any[]> {
    ...
    return new Promise((resolve) => {
      ...
      this.stonefsService.lsDir(this.id, path).subscribe((dirs) => {
  ...
  private getSubDirectories(path: string, tree: StonefsDir[] = this.dirs): StonefsDir[] {
  ...
  private createNode(dir: StonefsDir, subTree?: StonefsDir[]): any {
  ...
  private getSubTree(path: string): StonefsDir[] {
  ...
  private getDirectory(node: TreeNode): StonefsDir {
  ...
  updateQuotaModal() {
    ...
    this.modalService.show(FormModalComponent, {
      ...
      onSubmit: (values: StonefsQuotas) => this.updateQuota(values)
    });
  }

  private getModalQuotaTitle(action: string, path: string): string {
    return $localize`${action} StoneFS ${this.getQuotaName()} quota for '${path}'`;
  }
  ...
  private updateQuota(values: StonefsQuotas, onSuccess?: Function) {
    const path = this.selectedDir.path;
    ...
    const action =
      this.selectedDir.quotas[key] === 0
        ...
        : $localize`Updated`;
    this.stonefsService.quota(this.id, path, values).subscribe(() => {
      if (onSuccess) {
        onSuccess();
      }
  ...
  createSnapshot() {
    ...
    this.modalService.show(FormModalComponent, {
      ...
      onSubmit: (values: StonefsSnapshot) => {
        this.stonefsService.mkSnapshot(this.id, path, values.name).subscribe((name) => {
          this.notificationService.show(
            NotificationType.success,
  ...
  private updateTreeStructure(dirs: StonefsDir[]) {
    const getChildrenAndPaths = (
      directories: StonefsDir[],
      parent: string
    ): { children: StonefsDir[]; paths: string[] } => {
  ...
  private removeOldDirectory(rmDir: StonefsDir) {
  ...
  private updateDirectoriesParentNode(dir: StonefsDir) {
  ...
  private addNewDirectory(newDir: StonefsDir) {
    this.dirs.push(newDir);
    this.nodeIds[newDir.path] = newDir;
    this.updateDirectoriesParentNode(newDir);
  }

  private updateExistingDirectory(source: StonefsDir[], updatedDir: StonefsDir) {
  ...
  deleteSnapshotModal() {
    this.modalRef = this.modalService.show(CriticalConfirmationModalComponent, {
      itemDescription: $localize`StoneFs Snapshot`,
      itemNames: this.snapshot.selection.selected.map((snapshot: StonefsSnapshot) => snapshot.name),
  ...
  deleteSnapshot() {
    const path = this.selectedDir.path;
    this.snapshot.selection.selected.forEach((snapshot: StonefsSnapshot) => {
      const name = snapshot.name;
      this.stonefsService.rmSnapshot(this.id, path, name).subscribe(() => {
        this.notificationService.show(
          NotificationType.success,
