...
from stone.deployment.service_spec import AlertManagerSpec, GrafanaSpec, ServiceSpec, SNMPGatewaySpec
from stoneadm.services.stoneadmservice import StoneadmService, StoneadmDaemonDeploySpec
from stoneadm.services.ingress import IngressSpec
from mgr_util import verify_tls, ServerConfigException, create_self_signed_cert, build_url

logger = logging.getLogger(__name__)


class GrafanaService(StoneadmService):
    TYPE = 'grafana'
    ...
    def generate_config(self, daemon_spec: StoneadmDaemonDeploySpec) -> Tuple[Dict[str, Any], List[str]]:
        ...
        grafana_data_sources = self.mgr.template.render(
            'services/grafana/stone-dashboard.yml.j2', {'hosts': prom_services})
        ...
        if not (cert and pkey):
            cert, pkey = create_self_signed_cert('Stone', 'stoneadm')
        ...
        config_file = {
            'files': {
                ...
                'provisioning/datasources/stone-dashboard.yml': grafana_data_sources,
                'certs/cert_file': '# generated by stoneadm\n%s' % cert,
                'certs/cert_key': '# generated by stoneadm\n%s' % pkey,
            }
        }
...
class AlertmanagerService(StoneadmService):
    TYPE = 'alertmanager'
...
class PrometheusService(StoneadmService):
    TYPE = 'prometheus'
    ...
    def generate_config(
        ...
        if os.path.exists(self.mgr.prometheus_alerts_path):
            with open(self.mgr.prometheus_alerts_path, 'r', encoding='utf-8') as f:
                alerts = f.read()
            r['files']['/etc/prometheus/alerting/stone_alerts.yml'] = alerts
...
class NodeExporterService(StoneadmService):
    TYPE = 'node-exporter'
...
class SNMPGatewayService(StoneadmService):
    TYPE = 'snmp-gateway'
