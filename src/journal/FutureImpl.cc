...
void FutureImpl::init(const stone::ref_t<FutureImpl> &prev_future) {
...
void FutureImpl::flush(Context *on_safe) {
  ...
  stone::ref_t<FutureImpl> prev_future;
  {
...
stone::ref_t<FutureImpl> FutureImpl::prepare_flush(FlushHandlers *flush_handlers) {
  std::lock_guard locker{m_lock};
...
int FutureImpl::get_return_value() const {
  std::lock_guard locker{m_lock};
  stone_assert(m_safe && m_consistent);
  return m_return_value;
}

bool FutureImpl::attach(FlushHandler::ref flush_handler) {
  std::lock_guard locker{m_lock};
  stone_assert(!m_flush_handler);
