...
#ifndef STONE_OSDC_WRITEBACKHANDLER_H
#define STONE_OSDC_WRITEBACKHANDLER_H
...
class WritebackHandler {
 public:
  ...
  virtual void read(const object_t& oid, uint64_t object_no,
		    ...
		    snapid_t snapid, stone::buffer::list *pbl, uint64_t trunc_size,
  ...
  virtual stone_tid_t write(const object_t& oid, const object_locator_t& oloc,
			   uint64_t off, uint64_t len,
			   const SnapContext& snapc,
			   const stone::buffer::list &bl, stone::real_time mtime,
  ...
  virtual stone_tid_t write(const object_t& oid, const object_locator_t& oloc,
			   std::vector<std::pair<uint64_t, stone::buffer::list> >& io_vec,
