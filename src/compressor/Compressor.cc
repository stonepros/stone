...
CompressorRef Compressor::create(StoneContext *cct, const std::string &type)
{
  // support "random" for teuthology testing
  if (type == "random") {
    int alg = stone::util::generate_random_number(0, COMP_ALG_LAST - 1);
  ...
  auto factory = dynamic_cast<stone::CompressionPlugin*>(reg->get_with_load("compressor", type));
  if (factory == NULL) {
...
CompressorRef Compressor::create(StoneContext *cct, int alg)
{
