...
{
  //cerr << "stone-syn starting" << std::endl;
  ...
  // get monmap
  stone::async::io_context_pool  poolctx(1);
  ...
  cout << "stone-syn: starting " << num_client << " syn client(s)" << std::endl;
  for (int i=0; i<num_client; i++) {
    messengers[i] = Messenger::create_client_messenger(g_stone_context,
						       "synclient");
    mclients[i] = new MonClient(g_stone_context, poolctx);
