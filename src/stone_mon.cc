...
static void usage()
{
  cout << "usage: stone-mon -i <ID> [flags]\n"
       << "  --debug_mon n\n"
...
entity_addrvec_t make_mon_addrs(entity_addr_t a)
{
  ...
  if (a.get_port() == 0) {
    ...
    addrs.v.push_back(a);
  } else if (a.get_port() == STONE_MON_PORT_LEGACY) {
    a.set_type(entity_addr_t::TYPE_LEGACY);
    addrs.v.push_back(a);
...
int main(int argc, const char **argv)
{
  ...
  int flags = 0;
  {
    ...
    for (std::vector<const char*>::iterator i = args_copy.begin();
	 i != args_copy.end(); ) {
      if (stone_argparse_double_dash(args_copy, i)) {
	break;
      } else if (stone_argparse_flag(args_copy, i, "--mkfs", (char*)NULL)) {
	flags |= CINIT_FLAG_NO_DAEMON_ACTIONS;
      } else if (stone_argparse_witharg(args_copy, i, &val, "--inject_monmap", (char*)NULL)) {
	flags |= CINIT_FLAG_NO_DAEMON_ACTIONS;
      } else if (stone_argparse_witharg(args_copy, i, &val, "--extract-monmap", (char*)NULL)) {
	flags |= CINIT_FLAG_NO_DAEMON_ACTIONS;
      } else {
	++i;
      }
  ...
  auto cct = global_init(&defaults, args,
			 STONE_ENTITY_TYPE_MON, CODE_ENVIRONMENT_DAEMON,
			 flags);
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ) {
    if (stone_argparse_double_dash(args, i)) {
      ...
      mkfs = true;
    } else if (stone_argparse_flag(args, i, "--compact", (char*)NULL)) {
      compact = true;
    } else if (stone_argparse_flag(args, i, "--force-sync", (char*)NULL)) {
      force_sync = true;
    } else if (stone_argparse_flag(args, i, "--yes-i-really-mean-it", (char*)NULL)) {
      yes_really = true;
    } else if (stone_argparse_witharg(args, i, &val, "--osdmap", (char*)NULL)) {
      osdmapfn = val;
    } else if (stone_argparse_witharg(args, i, &val, "--inject_monmap", (char*)NULL)) {
      inject_monmap = val;
    } else if (stone_argparse_witharg(args, i, &val, "--extract-monmap", (char*)NULL)) {
      extract_monmap = val;
    } else if (stone_argparse_witharg(args, i, &val, "--set-crush-location", (char*)NULL)) {
      crush_loc = val;
    } else {
      ++i;
    }
  ...
  if (mkfs) {
	...
	exit(1);
      ...
      *_dout << dendl;
      ...
      ostringstream oss;
      int err = monmap.build_initial(g_stone_context, true, oss);
      if (oss.tellp())
        derr << oss.str() << dendl;
      if (err < 0) {
	...
	entity_addr_t local;
	if (have_local_addr(g_stone_context, ls, &local)) {
    ...
    Monitor mon(g_stone_context, g_conf()->name.get_id(), &store, 0, 0, &monmap);
    r = mon.mkfs(osdmapbl);
    if (r < 0) {
  ...
  // make sure we aren't upgrading too fast
  {
    ...
    if (r >= 0 && val.size()) {
      stone_release_t from_release = stone_release_from_name(val);
      ostringstream err;
  ...
  if (strcmp(magic.c_str(), STONE_MON_ONDISK_MAGIC)) {
    derr << "mon fs magic '" << magic << "' != current '" << STONE_MON_ONDISK_MAGIC << "'" << dendl;
    prefork.exit(1);
  }
  ...
  if (!inject_monmap.empty()) {
    ...
    bufferlist mapbl;
    tmp.encode(mapbl, STONE_FEATURES_ALL);
    bufferlist final;
    encode(v, final);
    encode(mapbl, final);
  ...
  if (monmap.contains(g_conf()->name.get_id())) {
    ...
    if (!g_conf()->public_addrv.empty()) {
      ...
      MonMap tmpmap;
      ostringstream oss;
      int err = tmpmap.build_initial(g_stone_context, true, oss);
      if (oss.tellp())
        derr << oss.str() << dendl;
      if (err < 0) {
	...
	derr << "no public_addr or public_network specified, and "
	     << g_conf()->name << " not present in monmap or stone.conf" << dendl;
	prefork.exit(1);
      }
    }
  }
  ...
  Messenger *msgr = Messenger::create(g_stone_context, public_msgr_type,
				      entity_name_t::MON(rank), "mon", 0);
  if (!msgr)
    exit(1);
  msgr->set_cluster_protocol(STONE_MON_PROTOCOL);
  msgr->set_default_send_priority(STONE_MSG_PRIO_HIGH);
  ...
  Throttle *client_throttler = new Throttle(g_stone_context, "mon_client_bytes",
  ...
  Throttle *daemon_throttler = new Throttle(g_stone_context, "mon_daemon_bytes",
  ...
  Messenger *mgr_msgr = Messenger::create(g_stone_context, public_msgr_type,
  ...
  mon = new Monitor(g_stone_context, g_conf()->name.get_id(), store,
  ...
  if ((mkdir(s, 0755) == 0) && (chdir(s) == 0)) {
    dout(0) << "stone-mon: gmon.out should be in " << s << dendl;
  }
