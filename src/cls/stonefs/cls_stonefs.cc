...
// generic, so just fess up and call this the stonefs filter.
class PGLSStoneFSFilter : public PGLSFilter {
protected:
...
bool PGLSStoneFSFilter::filter(const hobject_t &obj,
...
CLS_INIT(stonefs)
{
  ...
  // log file.
  CLS_LOG(0, "loading stonefs");

  cls_handle_t h_class;
  cls_method_handle_t h_accumulate_inode_metadata;

  cls_register("stonefs", &h_class);
