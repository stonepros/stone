...
int ClsStoneFSClient::accumulate_inode_metadata(
  librados::IoCtx &ctx,
  inodeno_t inode_no,
  ...
  const time_t mtime)
{
  ...
  bufferlist inbl;
  args.encode(inbl);
  op.exec("stonefs", "accumulate_inode_metadata", inbl);
...
int ClsStoneFSClient::delete_inode_accumulate_result(
    librados::IoCtx &ctx,
...
int ClsStoneFSClient::fetch_inode_accumulate_result(
  librados::IoCtx &ctx,
  const std::string &oid,
...
{
  stone_assert(backtrace != NULL);
  stone_assert(result != NULL);
...
void ClsStoneFSClient::build_tag_filter(
          ...
          bufferlist *out_bl)
{
  ...
  encode(std::string("stonefs.inode_tag"), *out_bl);
