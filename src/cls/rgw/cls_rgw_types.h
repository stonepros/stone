...
template<class T>
void encode_packed_val(T val, stone::buffer::list& bl)
{
  using stone::encode;
...
template<class T>
void decode_packed_val(T& val, stone::buffer::list::const_iterator& bl)
{
  using stone::decode;
  unsigned char c;
  decode(c, bl);
...
struct cls_rgw_gc_obj_info
{
  ...
  static void generate_test_instances(std::list<cls_rgw_gc_obj_info*>& ls) {
    ...
    ls.push_back(new cls_rgw_gc_obj_info);
    ls.back()->tag = "footag";
    stone_timespec ts{init_le32(21), init_le32(32)};
    ls.back()->time = stone::real_clock::from_stone_timespec(ts);
  }
};
