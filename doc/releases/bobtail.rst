...
* stone-fuse: create finisher flags after fork()
...
* stone-disk[-prepare,-activate] behavior has changed in various ways.
...
* stone-disk: support for dm-crypt
* stone-disk: many backports to allow bobtail deployments with stone-deploy, chef
...
* There is a fix in the syntax for the output of 'stone osd tree --format=json'.
...
* The MDS disk format has changed from prior releases *and* from v0.57.  In particular,
  ...
  the MDS for StoneFS; you must upgrade directly to v0.58 (or later) instead.
...
* stone-fuse: report file system size with correct units
* mds: get and set directory layout policies via virtual xattrs
* mds: on-disk format revision (see upgrading note above)
* mkstonefs, init-stone: close potential security issues with predictable filenames
...
* A bug was fixed in which the OSDMap epoch for PGs without any IO
  ...
  pools normally used by StoneFS), and a large number of OSDMap epochs
  have elapsed since the ``stone-osd`` daemon was last restarted, those
...
* mds, stone-fuse: allow file and directory layouts to be modified via virtual xattrs
...
* The meaning of the 'osd scrub min interval' and 'osd scrub max
  ...
  adjusted accordingly.  If you have customized these in stone.conf,
  please review their values when upgrading.

* CRUSH maps that are generated by default when calling ``stone-mon
  --mkfs`` directly now distribute replicas across hosts instead of
  across OSDs.  Any provisioning tools that are being used by Stone may
  ...
  ``mkstonefs`` to create the cluster, the default CRUSH rule is still
  inferred by the number of hosts and/or racks in the initial stone.conf.
...
* stone command: ability to easily adjust CRUSH tunables
...
Bobtail is the second stable release of Stone, named in honor of the
...
* mkstonefs: support for automatically formatting and mounting XFS and ext4 (in addition to btrfs)
...
.. _Upgrading from Argonaut to Bobtail: ../install/upgrading-stone/#upgrading-from-argonaut-to-bobtail

* Stonex authentication is now enabled by default (since v0.55).
  Upgrading a cluster without adjusting the Stone configuration will
...
* Stone daemons can be upgraded one-by-one while the cluster is online
  and in service.

* The ``stone-osd`` daemons must be upgraded and restarted *before* any
  ``radosgw`` daemons are restarted, as they depend on some new
  stone-osd functionality.  (The ``stone-mon``, ``stone-osd``, and
  ``stone-mds`` daemons can be upgraded and restarted in any order.)

* Once each individual daemon has been upgraded and restarted, it
  cannot be downgraded.

* The cluster of ``stone-mon`` daemons will migrate to a new internal
  ...
  extended period of time between ``stone-mon`` upgrades.
...
* You should not create or use "format 2" RBD images until after all
  ``stone-osd`` daemons have been upgraded.  Note that "format 1" is
  still the default.  You can use the new ``stone osd ls`` and
  ``stone tell osd.N version`` commands to doublecheck your cluster.
  ``stone osd ls`` will give a list of all OSD IDs that are part of the
  cluster, and you can use that to write a simple shell loop to display
  all the OSD version strings: ::

      for i in $(stone osd ls); do
          stone tell osd.${i} version
      done
...
* The 'stone osd create [<uuid>]' command now rejects an argument that
  is not a UUID.  (Previously it would take take an optional integer
  OSD id.)  This correct syntax has been 'stone osd create [<uuid>]'
...
* The CRUSH map root nodes now have type ``root`` instead of type
  ...
  directly related.  Any scripts or tools that use the ``stone osd
  crush ...`` commands may need to be adjusted accordingly.

* The ``stone osd pool create <poolname> <pgnum>`` command now requires
...
* The sysvinit start/stop script now defaults to adjusting the max
  ...
  ``stone.conf`` file in the appropriate section.  For example::
...
* The osd capabilities associated with a rados user have changed
  ...
  * restricting pool access by pool owner ('allow r uid=foo') is
  ...
  The new format is documented in the stone-authtool man page.
...
* 'stone osd tree' now uses 4 decimal places for weight so output is
  nicer for humans

* Several monitor operations are now idempotent:

  * stone osd pool create
  * stone osd pool delete
  * stone osd pool mksnap
  * stone osd rm
  * stone pg <pgid> revert

Notable changes
---------------

* auth: enable stonex by default
* auth: expanded authentication settings for greater flexibility
* auth: sign messages when using stonex
* build fixes for Fedora 18, CentOS/RHEL 6
* stone: new 'osd ls' and 'osd tell <osd.N> version' commands
* stone-debugpack: misc improvements
* stone-disk-prepare: creates and labels GPT partitions
* stone-disk-prepare: support for external journals, default mount/mkfs options, etc.
* stone-fuse/libstonefs: many misc fixes, admin socket debugging
* stone-fuse: fix handling for .. in root directory
* stone-fuse: many fixes (including memory leaks, hangs)
* stone-fuse: mount helper (mount.fuse.stone) for use with /etc/fstab
* stone.spec: misc packaging fixes
...
* mds: misc fixes
* mkstonefs: fix for default keyring, osd data/journal locations
* mkstonefs: support for formatting xfs, ext4 (as well as btrfs)
* init: support for automatically mounting xfs and ext4 osd data directories
* mon, radosgw, stone-fuse: fixed memory leaks
* mon: improved ENOSPC, fs error checking
* mon: less-destructive stone-mon --mkfs behavior
* mon: misc fixes
...
* mon: many bug fixes (various races causing stone-mon crashes)
...
* wireshark: stone protocol dissector patch updated
...
* Bug fixes to the new osd capability format parsing properly validate
  ...
  misconfigured. See the stone-authtool man page for details on osd
  capabilities.
