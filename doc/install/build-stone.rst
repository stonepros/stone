...
You can get Stone software by retrieving Stone source code and building it yourself.
To build Stone, you need to set up a development environment, compile Stone,
...
A debug build of Stone may take around 40 gigabytes. If you want to build Stone in
...
Before you can build Stone source code, you need to install several libraries
and tools::
...
Stone is built using cmake. To build Stone, navigate to your cloned Stone
...
Build Stone Packages
===================

To build packages, you must clone the `Stone`_ repository. You can create 
...
`Stone`_ repository, installed the `Build Prerequisites`_ and installed 
``debhelper``::
...
To create ``.rpm`` packages, ensure that you have cloned the `Stone`_ repository,
