...
StoneFS provides `top(1)` like utility to display various Stone Filesystem metrics
in realtime. `cephfs-top` is a curses based python script which makes use of `stats`
plugin in Stone Manager to fetch (and display) metrics.

Manager Plugin
--------------

Stone Filesystem clients periodically forward various metrics to Stone Metadata Servers (MDS)
which in turn get forwarded to Stone Manager by MDS rank zero. Each active MDS forward its
respective set of metrics to MDS rank zero. Metrics are aggergated and forwarded to Stone
Manager.
...
Once enabled, Stone Filesystem metrics can be fetched via::
...
- `client_metadata`: Stone Filesystem client metadata
...
By default, `cephfs-top` uses `client.fstop` user to connect to a Stone cluster::
...
.. note:: As of now, `cephfs-top` does not reliably work with multiple Stone Filesystems.
