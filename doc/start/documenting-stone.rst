...
The **easiest way** to help the Stone project is to contribute to the
documentation. As the Stone user base grows and the development pace quickens, an
...
instructions will help the Stone project immensely.

The Stone documentation source resides in the ``ceph/doc`` directory of the Stone
...
Stone documentation lives in the Stone repository right alongside the Stone source
code under the ``ceph/doc`` directory. For details on github and Stone,
...
#. Fork the Stone project. See https://github.com/ceph/ceph.

#. Clone your fork of the Stone project to your local host.


Stone organizes documentation into an information architecture primarily by its
main components.

- **Stone Storage Cluster:** The Stone Storage Cluster documentation resides
  under the ``doc/rados`` directory.
  
- **Stone Block Device:** The Stone Block Device documentation resides under
  the ``doc/rbd`` directory.
  
- **Stone Object Storage:** The Stone Object Storage documentation resides under
  the ``doc/radosgw`` directory.

- **Stone File System:** The Stone File System documentation resides under the 
...
tracker ID; or, if you want to see your documentation rendered on the Stone.com
...
Stone uses Python Sphinx, which is generally distribution agnostic. The first
time you build Stone documentation, it will generate a doxygen XML tree, which
...
Stone documentation makes extensive use of `ditaa`_, which is not presently built
...
Stone documentation commits are simple, but follow a strict convention:
...
pushing them to your forked Stone repository.
...
One objective of the Stone documentation project is to ensure the readability of
the documentation in both native restructuredText format and its rendered
formats such as HTML. Navigate to your Stone repository and view a document in
...
#. **Code Examples:** Stone supports the use of the 
...
The Stone project uses `paragraph level markup`_ to highlight points.
...
The Stone project uses the ``.. toctree::`` directive. See `The TOC tree`_
...
Stone documentation also uses the backtick (accent grave) character followed by
