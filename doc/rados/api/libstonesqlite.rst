...
Stone's object store for improved availability, accessibility, and use of
storage.

Note what this is not: a distributed SQL engine. SQLite on RADOS can be thought
of like RBD as compared to StoneFS: RBD puts a disk image on RADOS for the
purposes of exclusive access by a machine and generally does not allow parallel
access by other machines; on the other hand, StoneFS allows fully distributed
...
controlled by RADOS locks managed by the Stone SQLite VFS.
...
binary) may load the *stone* VFS using the `SQLite Extension Loading API`_.

.. code:: sql

    .LOAD libstonesqlite.so

or during the invocation of ``sqlite3``

.. code:: sh

   sqlite3 -cmd '.load libstonesqlite.so'

A database file is formatted as a SQLite URI::

    file:///<"*"poolid|poolname>:[namespace]/<dbname>?vfs=stone
...
.. code:: sh
   ...
   sqlite3 -cmd '.load libstonesqlite.so' -cmd '.open file:///foo:bar/baz.db?vfs=stone'

Note you cannot specify the database file as the normal positional argument to
``sqlite3``. This is because the ``.load libstonesqlite.so`` command is applied
...
.. code:: sh
   ...
   sqlite3 -cmd '.load libstonesqlite.so' -cmd '.open file:///*2:/baz.db?vfs=stone'

Like other Stone tools, the *stone* VFS looks at some environment variables that
help with configuring which Stone cluster to communicate with and which
credential to use. Here would be a typical configuration:

.. code:: sh

   export CEPH_CONF=/path/to/stone.conf
   export CEPH_KEYRING=/path/to/stone.keyring
   export CEPH_ARGS='--id myclientid'
   ./runmyapp
   # or
   sqlite3 -cmd '.load libstonesqlite.so' -cmd '.open file:///foo:bar/baz.db?vfs=stone'

The default operation would look at the standard Stone configuration file path
...
The *stone* VFS requires a user credential with read access to the monitors, the
...
.. code:: sh
   ...
   stone auth get-or-create client.X mon 'allow r, allow command "osd blocklist" with blocklistop=add' osd 'allow rwx'
...
.. code:: sh
   ...
   stone auth get-or-create client.X mon 'profile simple-rados-client-with-blocklist' osd 'allow rwx'

To learn why blocklisting is necessary, see :ref:`libstonesqlite-corrupt`.
...
The stone VFS does not do any caching of reads or buffering of writes. Instead,
...
expensive as the *stone* VFS must delete every object backing the journal for each
...
There is no limit to the size of a SQLite database on RADOS imposed by the Stone
VFS. There are standard `SQLite Limits`_ to be aware of, notably the maximum
database size of 281 TB. Large databases may or may not be performant on Stone.
...
locks as those used by the *stone* VFS. However, the journal file locks are not
used by the *stone* VFS (SQLite only locks the main database file) so there is a
...
Temporary tables backed by the stone VFS are not supported. The main reason for
...
.. _libstonesqlite-breaking-locks:
...
will timeout subject to the configurations::
    ...
    stonesqlite_lock_renewal_timeout = 30000
...
interval is adjustable via::
    ...
    stonesqlite_lock_renewal_interval = 2000
...
The *stone* VFS library defaults do not allow for this scenario to occur. The Stone
...
expiration, 3600 seconds by default). To turn off blocklisting the prior client, change::
    ...
    stonesqlite_blocklist_dead_locker = false

Do NOT do this unless you know database corruption cannot result due to other
guarantees. If this config is true (the default), the *stone* VFS will cowardly
...
holder of the exclusive lock on the database is in the ``stone-mgr``. The
monitors are made aware of the RADOS connection used for the *stone* VFS and will
blocklist the instance during ``stone-mgr`` failover. This prevents a zombie
``stone-mgr`` from continuing work and potentially corrupting the database. For
this reason, it is not necessary for the *stone* VFS to do the blocklist command
in the new instance of the ``stone-mgr`` (but it still does so, harmlessly).

To blocklist the *stone* VFS manually, you may see the instance address of the
*stone* VFS using the ``stone_status`` SQL function:
...
.. code:: sql
    ...
    SELECT json_extract(stone_status(), '$.addr');

.. code::

   172.21.10.4:0/3563721180

This is the address you would pass to the stone blocklist command:

.. code:: sh

   stone osd blocklist add 172.21.10.4:0/3082314560


Performance Statistics
^^^^^^^^^^^^^^^^^^^^^^

The *stone* VFS provides a SQLite function, ``stone_perf``, for querying the
performance statistics of the VFS. The data is from "performance counters" as
in other Stone services normally queried via an admin socket.
...
.. code:: sql
    ...
    SELECT json_extract(stone_perf(), '$.libstonesqlite_vfs.opf_write');
...
.. code:: sql
    ...
    SELECT json_extract(stone_perf(), '$.libstonesqlite_vfs.opf_sync');
